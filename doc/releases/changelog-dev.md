...
* The performance of the `qml.estimate` function has been significantly improved by memoizing the resource trace calculation for resource operators. This avoids redundant computations when the same operator type and parameters appear multiple times in a workflow.
