...
* Operations for quantum chemistry and embedding templates now support parameter broadcasting.
  ...
  also work with broadcasted parameters and can return the corresponding numerical
  representations, like their `matrix` with a leading broadcasting dimension:
  ...
  Similarly, embedding templates like `AmplitudeEmbedding` or `IQPEmbedding` now support
