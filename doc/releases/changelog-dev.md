...
* The ``control_wires`` argument to ``qml.MultiControlledX`` is deprecated. Instead, use the ``wires`` argument to 
  specify all wires with the target wire at the end, the same as in other controlled operations
  such as ``Toffoli``. 
