...
* Development of a circuit-cutting compiler extension to circuits with sampling
  ...
  - A user facing transform for cutting circuit with sample measurements has
    been added. A `qnode` containing `WireCut` operations and `sample` measurements can be decorated with `@cut_circuit_mc()` to perform this type of cutting.
