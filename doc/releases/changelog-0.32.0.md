...
* It is now possible to encode an operator `A` into a quantum circuit by decomposing it into
  ...
  ([qp.StatePrep](https://docs.pennylane.ai/en/stable/code/api/pennylane.StatePrep.html)) and
  SELECT ([qp.Select](https://docs.pennylane.ai/en/stable/code/api/pennylane.Select.html)) 
  routines.
  ...
  ```pycon
  >>> A = qp.PauliX(2) + 2 * qp.PauliY(2) + 3 * qp.PauliZ(2)
  ```
  ...
  def block_encode(A, control_wires):
      ...
      qp.StatePrep(state, wires=control_wires)
      qp.Select(unitaries, control=control_wires)
      qp.adjoint(qp.StatePrep)(state, wires=control_wires)
  ```

  ```pycon
  >>> print(qp.draw(block_encode, show_matrices=False)(A, control_wires=[0, 1]))
  0: ─╭|Ψ⟩─╭Select─╭|Ψ⟩†─┤
  1: ─╰|Ψ⟩─├Select─╰|Ψ⟩†─┤
  2: ──────╰Select───────┤
  ```
...
* Decomposing a Hermitian matrix into a linear combination of Pauli words via `qp.pauli_decompose` is 
...
* Python-native logging can now be enabled with `qp.logging.enable_logging()`.
  ...
  qp.logging.enable_logging()  # enables logging
  ...
  configuration file, which can be located by running `qp.logging.config_path()`. Follow our
...
* Input states obtained from advanced quantum chemistry calculations can be used in a circuit. 
  ...
  [qp.qchem.import_state](https://docs.pennylane.ai/en/stable/code/api/pennylane.qchem.import_state.html)
  ...
  The state vector can be implemented in a circuit using `qp.StatePrep`.

  ```pycon
  >>> dev = qp.device('default.qubit', wires=4)
  >>> @qp.qnode(dev)
  ... def circuit():
  ...     qp.StatePrep(wf_cisd, wires=range(4))
  ...     return qp.state()
  >>> print(circuit())
  [ 0.        +0.j  0.        +0.j  0.        +0.j  0.1066467 +0.j
    1.        +0.j  0.        +0.j  0.        +0.j  0.        +0.j
...
* PennyLane now allows you to define circuits that reuse a qubit after a mid-circuit
  ...
  [qp.measure](https://docs.pennylane.ai/en/stable/code/api/pennylane.measure.html).
...
* Circuit drawings and plots can now be created following a PennyLane style.
  ...
  The `qp.draw_mpl` function accepts a `style='pennylane'` argument to create PennyLane themed
  circuit diagrams:
  ...
  qp.draw_mpl(circuit, style="pennylane")(1, 1)
  ```
...
* Any class inheriting from `Operator` is now automatically registered as a pytree with JAX.
  ...
  ```pycon
  >>> op = qp.adjoint(qp.RX(1.0, wires=0))
  >>> jax.jit(qp.matrix)(op)
...
* The `qp.gradients` module no longer mutates operators in-place for any gradient transforms.
...
* `qp.equal` no longer raises errors when operators or measurements of different types are compared.
...
* Transform programs are now integrated with the QNode.
  ...
  ```python
  def null_postprocessing(results: qp.typing.ResultBatch) -> qp.typing.Result:
      return results[0]

  @qp.transforms.core.transform
  def scale_shots(tape: qp.tape.QuantumTape, shot_scaling) -> (Tuple[qp.tape.QuantumTape], Callable):
      new_shots = tape.shots.total_shots * shot_scaling
      new_tape = qp.tape.QuantumScript(tape.operations, tape.measurements, shots=new_shots)
      return (new_tape, ), null_postprocessing

  dev = qp.devices.experimental.DefaultQubit2()
...
* Transform Programs, `qp.transforms.core.TransformProgram`, can now be called on a batch of circuits
...
* QNode transforms in `qp.qinfo` now support custom wire labels.
  [#4331](https://github.com/PennyLaneAI/pennylane/pull/4331)

* `qp.transforms.adjoint_metric_tensor` now uses the simulation tools in `qp.devices.qubit` instead of
  private methods of `qp.devices.DefaultQubit`.
  [(#4456)](https://github.com/PennyLaneAI/pennylane/pull/4456)

* Auxiliary wires and device wires are now treated the same way in `qp.transforms.metric_tensor`
  as in `qp.gradients.hadamard_grad`. All valid wire input formats for `aux_wire` are supported.
...
* New functions called `adjoint_jvp` and `adjoint_vjp` that compute the JVP and VJP of a tape using the adjoint method 
  have been added to `qp.devices.qubit.adjoint_jacobian` 
...
* `qp.devices.experimental.Device` now accepts a shots keyword argument and has a `shots`
...
* `qp.interfaces.set_shots` now accepts a `Shots` object as well as `int`'s and tuples of `int`'s.
  [(#4388)](https://github.com/PennyLaneAI/pennylane/pull/4388)
...
* `qp.pulse.transmon_drive` has been updated in accordance with [1904.06560](https://arxiv.org/abs/1904.06560).
...
* A function called `qp.math.fidelity_statevector` that computes the fidelity between two state vectors has been added.
  [(#4322)](https://github.com/PennyLaneAI/pennylane/pull/4322)

* `qp.ctrl(qp.PauliX)` returns a `CNOT`, `Toffoli`, or `MultiControlledX` operation instead of `Controlled(PauliX)`.
  [(#4339)](https://github.com/PennyLaneAI/pennylane/pull/4339)

* When given a callable, `qp.ctrl` now does its custom pre-processing on all queued operators from the callable.
...
* `Device.default_expand_fn()` has been updated to decompose `qp.StatePrep` operations present in the middle of a provided circuit.
  [(#4437)](https://github.com/PennyLaneAI/pennylane/pull/4437)

* `QNode.construct` has been updated to only apply the `qp.defer_measurements` transform if the device
  does not natively support mid-circuit measurements.
  [(#4516)](https://github.com/PennyLaneAI/pennylane/pull/4516)

* The application of the `qp.defer_measurements` transform has been moved from 
  `QNode.construct` to `qp.Device.batch_transform` to allow more fine-grain 
...
* The `do_queue` keyword argument in `qp.operation.Operator` has been removed. Instead of
...
* `qp.StatePrep` has been renamed to `qp.StatePrepBase` and `qp.QubitStateVector` has been renamed to `qp.StatePrep`.
  `qp.operation.StatePrep` and `qp.QubitStateVector` are still accessible.
  [(#4450)](https://github.com/PennyLaneAI/pennylane/pull/4450)
...
* `qp.math.reduced_dm` has been removed. Use `qp.math.reduce_dm` or `qp.math.reduce_statevector` instead.
  [(#4301)](https://github.com/PennyLaneAI/pennylane/pull/4301)

* The `qp.specs` dictionary no longer supports direct key access to certain keys. 
  [(#4301)](https://github.com/PennyLaneAI/pennylane/pull/4301)
...
* `qp.enable_return` and `qp.disable_return` have been deprecated. Please avoid calling
...
* `qp.qchem.jordan_wigner` has been deprecated. Use `qp.jordan_wigner` instead.
  List input to define the fermionic operator has also been deprecated; the fermionic
  operators in the `qp.fermi` module should be used instead.
  [(#4332)](https://github.com/PennyLaneAI/pennylane/pull/4332)

* The `qp.RandomLayers.compute_decomposition` keyword argument `ratio_imprimitive` will be changed to `ratio_imprim` to
  match the call signature of the operation.
  [(#4314)](https://github.com/PennyLaneAI/pennylane/pull/4314)

* The CV observables `qp.X` and `qp.P` have been deprecated. Use `qp.QuadX`
  and `qp.QuadP` instead.
...
* The `sampler_seed` argument of `qp.gradients.spsa_grad` has been deprecated, along with a bug
...
* The `qp.pulse.transmon_interaction` and `qp.pulse.transmon_drive` documentation has been updated.
  [#4327](https://github.com/PennyLaneAI/pennylane/pull/4327)

* `qp.ApproxTimeEvolution.compute_decomposition()` now has a code example.
  [(#4354)](https://github.com/PennyLaneAI/pennylane/pull/4354)

* The documentation for `qp.devices.experimental.Device` has been improved to clarify
  some aspects of its use.
  [(#4391)](https://github.com/PennyLaneAI/pennylane/pull/4391)

* Input types and sources for operators in `qp.import_operator` are specified.
  [(#4476)](https://github.com/PennyLaneAI/pennylane/pull/4476)

<h3>Bug fixes 🐛</h3>

* `qp.Projector` is pickle-able again.
...
* `op.adjoint` has been replaced with `qp.adjoint` in `QNSPSAOptimizer`.
...
* A warning is now raised if control indicators are hidden when calling `qp.draw_mpl`
  [(#4295)](https://github.com/PennyLaneAI/pennylane/pull/4295)

* `qp.qinfo.purity` now produces correct results with custom wire labels.
  [(#4331)](https://github.com/PennyLaneAI/pennylane/pull/4331)

* `default.qutrit` now supports all qutrit operations used with `qp.adjoint`.
  [(#4348)](https://github.com/PennyLaneAI/pennylane/pull/4348)

* The observable data of `qp.GellMann` now includes its index, allowing correct comparison
  between instances of `qp.GellMann`, as well as Hamiltonians and Tensors
  containing `qp.GellMann`.
  [(#4366)](https://github.com/PennyLaneAI/pennylane/pull/4366)

* `qp.transforms.merge_amplitude_embedding` now works correctly when the `AmplitudeEmbedding`s
...
* When a `style` option is not provided, `qp.draw_mpl` uses the current style set from
  `qp.drawer.use_style` instead of `black_white`.
  [(#4357)](https://github.com/PennyLaneAI/pennylane/pull/4357)

* `qp.devices.qubit.preprocess.validate_and_expand_adjoint` no longer sets the
  trainable parameters of the expanded tape.
  [(#4365)](https://github.com/PennyLaneAI/pennylane/pull/4365)

* `qp.default_expand_fn` now selectively expands operations or measurements allowing more 
...
* `qp.ControlledQubitUnitary` no longer reports `has_decomposition` as `True` when it does
  not really have a decomposition.
  [(#4407)](https://github.com/PennyLaneAI/pennylane/pull/4407)

* `qp.transforms.split_non_commuting` now correctly works on tapes containing both `expval`
...
* The `sampler_seed` argument of `qp.gradients.spsa_grad` has been changed to `sampler_rng`. One can either provide
...
* `qp.math.get_dtype_name` now works with autograd array boxes.
  [(#4494)](https://github.com/PennyLaneAI/pennylane/pull/4494)

* The backprop gradient of `qp.math.fidelity` is now correct.
