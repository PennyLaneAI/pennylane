...
* Approximate matrix exponentiation with random product formulas is available with the new `QDrift`
  operation.
...
* Measurement statistics can now be collected for mid-circuit measurements. Currently,
  ...
  `default.qubit`, `default.mixed`, and `default.qubit.legacy` devices.
...
* Various changes to measurements to improve feature parity between the legacy `default.qubit.legacy` and
  the new `default.qubit`. This includes not trying to squeeze batched `CountsMP` results and implementing
