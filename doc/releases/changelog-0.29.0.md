...
* Support for creating pulse-based circuits that describe evolution under a time-dependent
  ...
  A time-dependent Hamiltonian can be created using `qp.pulse.ParametrizedHamiltonian`, which 
  ...
  The time-dependent Hamiltonian can be used within a circuit with `qp.evolve`:
  ...
  ```pycon
  >>> dev = qp.device("default.qubit.jax", wires=2)
  >>> qnode = qp.QNode(pulse_circuit, dev, interface="jax")
  >>> params = (p1, p2)
  ...
  Check out the [qp.pulse](https://docs.pennylane.ai/en/stable/code/qml_pulse.html) documentation
  page for more details!

<h4>Special unitary operation 🌞</h4>

* A new operation `qp.SpecialUnitary` has been added, providing access to an arbitrary
  ...
  `qp.SpecialUnitary` creates a unitary that exponentiates a linear combination of all possible Pauli words in lexicographical order — except for the identity operator — for `num_wires` wires, of which there are `4**num_wires - 1`. As its first argument,
  `qp.SpecialUnitary` takes a list of the `4**num_wires - 1` parameters that are the coefficients of the linear combination. 

  To see all possible Pauli words for `num_wires` wires, you can use the `qp.ops.qubit.special_unitary.pauli_basis_strings` function:
  ...
  To use `qp.SpecialUnitary`, for example, on a single qubit, we may define

  ```pycon
  >>> thetas = np.array([0.2, 0.1, -0.5])
  >>> U = qp.SpecialUnitary(thetas, 0)
  >>> qp.matrix(U)
  ...
  >>> rx = qp.RX(-2 * x, 0) # RX introduces a prefactor -0.5 that has to be compensated
  >>> qp.math.allclose(qp.matrix(su), qp.matrix(rx))
  True
  ```
  ...
  visiting the
  [qp.SpecialUnitary](https://docs.pennylane.ai/en/stable/code/api/pennylane.SpecialUnitary.html)
  documentation.

<h4>Always differentiable 📈</h4>

* The Hadamard test gradient transform is now available via `qp.gradients.hadamard_grad`. This transform 
  ...
  `qp.gradients.hadamard_grad` is a hardware-compatible transform that calculates the
  ...
  >>> qp.gradients.hadamard_grad(circuit)(params)
  ...
  ```pycon
  >>> dev = qp.device("default.qubit", wires=2)
  >>> @qp.qnode(dev, interface="jax", diff_method="hadamard")
  ... def circuit(params):
  ...     qp.RX(params[0], wires=0)
  ...     qp.RY(params[1], wires=0)
  ...     qp.RX(params[2], wires=0)
...
* The gradient transform `qp.gradients.spsa_grad` is now registered as a
  ...
  >>> @qp.qnode(dev, interface="jax", diff_method="spsa", h=0.05, num_directions=20)
  ... def circuit(x):
  ...     qp.RX(x, 0)
  ...     return qp.expval(qp.PauliZ(0))
  ...
  Note: The full SPSA optimization method is already available as `qp.SPSAOptimizer`.
...
* The JAX-JIT interface now supports higher-order gradient computation with the new return types system.
  ...
  @jax.jit
  @qp.qnode(dev, interface="jax-jit", diff_method="parameter-shift", max_diff=2)
  def circuit(a, b):
      qp.RY(a, wires=0)
      qp.RX(b, wires=1)
...
* The `qchem` workflow has been modified to support both Autograd and JAX frameworks.
  ...
  ```pycon
  >>> jax.grad(qp.qchem.hf_energy(mol))(*args)
  Array([[ 0.       ,  0.       ,  0.3650435],
         [ 0.       ,  0.       , -0.3650435]], dtype=float64)
  ```

* The kernel matrix utility functions in `qp.kernels` are now autodifferentiation-compatible.
  ...
  ```python
  dev = qp.device('default.qubit', wires=2, shots=(100, 100))
  @qp.qnode(dev)
  def circuit(x1, x2):
  ...
  >>> qp.kernels.square_kernel_matrix(X, kernel)[:, 0]
...
* Hamiltonian evolution using `qp.evolve` or `qp.exp` can now be decomposed into operations.
  ...
  ```pycon
  >>> qp.evolve(qp.sum(qp.PauliX(0), qp.PauliY(0), qp.PauliZ(0)), num_steps=2).decomposition()
  [RX((1+0j), wires=[0]),
   RY((1+0j), wires=[0]),
   RZ((1+0j), wires=[0]),
   RX((1+0j), wires=[0]),
...
* A new method called `qp.qchem.givens_decomposition` has been added, which decomposes a unitary into a sequence
  ...
  phase_mat, ordered_rotations = qp.qchem.givens_decomposition(unitary)
  ```
...
* A new template called `qp.BasisRotation` has been added, which performs a basis transformation defined by a set of
  ...
  ```pycon
  >>> circ_unitary = qp.matrix(circuit)()
...
* A new function called `qp.qchem.load_basisset` has been added to extract `qp.qchem` basis set data from the Basis Set Exchange
  library.
  [(#3363)](https://github.com/PennyLaneAI/pennylane/pull/3363)

* A new function called `qp.math.max_entropy` has been added to compute the maximum entropy of a quantum state.
  [(#3594)](https://github.com/PennyLaneAI/pennylane/pull/3594)

* A new template called `qp.TwoLocalSwapNetwork` has been added that implements a canonical 2-complete linear (2-CCL) swap network
  ...
  weights = np.random.random(size=qp.templates.TwoLocalSwapNetwork.shape(len(dev.wires)))
  ...
  >>> print(qp.draw(swap_network_circuit, expansion_strategy = 'device')())
...
* A new function called `qp.pulse.pwc` has been added as a convenience function for defining a `qp.pulse.ParametrizedHamiltonian`.
...
* A new function called`qp.pulse.pwc_from_function` has been added as a decorator for defining a
  ...
  ```pycon
  >>> @qp.pulse.pwc_from_function((2, 4), num_bins=10)
  ... def f1(p, t):
  ...     return p * t
  ```
...
* The function `qp.dot` has been updated to compute the dot product between a vector and a list of operators.
  [(#3586)](https://github.com/PennyLaneAI/pennylane/pull/3586)
...
* `qp.evolve` returns the evolution of an `Operator` or a `ParametrizedHamiltonian`.
  [(#3617)](https://github.com/PennyLaneAI/pennylane/pull/3617)
  [(#3706)](https://github.com/PennyLaneAI/pennylane/pull/3706)

* `qp.ControlledQubitUnitary` now inherits from `qp.ops.op_math.ControlledOp`, which defines `decomposition`, `expand`, and `sparse_matrix` rather than raising an error.
  [(#3450)](https://github.com/PennyLaneAI/pennylane/pull/3450)

* Parameter broadcasting support has been added for the `qp.ops.op_math.Controlled` class if the base operator supports
  broadcasting.
  [(#3450)](https://github.com/PennyLaneAI/pennylane/pull/3450)

* The `qp.generator` function now checks if the generator is Hermitian, rather than whether it is a subclass of
  `Observable`. This allows it to return valid generators from `SymbolicOp` and `CompositeOp` classes.
  [(#3485)](https://github.com/PennyLaneAI/pennylane/pull/3485)

* The `qp.equal` function has been extended to compare `Prod` and `Sum` operators.
  [(#3516)](https://github.com/PennyLaneAI/pennylane/pull/3516)

* `qp.purity` has been added as a measurement process for purity
...
* The `qp.utils.sparse_hamiltonian` function has been moved to thee `qp.Hamiltonian.sparse_matrix` method.
  [(#3585)](https://github.com/PennyLaneAI/pennylane/pull/3585)

* The `qp.pauli.PauliSentence.operation()` method has been improved to avoid instantiating an `SProd` operator when
...
* The `qp.GellMann` operators now include their index when displayed.
  [(#3641)](https://github.com/PennyLaneAI/pennylane/pull/3641)

* `qp.ops.ctrl_decomp_zyz` has been added to compute the decomposition of a controlled single-qubit operation given
  a single-qubit operation and the control wires.
  [(#3681)](https://github.com/PennyLaneAI/pennylane/pull/3681)

* `qp.pauli.is_pauli_word` now supports `Prod` and `SProd` operators, and it returns `False` when a
  `Hamiltonian` contains more than one term.
  [(#3692)](https://github.com/PennyLaneAI/pennylane/pull/3692)

* `qp.pauli.pauli_word_to_string` now supports `Prod`, `SProd` and `Hamiltonian` operators.
  [(#3692)](https://github.com/PennyLaneAI/pennylane/pull/3692)

* `qp.ops.op_math.Controlled` can now decompose single qubit target operations more effectively using the ZYZ
  decomposition.
  [(#3726)](https://github.com/PennyLaneAI/pennylane/pull/3726)

 * The `qp.qchem.Molecule` class raises an error when the molecule has an odd number of electrons or 
   when the spin multiplicity is not 1.
   [(#3748)](https://github.com/PennyLaneAI/pennylane/pull/3748)

* `qp.qchem.basis_rotation` now accounts for spin, allowing it to perform Basis Rotation Groupings
...
* `qp.dot` now groups coefficients together.
  [(#3691)](https://github.com/PennyLaneAI/pennylane/pull/3691)

  ```pycon
  >>> qp.dot(coeffs=[2, 2, 2], ops=[qp.PauliX(0), qp.PauliY(1), qp.PauliZ(2)])
  2*(PauliX(wires=[0]) + PauliY(wires=[1]) + PauliZ(wires=[2]))
  ```

* `qp.generator` now supports operators with `Sum` and `Prod` generators.
  [(#3691)](https://github.com/PennyLaneAI/pennylane/pull/3691)

* The `Sum._sort` method now takes into account the name of the operator when sorting.
  [(#3691)](https://github.com/PennyLaneAI/pennylane/pull/3691)

* A new tape transform called `qp.transforms.sign_expand` has been added. It implements the optimal decomposition of a fast-forwardable Hamiltonian that minimizes the variance of its estimator in the Single-Qubit-Measurement from [arXiv:2207.09479](https://arxiv.org/abs/2207.09479).
  [(#2852)](https://github.com/PennyLaneAI/pennylane/pull/2852)

<h4>Differentiability and interfaces</h4>

* The `qp.math` module now also contains a submodule for
  fast Fourier transforms, `qp.math.fft`.
...
* Validation has been added on gradient keyword arguments when initializing a QNode — if unexpected keyword arguments are passed,
  ...
  `qp.gradients.SUPPORTED_GRADIENT_KWARGS`.
...
* `qp.math.size` now supports PyTorch tensors.
...
* `qp.math.matmul` now supports PyTorch and Autograd tensors.
  [(#3613)](https://github.com/PennyLaneAI/pennylane/pull/3613)

* Add `qp.math.detach`, which detaches a tensor from its trace. This stops
...
* `qp.QuantumMonteCarlo` template is now JAX-JIT compatible when passing `jax.numpy` arrays to the template.
  [(#3734)](https://github.com/PennyLaneAI/pennylane/pull/3734)

* `DefaultQubitJax` now supports evolving the state vector when executing `qp.pulse.ParametrizedEvolution`
  gates.
...
* `qp.QubitStateVector` now implements the `StatePrep` interface.
  [(#3685)](https://github.com/PennyLaneAI/pennylane/pull/3685)

* `qp.BasisState` now implements the `StatePrep` interface.
...
* Writing Hamiltonians to a file using the `qp.data` module has been improved by employing a condensed writing format.
  [(#3592)](https://github.com/PennyLaneAI/pennylane/pull/3592)

* Lazy-loading in the `qp.data.Dataset.read()` method is more universally supported.
...
* `qp.draw` and `qp.draw_mpl` have been updated to draw any quantum function,
...
* `Operation.inv()` and the `Operation.inverse` setter have been removed. Please use `qp.adjoint` or `qp.pow` instead.
...
* The target wires of `qp.ControlledQubitUnitary` are no longer available via `op.hyperparameters["u_wires"]`.
...
* `qp.qchem.scf`, `qp.RandomLayers.compute_decomposition`, and `qp.Wires.select_random` now use
...
* `qp.transforms.measurement_grouping` has been removed. Users should use `qp.transforms.hamiltonian_expand`
  instead.
...
* `qp.utils.sparse_hamiltonian` function has been deprecated, and usage will now raise a warning.
  Instead, one should use the `qp.Hamiltonian.sparse_matrix` method.
  [(#3585)](https://github.com/PennyLaneAI/pennylane/pull/3585)
...
* `qp.op_sum` has been deprecated. Users should use `qp.sum` instead.
  [(#3686)](https://github.com/PennyLaneAI/pennylane/pull/3686)

* The use of `Evolution` directly has been deprecated. Users should use `qp.evolve` instead.
  This new function changes the sign of the given parameter.
  [(#3706)](https://github.com/PennyLaneAI/pennylane/pull/3706)

* Use of `qp.dot` with a `QNodeCollection` has been deprecated.
  [(#3586)](https://github.com/PennyLaneAI/pennylane/pull/3586)
...
* The documentation for `qp.operation` has been improved.
  [(#3664)](https://github.com/PennyLaneAI/pennylane/pull/3664)

* The code example in `qp.SparseHamiltonian` has been updated with the correct wire range.
  [(#3643)](https://github.com/PennyLaneAI/pennylane/pull/3643)

* A hyperlink has been added in the text for a URL in the `qp.qchem.mol_data` docstring.
  [(#3644)](https://github.com/PennyLaneAI/pennylane/pull/3644)

* A typo was corrected in the documentation for `qp.math.vn_entropy`.
[(#3740)](https://github.com/PennyLaneAI/pennylane/pull/3740)

<h3>Bug fixes 🐛</h3>

* Fixed a bug where measuring ``qp.probs`` in the computational basis with non-commuting
...
* Fixed a bug in `qp.transforms.metric_tensor` where prefactors of operation generators were taken
...
* `qp.pauli.PauliWord` is now pickle-able.
...
* `qp.data.Dataset.write()` now ensures that any lazy-loaded values are loaded before they are written to a file.
  [(#3605)](https://github.com/PennyLaneAI/pennylane/pull/3605)
...
* Fixed typo in the example of `qp.IsingZZ` gate decomposition.
  [(#3676)](https://github.com/PennyLaneAI/pennylane/pull/3676)

* Fixed a bug that made tapes/qnodes using `qp.Snapshot` incompatible with `qp.drawer.tape_mpl`.
...
* `qp.math.ndim` has been redirected to `jnp.ndim` when using it on a `jax` tensor.
  [(#3730)](https://github.com/PennyLaneAI/pennylane/pull/3730)

* Implementations of `marginal_prob` (and subsequently, `qp.probs`) now return
...
* The keyword arguments for `qp.equal` now are used when comparing the observables of a 
...
* Only converts input to `qp.Hermitian` to a numpy array if the input is a list.
  [(#3820)](https://github.com/PennyLaneAI/pennylane/pull/3820)
