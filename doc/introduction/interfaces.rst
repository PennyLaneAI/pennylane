...
.. note::
    ...
    See ``qpmath.SUPPORTED_INTERFACE_NAMES`` for a list of all accepted interface strings.
...
PennyLane offers a built-in Jax-based implementation compatible with ``jax.jit`` and ``qpqjit``.
...
.. code-block:: python
    ...
    @qpqnode(dev, diff_method="parameter-shift")
    def circuit(x):
        qpRX(x, wires=0)
...
* ``"reversed-hadamard"``: Use a variant of ``"hadamard"``, where the role of the
  ...
  can be found in the documentation for :func:`qpgradients.hadamard_grad <pennylane.gradients.hadamard_grad>`
...
* ``"direct-hadamard"``: Use a variant of ``"hadamard"``, where the additional auxiliary
  ...
  More information can be found in the documentation for :func:`qpgradients.hadamard_grad <pennylane.gradients.hadamard_grad>`
  and the `original paper outlining this gradient method <https://arxiv.org/pdf/2408.05406>`__.
...
* ``"reversed-direct-hadamard"``: Use a variant of ``"direct-hadamard"``, where 
  ...
  information can be found in the documentation for :func:`qpgradients.hadamard_grad <pennylane.gradients.hadamard_grad>`
  and the
  `original paper outlining this gradient method <https://arxiv.org/pdf/2408.05406>`__.

* :func:`qpgradients.stoch_pulse_grad <pennylane.gradients.stoch_pulse_grad>`: Use a stochastic variant of the
  parameter-shift rule for pulse programs.

* :func:`qpgradients.pulse_odegen <pennylane.gradients.pulse_odegen>`: Combine classical processing with the parameter-shift rule for multivariate gates to differentiate pulse programs.
...
:mod:`qpgradients <pennylane.gradients>` module.
...
``qpgrad()`` if using Autograd, or ``loss.backward()`` for PyTorch).
...
gradient transform, please see the :mod:`qpgradients <pennylane.gradients>` documentation.
...
.. code-block:: python
    ...
    def circuit(weights):
        ...
        qpCNOT(wires=[0, 1])
        qpRX(weights[2], wires=1)
        return qpexpval(qpPauliZ(1))

>>> weights = np.array([0.1, 0.2, 0.3], requires_grad=True)
>>> circuit(weights)
tensor(0.9316158, requires_grad=True)
>>> qpgradients.param_shift(circuit)(weights)  # gradient
(tensor(-0.09347337, requires_grad=True),
 tensor(-0.18884787, requires_grad=True),
 tensor(-0.28818254, requires_grad=True))
>>> def f(weights):
...     return np.stack(qpgradients.param_shift(circuit)(weights))
>>> qpjacobian(f)(weights)  # hessian
...
.. code-block:: python
    ...
    @qpqnode(dev, diff_method="parameter-shift", max_diff=2)
    def circuit(weights):
        qpRX(weights[0], wires=0)
        qpRY(weights[1], wires=1)
        qpCNOT(wires=[0, 1])
...
>>> weights = np.array([0.1, 0.2, 0.3], requires_grad=True)
>>> qpjacobian(qpjacobian(circuit))(weights)  # hessian
array([[-0.9316158 ,  0.01894799,  0.0289147 ],
       [ 0.01894799, -0.9316158 ,  0.05841749],
       [ 0.0289147 ,  0.05841749, -0.9316158 ]])
...
``diff_method="parameter-shift"`` is equivalent to passing ``diff_method=qpgradients.param_shift``.
...
* The return value of the QNode, e.g. ``qpexpval()`` or ``qpprobs()``
