...
.. note::
    ...
    * `lightning.kokkos <https://docs.pennylane.ai/projects/lightning/en/stable/lightning_kokkos/device.html>`_,
    ...
    Additionally, the tree-traversal MCM method is supported with Catalyst with the following limitations:
    
    * Postselecting and resetting qubits is not supported when performing an MCM with :func:`pennylane.measure <qml.measure>` (i.e., the `postselect` and `reset` arguments).
    * Statevector-based terminal measurements are not supported (e.g., :func:`pennylane.probs` and :func:`pennylane.state`).
