...
def excitations(n_electrons, n_spin_orbitals, delta_sz=0):
    r"""Generates single and double excitations from a Hartree-Fock reference state.

    Single and double excitations can be generated by acting with the excitation operators
    :math:`\hat T_1` and :math:`\hat T_2` on the Hartree-Fock (HF) reference state:

    .. math::

        && \hat{T}_1 \vert \mathrm{HF} \rangle = \sum_{r \in \mathrm{occ} \\ p \in \mathrm{virt}}
        \hat{c}_p^\dagger \hat{c}_r \vert \mathrm{HF} \rangle \\
        && \hat{T}_2 \vert \mathrm{HF} \rangle = \sum_{r>s \in \mathrm{occ} \\ p>q \in
        \mathrm{virt}} \hat{c}_p^\dagger \hat{c}_q^\dagger \hat{c}_r \hat{c}_s
        \vert \mathrm{HF} \rangle,


    where the indices :math:`r, s` and :math:`p, q` run over the occupied (occ) and unoccupied,
    referred to as virtual (virt), *spin-orbitals* and :math:`\hat c` and
    ...
    **Example**
    ...
    if n_spin_orbitals <= n_electrons:
        raise ValueError(
    ...
    singles = [
        ...
        for p in range(n_electrons, n_spin_orbitals)
    ...
    doubles = [
        ...
        for q in range(n_electrons, n_spin_orbitals - 1)
