...
def excitations(n_electrons, n_spin_orbitals, delta_sz=0):
    ...
    :math:`\vert \mathrm{HF} \rangle`.

    .. math::

        && \hat{T}_1 = \sum_{r \in \mathrm{occ} \\ p \in \mathrm{virt}}
    ...
    occupied (occ) and unoccupied, referred to as virtual (virt), *spin-orbitals*
    and :math:`\hat c` and :math:`\hat c^\dagger` are the electron annihilation and
