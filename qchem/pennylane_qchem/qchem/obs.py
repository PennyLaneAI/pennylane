...
def spin2(electrons, orbitals, mapping="jordan_wigner", wires=None):
    ...
    **Example**
    ...
    s2_op = openfermion.FermionOperator("") * 3 / 4 * electrons
    for i in table:
        s2_op += openfermion.FermionOperator(
...
def observable(fermion_ops, init_term=0, mapping="jordan_wigner", wires=None):
    ...
    **Example**
    ...
    mb_obs = openfermion.FermionOperator("") * init_term
    for ops in fermion_ops:
        if not isinstance(ops, openfermion.FermionOperator):
            raise TypeError(
    ...
    if mapping.strip().lower() == "bravyi_kitaev":
        return structure.convert_observable(openfermion.bravyi_kitaev(mb_obs), wires=wires)

    return structure.convert_observable(openfermion.jordan_wigner(mb_obs), wires=wires)
...
def spin_z(orbitals, mapping="jordan_wigner", wires=None):
    ...
    **Example**
    ...
    for i in table:
        sz_op += openfermion.FermionOperator(((int(i[0]), 1), (int(i[1]), 0)), i[2])
...
def particle_number(orbitals, mapping="jordan_wigner", wires=None):
    ...
    **Example**
    ...
    N_obs = openfermion.FermionOperator()
    for i in table:
        N_obs += openfermion.FermionOperator(((int(i[0]), 1), (int(i[1]), 0)), i[2])
...
def one_particle(matrix_elements, core=None, active=None, cutoff=1.0e-12):
    ...
    **Example**
    ...
    t_op = openfermion.FermionOperator("") * t_core
    for pair in pairs:
        alpha, beta = pair
        ...
        t_op += openfermion.FermionOperator(((a, 1), (b, 0)), element)

        # spin-down term
        t_op += openfermion.FermionOperator(((a + 1, 1), (b + 1, 0)), element)
...
def two_particle(matrix_elements, core=None, active=None, cutoff=1.0e-12):
    ...
    **Example**
    ...
    v_op = openfermion.FermionOperator("") * v_core
    ...
    if core:
        for alpha in active:
            for beta in active:
                ...
                a = 2 * active.index(alpha)
                b = 2 * active.index(beta)
                v_op += openfermion.FermionOperator(((a, 1), (b, 0)), element)

                # down-down term
                v_op += openfermion.FermionOperator(((a + 1, 1), (b + 1, 0)), element)
    ...
    for quad in quads:
        ...
        v_op += openfermion.FermionOperator(((a, 1), (b, 1), (g, 0), (d, 0)), 0.5 * element)
