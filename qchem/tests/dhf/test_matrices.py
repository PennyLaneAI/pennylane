...
def test_molecular_density_matrix(n_electron, c, p_ref):
    ...
    p = qchem.dhf.mol_density_matrix(n_electron, c)
...
def test_overlap_matrix(symbols, geometry, alpha, s_ref):
    ...
    args = [alpha]
    s = qchem.dhf.overlap_matrix(mol.basis_set)(*args)
...
def test_overlap_matrix_nodiff(symbols, geometry, s_ref):
    ...
    s = qchem.dhf.overlap_matrix(mol.basis_set)()
    assert np.allclose(s, s_ref)
...
def test_gradient_overlap_matrix(symbols, geometry, alpha, coeff, g_alpha_ref, g_coeff_ref):
    ...
    g_alpha = autograd.jacobian(qchem.dhf.overlap_matrix(mol.basis_set), argnum=0)(*args)
    g_coeff = autograd.jacobian(qchem.dhf.overlap_matrix(mol.basis_set), argnum=1)(*args)
...
def test_moment_matrix(symbols, geometry, alpha, e, idx, s_ref):
    ...
    mol = qchem.dhf.Molecule(symbols, geometry, alpha=alpha)
    args = [alpha]
    s = qchem.dhf.moment_matrix(mol.basis_set, e, idx)(*args)
    assert np.allclose(s, s_ref)
...
def test_moment_matrix_nodiff(symbols, geometry, e, idx, s_ref):
    ...
    used."""
    mol = qchem.dhf.Molecule(symbols, geometry)
    s = qchem.dhf.moment_matrix(mol.basis_set, e, idx)()
    assert np.allclose(s, s_ref)
...
def test_gradient_moment_matrix(symbols, geometry, alpha, coeff, e, idx, g_alpha_ref, g_coeff_ref):
    ...
    mol = qchem.dhf.Molecule(symbols, geometry, alpha=alpha, coeff=coeff)
    args = [mol.alpha, mol.coeff]
    g_alpha = autograd.jacobian(qchem.dhf.moment_matrix(mol.basis_set, e, idx), argnum=0)(*args)
    g_coeff = autograd.jacobian(qchem.dhf.moment_matrix(mol.basis_set, e, idx), argnum=1)(*args)
...
def test_kinetic_matrix(symbols, geometry, alpha, t_ref):
    ...
    mol = qchem.dhf.Molecule(symbols, geometry, alpha=alpha)
    args = [alpha]
    t = qchem.dhf.kinetic_matrix(mol.basis_set)(*args)
...
def test_kinetic_matrix_nodiff(symbols, geometry, t_ref):
    ...
    used."""
    mol = qchem.dhf.Molecule(symbols, geometry)
    t = qchem.dhf.kinetic_matrix(mol.basis_set)()
    assert np.allclose(t, t_ref)
...
def test_gradient_kinetic_matrix(symbols, geometry, alpha, coeff, g_alpha_ref, g_coeff_ref):
    ...
    mol = qchem.dhf.Molecule(symbols, geometry, alpha=alpha, coeff=coeff)
    args = [mol.alpha, mol.coeff]
    g_alpha = autograd.jacobian(qchem.dhf.kinetic_matrix(mol.basis_set), argnum=0)(*args)
    g_coeff = autograd.jacobian(qchem.dhf.kinetic_matrix(mol.basis_set), argnum=1)(*args)
    assert np.allclose(g_alpha, g_alpha_ref)
    assert np.allclose(g_coeff, g_coeff_ref)
...
def test_attraction_matrix(symbols, geometry, alpha, v_ref):
    ...
    mol = qchem.dhf.Molecule(symbols, geometry, alpha=alpha)
    args = [mol.alpha]
    v = qchem.dhf.attraction_matrix(mol.basis_set, mol.nuclear_charges, mol.coordinates)(*args)
...
def test_attraction_matrix_nodiff(symbols, geometry, v_ref):
    r"""Test that attraction_matrix returns the correct matrix."""
    mol = qchem.dhf.Molecule(symbols, geometry)
    v = qchem.dhf.attraction_matrix(mol.basis_set, mol.nuclear_charges, mol.coordinates)()
    assert np.allclose(v, v_ref)
...
def test_gradient_attraction_matrix(symbols, geometry, alpha, coeff, g_r_ref):
    ...
    g_r = autograd.jacobian(
        qchem.dhf.attraction_matrix(mol.basis_set, mol.nuclear_charges, mol.coordinates), argnum=0
    )(*args)
...
def test_repulsion_tensor(symbols, geometry, alpha, e_ref):
    ...
    mol = qchem.dhf.Molecule(symbols, geometry, alpha=alpha)
    args = [mol.alpha]
    e = qchem.dhf.repulsion_tensor(mol.basis_set)(*args)
...
def test_repulsion_tensor_nodiff(symbols, geometry, e_ref):
    ...
    used."""
    mol = qchem.dhf.Molecule(symbols, geometry)
    e = qchem.dhf.repulsion_tensor(mol.basis_set)()
    assert np.allclose(e, e_ref)
...
def test_core_matrix(symbols, geometry, alpha, c_ref):
    ...
    mol = qchem.dhf.Molecule(symbols, geometry, alpha=alpha)
    args = [mol.alpha]
    c = qchem.dhf.core_matrix(mol.basis_set, mol.nuclear_charges, mol.coordinates)(*args)
...
def test_core_matrix_nodiff(symbols, geometry, c_ref):
    ...
    used."""
    mol = qchem.dhf.Molecule(symbols, geometry)
    c = qchem.dhf.core_matrix(mol.basis_set, mol.nuclear_charges, mol.coordinates)()
    assert np.allclose(c, c_ref)
