...
def test_electron_integrals(symbols, geometry, core, active, e_core, one_ref, two_ref):
    ...
    e, one, two = qchem.dhf.electron_integrals(mol, core=core, active=active)(*args)
...
def test_fermionic_hamiltonian(symbols, geometry, alpha, coeffs_h_ref, ops_h_ref):
    ...
    mol = qchem.dhf.Molecule(symbols, geometry, alpha=alpha)
    args = [alpha]
    h = qchem.dhf.fermionic_hamiltonian(mol)(*args)
...
def test_mol_hamiltonian(symbols, geometry, h_ref_data):
    ...
    mol = qchem.dhf.Molecule(symbols, geometry)
    args = []
    h = qchem.dhf.mol_hamiltonian(mol)(*args)
...
def test_gradient_expvalH():
    ...
    def energy(mol):
        ...
        def circuit(*args):
            ...
            h_qubit = qchem.dhf.mol_hamiltonian(mol)(*args)
