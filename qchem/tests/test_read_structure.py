...
@pytest.mark.parametrize("names", [['h2.SDF', 'h2_ref.xyz'],
                                   ['gdb3.mol5.PDB', 'gdb3.mol5_ref.xyz'],
                                   ['gdb3.mol5.XYZ', 'gdb3.mol5_ref.xyz']])
def test_conversion_from_folder(names, tmpdir):
...
def test_reading_xyz_file(tmpdir):
    ...
    ref_geometry = [['C', (0.361, -0.452, -0.551)],
                    ['C', (-0.714, 0.125, 0.327)],
                    ['N', (0.683, 0.133, 0.745)],
                    ['H', (0.442, -1.529, -0.619)],
                    ['H', (0.672, 0.102, -1.428)],
...
def test_subprocess_run(monkeypatch):
    ...
    with monkeypatch.context() as m:
        ...
        with pytest.raises(RuntimeError,
                           match="Babel error. See the following Babel output for details"):
