...
def gate_fabric_uniform(n_layers, n_wires, low=0, high=2 * pi, seed=None):
    ...
    distribution.
    Each parameter is drawn uniformly at random from the half-open interval [``low``, ``high``).
    The parameters define the trainable angles entering the quantum number preserving gate :math:`\hat{Q}(\theta,\phi)`
    implemented by the :class:`~.DoubleExcitation` and :class:`~.OrbitalRotation`.
    ...
    if n_wires < 4:
        raise ValueError(
            f"The number of qubits must be greater than four; got 'n_wires' = {n_wires}"
        )

    if n_wires % 2:
        raise ValueError(f"The number of qubits must be even; got 'n_wires' = {n_wires}")
