...
class QuantumMultiplier(Operation):
    ...
    def __init__(self, multiplicand_wires, multiplier_wires, accumulator_wires, carry_wires, work_wire, do_queue=True):
        ...
        wires = self.multiplicand_wires + self.multiplier_wires + self.accumulator_wires + self.carry_wires + [self.work_wire]
    ...
    def expand(self):
        with qml.tape.QuantumTape() as tape:
            ...
            for ctrl in self.multiplier_wires[::-1]: #we go backwards because the last wire is the least significant
                ...
                qml.templates.ControlledQuantumAdder(a_wires=self.multiplicand_wires, b_wires=self.accumulator_wires,carry_wires=self.carry_wires[0:len(self.accumulator_wires)+1],work_wire=self.work_wire, control_wire=ctrl)
