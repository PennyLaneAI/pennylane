...
class TrotterCDF(Operator2):
    ...
    :math:`(p, q)` (hence the :math:`\tfrac{1}{2}`).

    The input encodes a *regrouped* form of this Hamiltonian: after substituting
    :math:`n = (I - Z)/2`, the single-site :math:`Z` terms generated by expanding the two-body
    :math:`\tilde{n}_p \tilde{n}_q` are folded into the one-body tensor, so each two-body fragment
    retains only its two-site :math:`Z_p Z_q` couplings. The input tensors map to this form as:
    ...
    * ``nuc_constant``: the scalar constant :math:`C`. Together with the identity terms produced by
    ...
    Args:
        ...
        hamiltonian (dict): A CDF Hamiltonian as a dictionary with keys ``nuc_constant``,
            ...
            ``core_tensors: (L+1, N, N)`` and ``leaf_tensors: (L+1, N, N)``, where ``N`` is the
