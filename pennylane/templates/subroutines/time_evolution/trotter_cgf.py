...
class TrotterCGF(Operator2):
    ...
    fragment, and all constants -- including the two-body identity balance
    :math:`-\tfrac{1}{4}\sum \lambda` (see the Implementation Details) -- into :math:`C`, so each
    two-body fragment retains only its two-site :math:`Z_{lp} Z_{mq}` couplings. The input tensors
    therefore map to this regrouped form as:
    ...
    * ``nuc_constant``: the constant :math:`C`, applied together with the one-body identity terms as a
      single :class:`~.GlobalPhase` (the energy shift; :math:`C` must already carry the two-body
      identity balance, see the Implementation Details).
