...
class TrotterCGF(Operator2):
    ...
    :math:`\tilde{n}^{(\nu)}_{lp} = \mathcal{U}^{(\nu,l)} n_{lp} \mathcal{U}^{(\nu,l)\dagger}` is
    its rotation into the basis of fragment :math:`\nu`, two-body fragments couple distinct modes
    (:math:`l > m`), and :math:`C` is a scalar constant.
    ...
    * ``leaf_tensors[nu][l]``: the per-mode :math:`N \times N` rotation :math:`\mathcal{U}^{(\nu,l)}
      =` :class:`~.BasisRotation` ``(leaf_tensors[nu][l])`` on mode :math:`l`'s modal register;
    * ``core_tensors[0]``: the (regrouped) one-body tensor, whose diagonal sets the *single-site*
      :class:`~.RZ` angles of the one-body layer;
    * ``core_tensors[nu]`` (:math:`\nu \geq 1`): the two-body tensors, whose entries set the
      *two-site* :class:`~.IsingZZ` angles of the two-body layers;
