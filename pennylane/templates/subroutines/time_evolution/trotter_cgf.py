...
class TrotterCGF(Operator2):
    ...
    <https://arxiv.org/abs/2508.11865>`__). :math:`H` consists of a one-body fragment and
    :math:`L` two-body fragments, each diagonal in its own per-mode basis:
    ...
    :math:`\tilde{n}^{(\nu)}_{lp} = \mathcal{U}^{(\nu,l)} n_{lp} \mathcal{U}^{(\nu,l)\dagger}` is its
    rotation into the diagonal basis of fragment :math:`\nu` by the real orthogonal per-mode rotation
    :math:`\mathcal{U}^{(\nu,l)}`, two-body fragments couple distinct modes (:math:`l > m`), and
    :math:`C` is a scalar constant.

    The input encodes the *regrouped* single-boson form of this Hamiltonian: after substituting
    :math:`n_{lp} = (I - Z_{lp})/2`, all single-site :math:`Z` terms (including those generated by
    expanding the two-body :math:`\tilde{n}_{lp} \tilde{n}_{mq}`) are collected into the one-body
    fragment and all constants into :math:`C`, so each two-body fragment retains only its two-site
    :math:`Z_{lp} Z_{mq}` couplings. The input tensors therefore map to this regrouped form as:

    * ``leaf_tensors[nu][l]``: the per-mode :math:`N \times N` real orthogonal rotation that
      diagonalizes fragment :math:`\nu` on mode :math:`l`'s modal register. The one-body and two-body
      leaves follow *opposite* modal conventions that differ by a transpose: the one-body leaf
      (:math:`\nu = 0`) stores its eigenvectors on the *columns*, so :math:`\mathcal{U}^{(0,l)} =`
      :class:`~.BasisRotation` ``(leaf_tensors[0][l])``, while each two-body leaf (:math:`\nu \geq 1`)
      stores the modal index on the *rows*, so :math:`\mathcal{U}^{(\nu,l)} =`
      :class:`~.BasisRotation` ``(leaf_tensors[nu][l].T)`` (see the Implementation Details);
    * ``core_tensors[0]``: the (regrouped) one-body tensor, whose diagonal
      ``core_tensors[0][l, l, p, p]`` sets the *single-site* :class:`~.RZ` angles of the one-body
      layer;
    ...
    * ``nuc_constant``: the constant :math:`C`, applied together with the identity terms from the
    ...
    The leaves are assumed real orthogonal, and only ``core_tensors[0][l, l, p, p]`` and the
    :math:`l > m` triangle of ``core_tensors[nu]`` (:math:`\nu \geq 1`) are read.

    Note that "one-body"/"two-body" count number operators (:math:`n_{lp}`, :math:`n_{lp} n_{mq}`),
    not Pauli weight; the regrouping is what lets each two-body layer use only two-site
    :class:`~.IsingZZ` gates.
    ...
    Args:
        ...
        hamiltonian (dict): A CGF Hamiltonian as a dictionary with keys ``nuc_constant``,
            ...
            and ``L`` is the number of two-body fragments. Only the one-body diagonal
            ``core_tensors[0][l, l, p, p]`` and the strict lower mode triangle
            ``core_tensors[nu][l, m]`` (:math:`l > m`, :math:`\nu \geq 1`) are read; all
    ...
    **Example**
    ...
    Let us create mock CGF Hamiltonian data with the correct tensor shapes and real
    orthogonal leaves.
    ...
    {'CNOT': 840, 'Hadamard': 1, 'PhaseShift': 43, 'RZ': 480, 'SingleExcitation': 186}
    ...
    .. details ::
        ...
        **2. Evolving a fragment**
        ...
        leaves are assumed real orthogonal (they are merged by transpose, not adjoint).
        ...
        **5. Constant terms (global phase / energy shift)**
        ...
        of :math:`e^{-iHt}`. Each two-body layer is a pure two-site :math:`Z_{lp} Z_{mq}` rotation and
        contributes no identity part, so the only identity contributions are the one-body
        :math:`\tfrac{1}{2}\sum_{l,p}\epsilon_{lp}` (from :math:`n = (I - Z)/2` in step 3) and the
        scalar :math:`C =` ``nuc_constant``,
        ...
        ``trace(trace(core_tensors[0]))/2`` for the one-body part). Any residual constant from the
        upstream regrouping (e.g. the two-body scalar) is expected to be supplied through
        ``nuc_constant``.
    """
...
def _trotter_cgf_decomposition(evolution_time, num_trotter_steps, hamiltonian, wires, double_phase):
    ...
    if num_trotter_steps > 0:
        _run_trotter_steps(
            evolution_time,
            num_trotter_steps,
            _align_one_body_leaf(hamiltonian),
            wires,
            (),
            **_CGF_HELPERS,
        )
...
def _controlled_trotter_cgf_decomp(base, control_wires, control_values, work_wires, work_wire_type):
    ...
    phi = (_energy_shift(hamiltonian) * evolution_time) % (4 * np.pi)
    hamiltonian = _align_one_body_leaf(hamiltonian)
