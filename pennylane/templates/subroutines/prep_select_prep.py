...
class PrepSelectPrep(Operation):
    ...
    @classmethod
    def _unflatten(cls, data, metadata) -> "PrepSelectPrep":
        return cls(data, metadata)

    def __repr__(self):
        return f"PrepSelectPrep(coeffs={self.hyperparameters['coeffs']}, ops={self.hyperparameters['ops']}, control={self.hyperparameters['control']})"
    ...
    def map_wires(self, wire_map: dict) -> "Select":
        ...
        return PrepSelectPrep(new_lcu, new_control)

    def decomposition(self):
        return self.compute_decomposition(self.coeffs, self.ops, self.control)
    ...
    ### THIS IS COPIED DIRECTLY FROM SELECT
    def __copy__(self):
        """Copy this op"""
        cls = self.__class__
