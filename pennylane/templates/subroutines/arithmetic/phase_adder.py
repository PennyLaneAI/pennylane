...
from pennylane.templates.subroutine import Subroutine
from pennylane.templates.subroutines.qft import QFT
from pennylane.wires import WiresLike


def setup_inputs(k, x_wires, mod=None, work_wire=()):
    if mod is None:
        mod = 2 ** len(x_wires)
    return (k, x_wires, mod, work_wire), {}


def _valiation(k, x_wires, mod, work_wires):
    num_work_wires = len(work_wires)
    if math.is_abstract(mod) or math.is_abstract(k):
        return
    if mod != 2 ** len(x_wires) and num_work_wires != 1:
...
@partial(Subroutine, wire_argnames={"x_wires", "work_wire"}, setup_inputs=setup_inputs)
def PhaseAdder(k, x_wires: WiresLike, mod, work_wire=()):
    ...
    **Example**
    ...
    _valiation(k, x_wires, mod, work_wire)
    k = k % mod
    n_wires = len(x_wires)
