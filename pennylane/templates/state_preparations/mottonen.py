    ...
    def compute_decomposition(state_vector, wires):  # pylint: disable=arguments-differ
        ...
        **Example**
        ...
        for k in range(len(wires_reverse), 0, -1):
            ...
            theta = _compute_theta(alpha_y_k)
            gray_code_rank = len(control)

            if gray_code_rank == 0:
                if qml.math.all(theta[..., 0] != 0.0):
                    op_list.append(qml.RY(theta[..., 0], wires=[target]))
                return op_list
            ...
            for i, control_index in enumerate(control_indices):
                if qml.math.all(theta[..., i] != 0.0):
                    op_list.append(qml.RY(theta[..., i], wires=[target]))
                op_list.append(qml.CNOT(wires=[control[control_index], target]))
        ...
        if not qml.math.allclose(omega, 0):
            for k in range(len(wires_reverse), 0, -1):
                ...
                target = wires_reverse[k - 1]
                if len(alpha_z_k) > 0:
                    theta = _compute_theta(alpha_z_k)

                    gray_code_rank = len(control)

                    if gray_code_rank == 0:
                        if qml.math.all(theta[..., 0] != 0.0):
                            op_list.append(qml.RZ(theta[..., 0], wires=[target]))
                        return op_list
                    ...
                    for i, control_index in enumerate(control_indices):
                        if qml.math.all(theta[..., i] != 0.0):
                            op_list.append(qml.RZ(theta[..., i], wires=[target]))
                        op_list.append(qml.CNOT(wires=[control[control_index], target]))
