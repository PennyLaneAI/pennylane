    ...
    def greedy_layers(self):
        ...
        for wire in operations:
            operations[wire] = list(
                filter(
                    lambda op: not (
                        isinstance(op, (qml.operation.Observable, qml.tape.MeasurementProcess)) and op.return_type is not None
                    ),
                    operations[wire],
                )
            )
        ...
        for wire in sorted(self._grid):
            observables[wire] = list(
                filter(
                    lambda op: isinstance(op, (qml.operation.Observable, qml.tape.MeasurementProcess))
