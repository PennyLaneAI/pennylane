...
def variational_kak_adj(H, g, dims, adj, verbose=False, opt_kwargs=None, pick_min=False):
    ...
    **Example**
    ...
    if not has_jax:  # pragma: no cover
        raise ImportError(
            "jax and optax are required for variational_kak_adj. You can install them with pip install jax~=0.7.0 jaxlib~=0.6.0 optax."
        )  # pragma: no cover
...
def run_opt(
    ...
    **Example**
    ...
    if not has_jax:  # pragma: no cover
        raise ImportError(
            "jax and optax are required for run_opt. You can install them with pip install jax~=0.7.0 jaxlib~=0.6.0 optax."
        )  # pragma: no cover
