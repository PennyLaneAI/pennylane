...
class CompactState:
    ...
    def __init__(
        ...
        self.num_basis_bits = num_basis_bits
        self.num_basis_states = num_basis_states

    @classmethod
    def from_statevect(cls, num_qubits, precision=1e-3):
        return CompactState(num_wires=num_qubits, precision=precision)
    
    @classmethod
    def from_basis_states(cls, num_basis_states, num_basis_bits, precision=1e-3):
        num_wires = math.ceil(math.log2(num_basis_states))
        return CompactState(num_wires=num_wires, num_basis_bits=num_basis_bits, num_basis_states=num_basis_states, precision=precision)
    
    @classmethod
    def from_mps(cls, input_dim, precision=1e-3):
        return CompactState(num_wires=math.ceil(math.log2(input_dim)), precision=precision)
