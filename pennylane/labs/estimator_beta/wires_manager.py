...
class Deallocate(_WireAction):
    def __init__(self, num_wires = None, allocated_register=None, state=AllocateState.ZERO, restored=False):
        ...
        if state == AllocateState.ANY:
            if restored == True:
                if allocated_register is None:
                    raise ValueError("Must provide `allocated_register` when deallocating an ANY state register with `restored=True`")
            else:
                raise ValueError('When deallocating an ANY state, `restored=False` is not supported.')
...
def _estimate_auxiliary_wires(
    ...
    for action in list_actions:
        if isinstance(action, GateCount):
            ...
            sub_max_alloc, sub_max_dealloc, sub_total = _estimate_auxiliary_wires(
                resource_decomp, action.count, gate_set, config, num_available_any_state_aux,
            )
            ...
            if action.state == AllocateState.ANY and action.restored == True:
                try:
                    ...
                    raise ValueError(f"Trying to deallocate an ANY state register before it was allocated {action}") from e
    ...
    if len(any_state_aux_allocation) != 0:
        raise ValueError(f"Did NOT deallocate and restore all ANY state allocations as promised:\n{any_state_aux_allocation}")
