...
fermionic_fragments = [fermionic_observable(one = one_body, constant = [0.]).to_mat(format='dense')]
for i in range(two_body_fragments.shape[0]):
    obc = qml.math.einsum('pk,kk,qk->pq', U[i], Z[i], U[i])
    fermionic_fragments.append(fermionic_observable(one = -obc, two = two_body_fragments[i], constant = [0.]).to_mat(format='dense'))
...
def test_perturbation_theory(backend, num_workers, parallel_mode, n_states):
    ...
    actual = perturbation_error(pf, fragments, states, order=order, backend=backend, num_workers=num_workers, parallel_mode=parallel_mode)
