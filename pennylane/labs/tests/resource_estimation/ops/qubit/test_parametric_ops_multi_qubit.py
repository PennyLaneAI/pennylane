...
class TestMultiRZ:
    ...
    def test_resources(self, num_wires, eps):
        ...
        assert re.ResourceMultiRZ.default_resource_decomp(num_wires, eps) == expected
    ...
    def test_pow_decomp(self, z, num_wires, eps):
        ...
        assert op.default_pow_resource_decomp(z, **op.resource_params) == expected_res
...
class TestPauliRot:
    ...
    def test_resources(self, pauli_string, expected_h_count, expected_s_count, eps):
        ...
        assert re.ResourcePauliRot.default_resource_decomp(pauli_string, eps=eps) == expected
    ...
    def test_resources_empty_pauli_string(self):
        ...
        expected = [re.GateCount(re.ResourceGlobalPhase.resource_rep())]
        assert re.ResourcePauliRot.default_resource_decomp(pauli_string="") == expected
    ...
    def test_adjoint_decomp(self, pauli_word, eps):
        ...
        assert (
            re.ResourcePauliRot.default_adjoint_resource_decomp(pauli_string=pauli_word, eps=eps)
            == expected
        )
...
class TestIsingXX:
    ...
    def test_resources(self, eps):
        ...
        assert re.ResourceIsingXX.default_resource_decomp(eps=eps) == expected
    ...
    def test_resources_pow(self, eps):
        ...
        assert re.ResourceIsingXX.default_pow_resource_decomp(pow_z=3, eps=eps) == expected

    @pytest.mark.parametrize("eps", (None, 1e-3))
    def test_resources_adjoint(self, eps):
        """Test that the adjoint resources are correct."""
        expected = [re.GateCount(re.ResourceIsingXX.resource_rep(eps=eps))]
        assert re.ResourceIsingXX.default_adjoint_resource_decomp(eps=eps) == expected
    ...
    def test_resources_controlled(self, eps):
        ...
        assert op.default_resource_decomp(**op.resource_params) == expected
...
class TestIsingXY:
    ...
    def test_resources(self, eps):
        ...
        assert re.ResourceIsingXY.default_resource_decomp(eps=eps) == expected

    @pytest.mark.parametrize("eps", (None, 1e-3))
    def test_resources_pow(self, eps):
        """Test that the pow resources are correct."""
        expected = [re.GateCount(re.ResourceIsingXY.resource_rep(eps=eps))]
        assert re.ResourceIsingXY.default_pow_resource_decomp(pow_z=3, eps=eps) == expected

    @pytest.mark.parametrize("eps", (None, 1e-3))
    def test_resources_adjoint(self, eps):
        """Test that the adjoint resources are correct."""
        expected = [re.GateCount(re.ResourceIsingXY.resource_rep(eps=eps))]
        assert re.ResourceIsingXY.default_adjoint_resource_decomp(eps=eps) == expected
...
class TestIsingYY:
    ...
    def test_resources(self, eps):
        ...
        assert re.ResourceIsingYY.default_resource_decomp(eps=eps) == expected

    @pytest.mark.parametrize("eps", (None, 1e-3))
    def test_resources_pow(self, eps):
        """Test that the pow resources are correct."""
        expected = [re.GateCount(re.ResourceIsingYY.resource_rep(eps=eps))]
        assert re.ResourceIsingYY.default_pow_resource_decomp(pow_z=3, eps=eps) == expected

    @pytest.mark.parametrize("eps", (None, 1e-3))
    def test_resources_adjoint(self, eps):
        """Test that the adjoint resources are correct."""
        expected = [re.GateCount(re.ResourceIsingYY.resource_rep(eps=eps))]
        assert re.ResourceIsingYY.default_adjoint_resource_decomp(eps=eps) == expected
...
class TestIsingZZ:
    ...
    def test_resources(self, eps):
        ...
        assert re.ResourceIsingZZ.default_resource_decomp(eps=eps) == expected

    @pytest.mark.parametrize("eps", (None, 1e-3))
    def test_resources_pow(self, eps):
        """Test that the pow resources are correct."""
        expected = [re.GateCount(re.ResourceIsingZZ.resource_rep(eps=eps))]
        assert re.ResourceIsingZZ.default_pow_resource_decomp(pow_z=3, eps=eps) == expected

    @pytest.mark.parametrize("eps", (None, 1e-3))
    def test_resources_adjoint(self, eps):
        """Test that the adjoint resources are correct."""
        expected = [re.GateCount(re.ResourceIsingZZ.resource_rep(eps=eps))]
        assert re.ResourceIsingZZ.default_adjoint_resource_decomp(eps=eps) == expected
...
class TestPSWAP:
    ...
    def test_resources(self, eps):
        ...
        assert re.ResourcePSWAP.default_resource_decomp(eps=eps) == expected

    @pytest.mark.parametrize("eps", (None, 1e-3))
    def test_resources_adjoint(self, eps):
        """Test that the adjoint resources are correct."""
        expected = [re.GateCount(re.ResourcePSWAP.resource_rep(eps=eps))]
        assert re.ResourcePSWAP.default_adjoint_resource_decomp(eps=eps) == expected
