...
class TestResourceHybridQRAM:
    """Test the ResourceSelectOnlyQRAM class."""

    @pytest.mark.parametrize(
        ("bitstrings", "num_tree_control_wires", "num_target_wires", "num_select_wires"),
        [(["000", "010", "101", "111"], 3, 3, 5)],
    )
    def test_resource_params(
        self, bitstrings, num_tree_control_wires, num_target_wires, num_select_wires
    ):
        """Test that the resource params are correct."""
        op = plre.ResourceHybridQRAM(
            bitstrings, num_tree_control_wires, num_target_wires, num_select_wires
        )
        assert op.resource_params == {
    ...
    def test_resource_rep(
        ...
        assert (
            plre.ResourceHybridQRAM.resource_rep(
                bitstrings, num_tree_control_wires, num_target_wires, num_select_wires
            )
            == expected
        )
    ...
    @pytest.mark.parametrize(
        "bitstrings, num_tree_control_wires, num_target_wires, num_select_wires, expected_res",
        (
            (
                ["000", "010", "101", "111"],
                1,
                ...
                1,
                [
                    GateCount(resource_rep(plre.ResourceX), 0),
                    GateCount(resource_rep(plre.ResourceHadamard), 12),
                    GateCount(resource_rep(plre.ResourceCNOT), 2),
                    GateCount(resource_rep(plre.ResourceCZ), 6),
                    GateCount(
                        plre.ResourceControlled.resource_rep(
                            resource_rep(plre.ResourceX), num_ctrl_wires=1, num_ctrl_values=1
                        ),
                        2,
                    ),
                    GateCount(resource_rep(plre.ResourceCSWAP), 20),
                    GateCount(
                        plre.ResourceControlled.resource_rep(
                            base_cmpr_op=plre.ResourceControlled.resource_rep(
                                base_cmpr_op=resource_rep(plre.ResourceSWAP),
                                num_ctrl_wires=1,
                                num_ctrl_values=0,
                            ),
                            num_ctrl_wires=1,
    ...
    def test_resources(
        self, bitstrings, num_tree_control_wires, num_target_wires, num_select_wires, expected_res
    ):
        """Test that the resources are correct."""
        assert (
            plre.ResourceHybridQRAM.resource_decomp(
                bitstrings, num_tree_control_wires, num_target_wires, num_select_wires
            )
            == expected_res
        )
