...
class TestSelectPauliRotDecompositions:
    ...
    @staticmethod
    def make_wires(precision, use_qjit):
        """Create wire registers for tests."""
        if use_qjit:
            wire = 0
            start = 1
            ...
            control_wires = list(range(start, start := start + 2))
        else:
            wire = "targ"
        ...
        return wire, control_wires, angle_wires, phase_grad_wires, work_wires
    ...
    def test_units_select_pauli_rot_phase_gradient(self, p):
        ...
        wire, control_wires, angle_wires, phase_grad_wires, work_wires = self.make_wires(p, False)
...
class TestPauliRotationDecomposition:
    ...
    def make_wires(precision, use_qjit):
        ...
        if use_qjit:
            ...
            phase_grad_wires = qp.wires.Wires([f"qft_{i}" for i in range(precision)])
            work_wires = qp.wires.Wires([f"work_{i}" for i in range(precision - 1)])
        return wire, angle_wires, phase_grad_wires, work_wires
    ...
    def test_units_rz_phase_gradient(self, p):
        ...
        wire, angle_wires, phase_grad_wires, work_wires = self.make_wires(p, False)
    ...
    def test_global_phases(self):
        ...
        p = 4
        wire, angle_wires, phase_grad_wires, work_wires = self.make_wires(p, False)
        circ = qp.tape.QuantumScript([qp.RZ(phi, wire) for phi in phis])
    ...
    def test_IsingXY_not_supported(self):
        ...
        _, angle_wires, phase_grad_wires, work_wires = self.make_wires(2, False)
    ...
    def test_integration_single_wire(self, phi, RGate):
        ...
        precision = 3
        wire, angle_wires, phase_grad_wires, work_wires = self.make_wires(precision, False)
