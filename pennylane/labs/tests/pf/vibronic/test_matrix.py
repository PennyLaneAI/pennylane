...
from pennylane.labs.pf import Node, RealspaceOperator, RealspaceSum, VibronicMatrix, op_norm

# pylint: disable=too-many-arguments

vword0 = RealspaceSum([RealspaceOperator(tuple(), Node.tensor_node(np.array(0.5)))])
blocks0 = {(0, 0): vword0}
vmat0 = VibronicMatrix(1, 1, blocks0)

vword1 = RealspaceSum([RealspaceOperator(("P",), Node.tensor_node(np.array([1])))])
blocks1 = {(0, 0): vword1}
vmat1 = VibronicMatrix(1, 1, blocks1)

vword2a = RealspaceSum(
    [RealspaceOperator(("P", "P"), Node.tensor_node(np.array([[0, 1], [2, 3]])))]
)
vword2b = RealspaceSum([RealspaceOperator(("P",), Node.tensor_node(np.array([1, 2])))])
