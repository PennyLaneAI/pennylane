...
"""Regression tests for the qubit TCDQ MMD loss."""
...
class TestComputeSingleMmd:
    ...
    def test_sqrt_loss_flag(self):
        ...
        variances = jnp.zeros(n_ops)

        val = _compute_single_mmd(tr_iqp, variances, gt, vis, False)
        sqr = _compute_single_mmd(tr_iqp, variances, gt, vis, True)

        assert np.isclose(float(sqr), np.sqrt(abs(float(val))), atol=1e-7)

    def test_correction_removes_bias(self):
        """The variance correction should make the model-model term unbiased.

        Subtracting the estimated variance of each expectation-value estimator
        from its squared estimate gives the model-model U-statistic.
        """
