...
class Build_PES:
    ...
    - saving: whether HDF5 files are created storing the calculated quantities
        ...
        - localize: perform mode localization technique
        - loc_freqs: array of frequencies (in cm-1) where separation happens for mode localization
    """
    ...
    def __init__(
        ...
        self.harmonic_res = self.harmonic_analysis()
        self.freqs = self.harmonic_res["freq_wavenumber"] / au_to_cm
        self.displ_vecs = self.harmonic_res["norm_mode"]
        if self.localize:
            self.loc_res, self.uloc, self.displ_vecs = pm_custom_separate_localization(
                self.harmonic_res, freq_separation=loc_freqs
            )

        self.freqs = self.loc_res["freq_wavenumber"] / au_to_cm

        #        displ_vecs = np.array(COMM.bcast(displ_vecs, root=0))
        #        freqs = np.array(COMM.bcast(freqs, root=0))
        ...
        print("onebody: ", self.pes_onebody, self.dipole_onebody)
        COMM.Barrier()
        ...
        #        print("twobody: ", self.pes_twobody, self.dipole_twobody)
        COMM.Barrier()

        if do_cubic:
            self.pes_threebody, self.dipole_threebody = self.get_pes_threebody(do_dipole=do_dip_2)
    ...
    def run_electronic_structure(self, molecule):
        ...
        mol = pyscf.gto.Mole(
            atom=geom, symmetry="C1", spin=spin, charge=molecule.charge, unit="bohr"
        )
    ...
    def get_equilibrium_geom(self):
        ...
        print("After optimization: ", geom_eq.atom_coords(unit="A"))
        mol_eq = qml.qchem.Molecule(
            self.molecule.symbols,
            geom_eq.atom_coords(unit="A"),
            unit="angstrom",
    ...
    def get_pes_onebody(self, do_dipole=False):
        ...
        for ii in tqdm(range(len(self.displ_vecs)), desc="Loop one-body pes"):
            ...
            for jj in jobs_on_rank:
                ...
                positions = np.array(
                    [
                        np.array(init_geom[ll][1]) * bohr_to_ang + scaling * pt * displ_vec[ll, :]
        ...
        for aa in range(len(self.displ_vecs)):
            ...
            for bb in range(len(self.displ_vecs)):
                ...
                for [ii, pt1, jj, pt2] in jobs_on_rank:
                    ...
                    positions = np.array(
                        [
                            np.array(init_geom[ll][1]) * bohr_to_ang
                    ...
                    local_pes_twobody[ii, jj] = (
                        disp_hf.e_tot
                        - self.pes_onebody[aa, ii]
                        - pes_onebody[bb, jj]
                        - self.scf_result.e_tot
                    )

                    if do_dipole:
                        local_dipole_twobody[ii, jj, :] = (
                            get_dipole(disp_hf, self.method)
                            - self.dipole_onebody[aa, ii, :]
                            - self.dipole_onebody[bb, jj, :]
                            - ref_dipole
                        )
    ...
    def get_pes_threebody(self, do_dipole=False):
        ...
        for [aa, bb, cc] in tqdm(all_mode_combos, desc="Outer loop three-body pes"):
            ...
            mm = 0
            for [ii, pt1, jj, pt2, kk, pt3] in tqdm(
                boscombos_on_rank, desc="Inner loop three-body pes"
            ):
                ...
                local_pes_threebody[ind] = (
                    disp_hf.e_tot
                    - self.pes_twobody[aa, bb, ii, jj]
                    - self.pes_twobody[aa, cc, ii, kk]
                    - self.pes_twobody[bb, cc, jj, kk]
                    - self.pes_onebody[aa, ii]
                    - self.pes_onebody[bb, jj]
                    - self.pes_onebody[cc, kk]
                    - self.scf_result.e_tot
                )
                if do_dipole:
                    local_dipole_threebody[ind, :] = (
                        get_dipole(disp_hf, self.method)
                        - self.dipole_twobody[aa, bb, ii, jj, :]
                        - self.dipole_twobody[aa, cc, ii, kk, :]
                        - self.dipole_twobody[bb, cc, jj, kk, :]
                        - self.dipole_onebody[aa, ii, :]
                        - self.dipole_onebody[bb, jj, :]
                        - self.dipole_onebody[cc, kk, :]
                        - ref_dipole
                    )
                mm += 1
...
molecule = qml.qchem.Molecule(sym, geometry, basis_name="6-31g", unit="Angstrom")
pes = Build_PES(molecule)
get_christiansen_form(pes, do_cubic=False)
