...
def pes_onemode(
    ...
    for ii in range(len(displ_vecs)):
        ...
        for jj in jobs_on_rank:
            ...
            positions = np.array(
                [
                    np.array(init_geom[ll][1]) + scaling * pt * displ_vec[ll, :]
                    for ll in range(len(molecule.symbols))
                ]
            )

            disp_mol = qml.qchem.Molecule(
                molecule.symbols,
                positions,
    ...
    for aa in range(len(displ_vecs)):
        ...
        for bb in range(len(displ_vecs)):
            ...
            for [ii, pt1, jj, pt2] in jobs_on_rank:
                ...
                disp_hf = single_point(disp_mol, method=method)
                local_pes_twobody[ii, jj] = (
                    disp_hf.e_tot - pes_onebody[aa, ii] - pes_onebody[bb, jj] - scf_result.e_tot
                )

                if do_dipole:
                    local_dipole_twobody[ii, jj, :] = (
                        get_dipole(disp_hf, method)
                        - dipole_onebody[aa, ii, :]
                        - dipole_onebody[bb, jj, :]
                        - ref_dipole
                    )

            # gather the results on head process only
            pes_twobody[aa, bb, :, :] = np.sum(
                np.array(comm.gather(local_pes_twobody, root=0)), axis=0
            )
            if do_dipole:
                dipole_twobody[aa, bb, :, :] = np.sum(
                    np.array(comm.gather(local_dipole_twobody, root=0)), axis=0
                )
...
def _local_pes_threemode(
    ...
    if do_dipole:
        local_dipole_threebody = np.zeros(
            (len(all_mode_combos) * len(boscombos_on_rank), 3), dtype=float
        )
        ref_dipole = get_dipole(scf_result, method)
    ...
    for [aa, bb, cc] in all_mode_combos:
        ...
        for [ii, pt1, jj, pt2, kk, pt3] in boscombos_on_rank:
            ...
            local_pes_threebody[ind] = (
                disp_hf.e_tot
                - pes_twobody[aa, bb, ii, jj]
                - pes_twobody[aa, cc, ii, kk]
                - pes_twobody[bb, cc, jj, kk]
                - pes_onebody[aa, ii]
                - pes_onebody[bb, jj]
                - pes_onebody[cc, kk]
                - scf_result.e_tot
            ...
            if do_dipole:
                local_dipole_threebody[ind, :] = (
                    get_dipole(disp_hf, method)
                    - dipole_twobody[aa, bb, ii, jj, :]
                    - dipole_twobody[aa, cc, ii, kk, :]
                    - dipole_twobody[bb, cc, jj, kk, :]
                    - dipole_onebody[aa, ii, :]
                    - dipole_onebody[bb, jj, :]
                    - dipole_onebody[cc, kk, :]
                    - ref_dipole
                )
            mm += 1
...
def _load_pes_threemode(num_pieces, nmodes, ngridpoints):
    ...
    """
    final_shape = (nmodes, nmodes, nmodes, ngridpoints, ngridpoints, ngridpoints)
...
def _load_dipole_threemode(num_pieces, nmodes, ngridpoints):
    ...
    for mode_combo in range(nmode_combos):
        ...
        for rank in range(num_pieces):
            ...
            chunk = np.array_split(local_dipole_threebody, nmode_combos, axis=0)[mode_combo]
            l1 += chunk.shape[0]
            local_chunk[l0:l1, :] = chunk
            l0 += chunk.shape[0]
...
def pes_threemode(
    ...
    if rank == 0:
        ...
        if do_dipole:
            dipole_threebody = _load_dipole_threemode(
                comm.Get_size(), len(freqs_au), len(gauss_grid)
            )
...
def vibrational_pes(
    ...
    pes_onebody, dipole_onebody = pes_onemode(
        molecule, scf_result, freqs_au, displ_vecs, gauss_grid, method=method, do_dipole=True
    )
    comm.Barrier()
    ...
    return VibrationalPES(
        freqs_au, gauss_grid, gauss_weights, uloc, pes_arr, dipole_arr, localize, get_anh_dipole
    )
