...
class TrotterCDF(Operation):
    r"""Second-order Suzuki-Trotter product for a Compressed Double Factorized (CDF) Hamiltonian.

    Args:
        hamiltonian (dict): A CDF Hamiltonian dictionary with keys:
            * ``"nuc_constant"`` (float): Zero-energy constant.
            * ``"core_tensors"`` (array[float]): Shape ``(L+1, N, N)`` --
              diagonal interaction tensors per fragment, where N is the number of spatial orbitals,
              and L is the number of two-body fragments.
            * ``"leaf_tensors"`` (array[float]): Shape ``(L+1, N, N)`` --
              per-mode basis-rotation unitaries per fragment, where N is the number of spatial orbitals,
              and L is the number of two-body fragments.
        time (float): Total evolution time :math:`t`.
        num_steps (int): Number of second-order Trotter steps.
        wires (Wires): System wires, expects ``2*N`` wires for alpha / beta spins.
        control_wires (Wires | None): Optional control wire(s) for controlled
            time evolution (e.g. Hadamard test).

    """

    resource_keys = {"num_fragments", "num_orbitals", "num_steps", "control_wires"}
    ...
    def __init__(self, hamiltonian, time, num_steps, wires=None, control_wires=None, id=None):
        ...
        if not (Z.ndim == 3 and U.ndim == 3):
            raise TypeError(
                "TrotterCDF expects core_tensors with ndim=3 and leaf_tensors with ndim=3. "
                f"Got core_tensors.ndim={Z.ndim}, leaf_tensors.ndim={U.ndim}."
            )
    ...
    def resource_params(self) -> dict:
        hamiltonian = self._hyperparameters["hamiltonian"]
        num_fragments = hamiltonian["core_tensors"].shape[0] - 1
        num_orbitals = hamiltonian["core_tensors"].shape[1]
        return {
            "num_fragments": num_fragments,
...
def _trotter_cdf_resources(num_steps, num_fragments, num_orbitals, control_wires):
    ...
    num_zz_gates = 2 * num_fragments * num_orbitals * (2 * num_orbitals - 1) * num_steps
