...
def get_reduced_model_manual(omegas, couplings, m_max, order_max=2, selected_modes=None, selected_states=None):
    """
    `omegas` and `couplings` are the original omega and coupling tensors
    ...
    for order in couplings_red:
        if order == 1:
            ...
            couplings_red[order] = couplings_red[order][:, :, selected_modes, :][:, :, :, selected_modes]
        elif order == 3:
            raise ValueError("cant handle cubic order yet")
        elif order == 4: 
