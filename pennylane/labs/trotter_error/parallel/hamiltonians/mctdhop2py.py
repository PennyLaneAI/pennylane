...
def parse_parameter_section(lines, verbose=True):
    ...
    all_units = (
        []
    )  # make sure everything is done in the same units, otherwise need to implement conversions.
    ...
    for term in terms:
        if not term["KEO"]:  # potential
            ...
            if len(el_idxs) == 0:  # Acts as identity on electronic space
                if deg == 2:  # quadratic
                    if not (
                        term["COEFF"] == omega[term["VIB"][0] - 2] / 2
                    ):  # skip harmonic part, doesnt get added to betas!
