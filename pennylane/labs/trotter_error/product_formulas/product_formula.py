...
class ProductFormula:
    ...
    Product formulas approximate the time-evolution operator of a Hamiltonian
    :math:`H = \sum_j H_j` by a product of exponentials of the individual
    fragments :math:`H_j` given by the formula

    .. math::

        e^{i H t} \approx \prod_{k=1}^n e^{i c_{j_k} H_{j_k} t},

    for :math:`j_1, \dots, j_n \in \{H_1, \dots, H_n\}`.
    ...
    represent repeated Trotter steps.
