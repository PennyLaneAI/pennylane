...
class ASTNode:
    ...
    @abstractmethod
    def leaves(self) -> list[SymbolNode]:
        """Returns a list of the leaf nodes in the commutator AST"""
...
class SymbolNode(ASTNode):
    """
    Represents a leaf node in the commutator tree, holding a linear combination of symbols.
    Symbols can be any hashable element.
    """

    def __init__(
        self, symbols: Hashable | Sequence[Hashable], coeffs: float | Sequence[float] = 1.0
    ):
        ...
        for symbol, coeff in zip(symbols, coeffs):
            ...
            if isinstance(symbol, ASTNode):
                raise TypeError("Symbols cannot be type `ASTNode`.")
            tmp_symbol_list.append((symbol, coeff))

        self.symbols = frozenset(tmp_symbol_list)
    ...
    def __str__(self):
        if len(self.symbols) == 1:
            symbol, coeff = list(self.symbols)[0]
            return str(symbol) if np.isclose(coeff, 1.0) else f"{coeff}*{symbol}"
    ...
    def is_zero(self) -> bool:
        return all(np.isclose(coeff, 0) for _, coeff in self.symbols)

    def split(self) -> list[SymbolNode]:
        """Split a linear combination of symbols into individual symbol nodes"""
        return [SymbolNode(symbol, coeff) for symbol, coeff in self.symbols]

    def eval(self, fragments: dict[Hashable, Fragment]) -> Fragment:
        ret = _AdditiveIdentity()

        tmp_symbols = list(self.symbols)
        tmp_symbols.sort(key=lambda x: str(x[0]))

        for symbol, coeff in tmp_symbols:
            try:
...
def bilinear_expansion(
    commutator_node: ASTNode,
    terms: dict[SymbolNode, dict[ASTNode, float]],
    max_order: int,
    bch_coeff: float,
) -> dict[ASTNode, float]:
    r"""
    ...
    Assumes all symbol nodes in `commutator_node` are placeholders
    that have a corresponding expansion in the `terms` dictionary.
    """

    expanded = defaultdict(float)
    target_leaves = commutator_node.leaves()
...
def _separate_by_order(d: dict[ASTNode, float], max_order: int) -> list[dict[ASTNode, float]]:
...
def is_mergeable(
    ...
    For example in [[A, B], [C, [D, E]]], C is the component at index 2.

    Two commutators are mergable at index k if they have the same nesting structure, and their
    symbols only differ at index k. For example,
    ...
    if index < node1.left.order:
        if node1.right != node2.right:
            return False
        return is_mergeable(node1.left, node2.left, index)
...
def merge(node1: ASTNode, node2: ASTNode, index: int):
    ...
    if not is_mergeable(node1, node2, index):
        raise RuntimeError(f"The two commutators are not mergeable at index {index}.")

    sum_comm = copy.deepcopy(node1)

    left_node = node1[index]
    right_node = node2[index]

    tmp_dict = defaultdict(float)
    for symbol, coeff in chain(left_node, right_node):
        tmp_dict[symbol] += coeff

    sum_comm[index] = frozenset(tmp_dict.items())

    return sum_comm


def is_tree_isomorphic(node1: ASTNode, node2: ASTNode) -> bool:
    """
...
def _replace_node_mutate_source(
    source_node: ASTNode, target_node: ASTNode, new_nodes: list[ASTNode]
):
    """
    ...
    if source_node.left == target_node:
        if not new_nodes:
            raise RuntimeError(
                "Got fewer replacement nodes than the number of target nodes in the commutator "
                "tree."
            )
        source_node.left = copy.deepcopy(new_nodes.pop(0))

    elif isinstance(source_node.left, CommutatorNode):
        _replace_node_mutate_source(source_node.left, target_node, new_nodes)
    ...
    if source_node.right == target_node:
        ...
        _replace_node_mutate_source(source_node.right, target_node, new_nodes)


def replace_node(source_node: ASTNode, target_node: ASTNode, new_nodes: list[ASTNode]):
    """
    Replace target_node in the tree with the new_nodes.
    Each next occurrence of target_node will be replaced with the next node in new_nodes.
    ...
    Returns:
        An ASTNode object with replacements.
    """
    ...
    _replace_node_mutate_source(result, target_node, replacements)
