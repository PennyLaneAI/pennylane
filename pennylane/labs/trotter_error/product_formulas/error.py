...
def perturbation_error(
    ...
    Args:
        ...
        return_convergence_info (bool): If True, returns a tuple (expectation_values, convergence_info)
            ...
            mean and variance histories, execution times, and distribution information.
    ...
    **Example**
    ...
    if sampling_method not in valid_sampling_methods:
        raise ValueError(
            f"sampling_method must be one of {valid_sampling_methods}, got '{sampling_method}'"
        )
...
def _group_sums(
    ...
    """
    # Skip the 0th order terms (identity) and start from 1st order
    return [
        x
        for xs in [_group_sums_in_dict(term_dict) for term_dict in term_dicts[1:] if term_dict]
        for x in xs
    ]
...
def _adaptive_sampling(
    ...
    if convergence_info:
        total_samples = sum(
            state_info["n_samples"] for state_info in convergence_info["states_info"]
        )
...
def _setup_importance_probabilities(
    ...
    if total_prob_sum == 0:
        warnings.warn(
            "All probabilities are zero, falling back to uniform distribution", UserWarning
        )
...
def _adaptive_sample_single_state(
    ...
    while n_samples < max_sample_size:
        ...
        current_variance = (
            (sum_squared - n_samples * current_mean * current_mean) / (n_samples - 1)
            if n_samples > 1
            else 0.0
        )
        ...
        if n_samples >= min_sample_size:
            ...
            variance = (
                (sum_squared - n_samples * mean * mean) / (n_samples - 1) if n_samples > 1 else 0
            )
            ...
            if abs(mean) > 1e-12:
                ...
                if std_dev < target_error:
                    convergence_time = time.time() - state_start_time
                    print(
                        f"  ✓ CONVERGED after {n_samples} samples in {convergence_time:.2f}s (mean ≈ 0)"
                    )
    ...
    if n_samples >= max_sample_size:
        ...
        variance = (
            (sum_squared - n_samples * final_mean * final_mean) / (n_samples - 1)
            if n_samples > 1
            else 0
        )
        variance_abs = abs(variance)
...
def _convergence_sample_single_state(
    ...
    while n_samples < max_sample_size:
        ...
        if n_samples >= min_sample_size and len(mean_history) > convergence_window:
            ...
            if convergence_checks_passed >= min_convergence_checks:
                ...
                print(
                    f"    Relative change: {relative_change:.6e} (tolerance: {convergence_tolerance:.6e})"
                )
                break
    ...
    if n_samples >= max_sample_size:
        ...
        if len(mean_history) > convergence_window:
            ...
            print(
                f"    Final relative change: {rel_change:.6e} (tolerance: {convergence_tolerance:.6e})"
            )
...
def _fixed_sampling(
    ...
    """
    print(
        f"Using fixed-size sampling with {sample_size} commutators out of {len(commutators)} total"
    )
    ...
    if sampling_method == "random":
        ...
        sampled_indices = np.random.choice(
            len(commutators), size=effective_sample_size, replace=True
        )
        ...
        print(
            f"Will select the top {sample_size} most important commutators (deterministic, without replacement)"
        )
        ...
        probabilities = _setup_importance_probabilities(
            ...
            timestep,
            gridpoints,
            "importance",  # Force importance calculation
        )
        prob_time = time.time() - start_time
        print(f"Probability calculation completed in {prob_time:.2f} seconds")
        ...
        print(
            f"Selected top {len(sampled_commutators)} commutators (uniform weights: {commutator_weights[0] if commutator_weights else 'N/A'})"
        )
    else:
...
def _sample_importance_commutators(
    ...
    sampled_indices = np.random.choice(
        len(commutators), size=sample_size, replace=True, p=probabilities
    )
...
def _compute_expectation_values(
    ...
    if backend == "serial":
        ...
        for state_idx, state in enumerate(states):
            ...
            for weight, commutator in tqdm(
                zip(weights, commutators),
                desc=f"Processing commutators for state {state_idx+1}",
...
def _compute_state_expectation(commutators, weights, fragments, state):
    ...
    new_state = sum(
        (
            weight * _apply_commutator(commutator, fragments, state)
            for commutator, weight in zip(commutators, weights)
        ),
