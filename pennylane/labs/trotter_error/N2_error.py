...
sizes = ([5, 4], [6,8], [8, 10], [10, 14])
fig, ax = plt.subplots(1, 1, figsize=(7, 7))
Y3s = []
for ncas, nelecas in sizes:
    cdf_folder = os.path.join(os.path.dirname(__file__), f"N2_CDFs/opt_hamiltonian_n2_{ncas}_{nelecas}")
...
ax.plot(x, np.exp(y), 'o', label=r"$N_{2}$")
