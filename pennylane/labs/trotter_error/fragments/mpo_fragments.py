...
def mpo_fragments(
    fragments: Sequence[MPO],
    bond_dim: int = 100,
    norm_values: Sequence[float] = None,
    n_sites: int = None,
    h1e_vals: Sequence[Array] = None,
    g2e_vals: Sequence[Array] = None,
    driver=None,
) -> List[MPOFragment]:
    ...
    for i, fragment in enumerate(fragments):
        ...
        norm = (
            np.sum(np.abs(h1e)) + np.sum(np.abs(g2e))
            if h1e is not None and g2e is not None
            else (norm_values[i] if norm_values is not None and i < len(norm_values) else None)
        )

        mpo_fragment_list.append(
            MPOFragment(
                fragment,
                h1e=h1e,
                g2e=g2e,
    ...
    **Example**
    ...
    def __init__(
        ...
        self.h1e = (
            h1e
            if h1e is not None
            else (np.zeros((n_sites, n_sites), dtype=np.float64) if n_sites is not None else None)
        )
        self.g2e = (
            g2e
            if g2e is not None
            else (
                np.zeros((n_sites, n_sites, n_sites, n_sites), dtype=np.float64)
    ...
    def __mul__(self, scalar: float):
        ...
        return MPOFragment(
            fragment, h1e=h1e, g2e=g2e, driver=self.driver, norm=norm, bond_dim=self.bond_dim
        )

    __rmul__ = __mul__

    def __add__(self, other: MPOFragment):
        if (self.h1e is not None) and (other.h1e is not None) and (self.driver is not None):
            h1e = self.h1e + other.h1e
            g2e = self.g2e + other.g2e
            new_fragment = self.driver.get_qc_mpo(
                h1e=h1e, g2e=g2e, ecore=0, iprint=0, add_ident=False
            )
            ...
            norm = (
                self._norm_value + other._norm_value
                if self._norm_value is not None and other._norm_value is not None
                else None
            )
    ...
    def __matmul__(self, other: MPOFragment):
        ...
        return MPOFragment(
            new_fragment, h1e=h1e, g2e=g2e, driver=None, norm=norm, bond_dim=self.bond_dim
        )
    ...
    def norm(self, params: Dict = None) -> float:
        ...
        if hasattr(self, "_norm_value") and self._norm_value is not None:
            ...
            raise ValueError(
                "Norm value is not set and cannot be computed from h1e and g2e values."
            )
