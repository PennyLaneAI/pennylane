...
class CircuitDrawer:
    ...
    def make_wire_conversion_dicts(self, raw_operation_grid, raw_observable_grid):
        ...
        all_wires = [op.wires.as_list() for op in all_operators if op is not None]
        ...
        for i in range(grid.num_layers):
            ...
            for op in layer_operators:
                ...
                if isinstance(op, qml.operation.Tensor):
                    # pylint: disable=protected-access
                    wires = list(qml.utils._flatten(op.wires.as_list()))
                else:
                    wires = op.wires.as_list()
        ...
        while i < n - 1:
            ...
            for j in range(len(layer_ops)):
                ...
                if len(op.wires.as_list()) > 1:
                    sorted_wires = op.wires.as_list().copy()
                    sorted_wires.sort()
                    ...
                    for k in range(j + 1, len(layer_ops)):
                        ...
                        other_sorted_wires = other_op.wires.as_list().copy()
