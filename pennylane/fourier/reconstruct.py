...
def reconstruct(
    qnode, _spectra=None, order=1, shifts=None, nums_frequency=None, ids="auto", decimals=5
):
    @wraps(qnode)
    def wrapper(*args, **kwargs):
        ...
        if nums_frequency is None:
            if _spectra is None:
                _spectra = qml.fourier.spectrum(qnode, encoding_gates=ids, decimals=decimals)(
                    *args, **kwargs
                )
            # else:
