...
def simple_spectrum(qnode, encoding_gates=None, decimals=5):
    ...
    .. note::
        ...
        More precisely, the ``simple_spectrum`` function relies on the gate to
    ...
    *Fourier series*.
    ...
    The ``simple_spectrum`` function computes all frequencies that will potentially appear in the
    ...
    **Example**
    ...
    .. code-block:: python
        ...
        import numpy as np
        from pennylane.fourier import simple_spectrum
        ...
        res = simple_spectrum(circuit)(x, w)
    ...
    .. code-block:: python
        ...
        x = np.array([1, 2])
        res = simple_spectrum(circuit, encoding_gates=["x0"])(x)
    ...
    .. note::
        The ``simple_spectrum`` function does not check if the result of the
        circuit is an expectation, or if gates with the same ``id``
        take the same value in a given call of the function.

    The ``simple_spectrum`` function works in all interfaces:
