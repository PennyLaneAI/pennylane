...
class AdjointOp(IRDLOperation):
    ...
    Results:
        qreg (QuregSSAValue): the modified quantum register.
...
class AllocOp(IRDLOperation):
    ...
    Results:
        qreg (QuregSSAValue): The allocated quantum register
    """
...
class DeallocOp(IRDLOperation):
    ...
    .. note::
        ...
        One should not use the qreg value deallocated after this
...
class HamiltonianOp(IRDLOperation):
    ...
    Args:
        coeffs (SSAValue): A tensor of rank 1, with dimension corresponding to the number of
            input observables.
        terms (Sequence[ObservableSSAValue]): observables.

    Results:
        obs (ObservableSSAValue): The hamiltonian observable corresponding to the ``coeffs``
            and ``terms``.
...
class HermitianOp(IRDLOperation):
    ...
    Args:
        matrix (SSAValue): A tensor of rank 2.
        qubits (Sequence[QubitSSAValue]): qubits to measure.

    Result:
        obs (ObservableSSAValue): A hermitian observable.
    """
...
class NamedObsOp(IRDLOperation):
    ...
    Args:
        ...
        type (NamedObservableAttr): Type of observable
    Results:
        obs (SSAObservableValue): Observable value.
...
class ProbsOp(IRDLOperation):
    ...
    Results:
        probabilities (SSAValue): A tensor containing probabilities.
...
class StateOp(IRDLOperation):
    ...
    Args:
        ...
        dynamic_shape (SSAValue | None): Optional variable used during dynamic register allocation
            to denote how many elements will be available in the probabilities tensor output.
    Results:
        state (SSAValue): Tensor containing the state.
...
class TensorOp(IRDLOperation):
    ...
    Args:
        terms (Sequence[ObservableSSAValue]): observables that must be defined by
            operations in the local scope.
    Results:
        obs (ObservableSSAValue): Value corresponding to the tensor observable.
...
class VarianceOp(IRDLOperation):
    ...
    Results:
        variance (SSAValue): variance for the observable
...
class YieldOp(IRDLOperation):
    ...
    Args:
        retvals (QuregSSAValue): the quantum register to be returned
