...
"""Builds a PyDot graph representation of a captured PennyLane circuit."""
...
class PyDotGraphBuilder:
    ...
    def add_cluster_to_graph(
        self, cluster: pydot.Cluster, graph: pydot.Graph | pydot.Cluster = None
    ) -> None:
    ...
    def add_quantum_node_to_graph(
        self,
        node: OperatorNode | MeasurementNode | DeviceNode,
    ...
    def _connect_quantum_node(self, node: OperatorNode | MeasurementNode) -> None:
...
def _rgb_to_hex(rgb_tuple: tuple[float, float, float]) -> str:
    return f"#{max(0, min(255, int(rgb_tuple[0]))):02x}{max(0, min(255, int(rgb_tuple[1]))):02x}{max(0, min(255, int(rgb_tuple[2]))):02x}"
...
def shade_pydot_clusters_by_depth(
    ...
    """Shades clusters in a pydot graph based on their depth."""
