...
r"""
This file contains the OperatorChecker debugging and developing tool.
"""
import inspect
...
def is_diagonal(matrix, atol=1e-10):
    ...
    off_diagonal = matrix - qml.math.diag(qml.math.diag(matrix))
    return qml.math.allclose(off_diagonal, qml.math.zeros_like(matrix), atol=atol, rtol=0.0)


def torch_jacobian(fn):
    """Jacobian in the torch interface with the same
    syntax as qml.jacobian. Note that this is not performant!"""
    return lambda *args: torch.autograd.functional.jacobian(fn, args)


def tf_jacobian(fn):
    """Jacobian in the tensorflow interface with the same
    syntax as qml.jacobian. Note that this is not performant!"""
...
def recipe_yields_commutator(recipe, op, par, wires):
    ...
    Args:
        ...
        op (type): Operator subclass to be checked.
        par (array_like): Parameter position at which to check the operator.
            As the test is only implemented for single-parameter operators,
            the shape of ``par`` is expected to be ``(1,)``.
        wires (.Wires): Wires with which to instantiate the checked operator.
    ...
    H += H.T.conj()
    return qml.math.allclose(d_apply_recipe(H), d_commutator_fn(H))


def wrap_op_method(op, method, expected_exc):
    r"""Wrap a method of an operator with a try-except clause, allowing for an expected
    exception and catching (and returning) other exceptions.

    Args:
        op (type or .Operator): Operator that has the method to be wrapped
        method (str): Name of the method to be wrapped
        expected_exc (type): Exception type to ignore

    Returns:
        callable: The wrapped method of the operator.
    ...
    def wrapped_method(*args, **kwargs):
        r"""Wrapped operator method that tolerates an expected exception
...
def matrix_from_hardcoded_matrix(op, par, wires):
    r"""Get the matrix of an operator, using ``get_matrix``.

    Args:
        op (type): Operator type to obtain the matrix for
        par (array_like): Parameters of the operator
        wires (.wires.Wires): Wires of the operator

    Returns:
        object: Matrix of the operator if it is defined and no problem occured
...
def matrix_from_sparse_matrix(op, par, wires):
    r"""Get the matrix of an operator, using ``sparse_matrix``.
...
def matrix_from_terms(op, par, wires):
    r"""Get the matrix of an operator, using its ``terms``.
...
def matrix_from_decomposition(op, par, wires):
    r"""Get the matrix of an operator, using its ``decomposition``.
...
def matrix_from_single_qubit_rot_angles(op, par, wires):
    r"""Get the matrix of an operator, using its ``single_qubit_rot_angles``.
...
def matrix_from_generator(op, par, wires):
    r"""Get the matrix of an operator, using its ``generator``.
...
class CheckerError(Exception):
    """An internal error raised in OperatorChecker used to mark specific exceptions."""


class OperatorChecker:
    r"""Check one or multiple operator subclasses to define all required properties,
    ...
    Args:
        ...
        max_num_params (int): Largest number of parameters to check for operators
    ...
    - ``"comment"``: Comments regarding certain properties of the tested operator(s).
      These does not require any action to change the operator but is used to raise awareness
      for behaviour that might be unexpected or differing from common operators.

    - ``"hint"``: Remarks similar to warnings that indicate concrete hints to change the checked
      operator(s). The recommended changes are expected to improve the code quality, performance
      or consistency with other operators.

    - ``"error"``: Problems with the checked operator(s) that require changes. These problems
      might be in the core of the ``Operator``, preventing instantiation, or in a specific
      method or property that is rendered unusable or inconsistent by the problem.

    **Call signature**

    Once instantiated, the ``OperatorChecker`` can be called on an operator instance or class
    with the following signature:

    Args:

      - op (type or .Operator): Operator(s) to check, either provided as class
        or as instance.

      - parameters (Sequence[int or float]): Parameters with which the operator(s) is/are
        expected to work. Ignored if ``op`` is not a type but a class instance. If not
        provided, random parameters are used.

      - wires (.wires.Wires): Wires with which the operator(s) is/are expected to work.
    ...
    Returns:
      ...
      - str: The result status of the operator check, one of ``"error"``, ``"hint"``,
        ``"comment"``, and ``"pass"``.
      - str: A copy of the text printed via the ``print_fn`` during the check.

    **Example**

    Let's consider a custom operator, which is a copy of ``qml.RX`` but with its
    ...
    Now we can run the OperatorChecker on this operator class:

    >>> checker = qml.debugging.OperatorChecker()
    >>> result, output = checker(MyRX)
    Checking operator MyRX for consistency.
    = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = =
    ...
    .. UsageDetails::
        ...
        **Retrieving previous check results**
        ...
        An instance of ``OperatorChecker`` is stateful, allowing us to retrieve the results
        of checks that were performed earlier:

        >>> checker = qml.debugging.OperatorChecker(verbosity="error")
        ...
        **Using a custom print function**
        ...
        >>> checker = qml.debugging.OperatorChecker(print_fn=print_to_file)
        >>> checker(qml.RX)
        >>> with open("check_output.txt", "r") as f:
        ...     for line in f.readlines():
        ...         print(line)
        Checking operator RX for consistency.
        = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = =
        No problems have been found with the operator RX.
    """
    ...
    def __call__(self, op, parameters=None, wires=None, seed=None):
        r"""Call the OperatorChecker on one or multiple operators."""
        self.seed = seed
        # Initialize result status and output for this operator
        self.results[op] = "pass"
        self.outputs[op] = ""
        # Temporary storage
        self.tmp = {
            "printed_header": False,  # Header for this op has not been printed yet
            "op": op,  # Currently investigated operator
            "res": verbosity_levels["pass"],  # Current result status
            "name": op.name if isinstance(op, qml.operation.Operator) else op.__name__,
        }
        try:
            self.check_single_operator(op, parameters, wires)
        except CheckerError:
            pass

        # Store the result for this operator outside of tmp and print summary
        self.results[op] = levels_verbosity[self.tmp["res"]]
        if self.results[op] == "pass":
            self.print_(
                f"No problems have been found with the operator {self.tmp['name']}.\n", "pass"
            )
    ...
    def print_(self, string, level=None):
        ...
        and increment the result status for the currently checked operator if necessary.
        ...
        a given operator (and the verbosity levels actually allow for an output).
        """
        ...
        if level == "error" or level in self._verbosity:
            if not self.tmp["printed_header"]:
                header = f"Checking operator {self.tmp['name']} for consistency.\n" + "= " * 40
    ...
    def check_single_operator(self, op, parameters, wires):
        """Check one operator subclass to define all required properties,
        be well-defined, and have consistent properties.

        Args:
            op (type): Operator to check, may be a class or and instance.
            parameters (Sequence[int or float]): Parameters with which the operator(s)
                is/are expected to work.
            wires (.wires.Wires): Wires with which the operator(s) is/are expected to work.
        """
        if self.seed is not None:
            qml.math.random.seed(self.seed)
        # Retrieve parameters and wires if op is operator instance instead of class
    ...
    def _check_wires(self, op, wires):
        ...
        if isinstance(op.num_wires, property):
            self.print_(
                f"The operator {self.tmp['name']} does not define the number of wires it acts on.",
                "fatal_error",
            )
        ...
        if wires is None:
            if op.num_wires != AnyWires:
                ...
                # Use a dummy case of 2 wires for operators with flexible number of wires
            ...
            if op.num_wires != AnyWires and len(wires) != op.num_wires:
                self.print_(
                    ...
                    f"number ({op.num_wires}) for operator {self.tmp['name']}",
                    "fatal_error",
                )
                raise CheckerError("Fatal error: Subsequent checks will not be possible.")
    ...
    def _check_parameters(self, op, parameters):
        ...
        if parameters is None:
            ...
            self.print_(
                ...
                f"the expected number ({op.num_params}) for operator {self.tmp['name']}",
                "fatal_error",
            )
            raise CheckerError("Fatal error: Subsequent checks will not be possible.")
        else:
    ...
    def _check_instantiation(self, op, parameters, wires):
        """Check whether instantiation of an operator works, either
        ...
        if len(possible_num_params) == 1 and len(parameters) > 1:
            ...
            if len(parameters) == 1:
                err_str += (
                    ...
                    "for this operator,\ncheck the input to check_operator."
                )
    ...
    def _check_single_method(self, op, method_tuple, parameters, wires):
        """Check whether a specific method of an operator works with
        ...
        if self.tmp["num_params_known"]:
            ...
            # for in the hyperparameters of the operator.
            try:
                instance = op(*par, wires=wires)
                getattr(instance, method.replace("compute", "get"))()
                self.print_(exc, "comment")
                self.print_(
                    f"Operator method {self.tmp['name']}.{method} does not work\n"
        ...
        if succeeding_nums:
            self.print_(
                f"Operator method {self.tmp['name']}.{method} works\n"
    ...
    def _check_properties(self, op, parameters, wires):
        ...
        of additional properties that are given by attributes of the operator."""
        for par in parameters:
            instance = op(*par, wires=wires)
            ...
            # Check that the diagonalizing gates diagonalize the operator matrix
    ...
    def _check_matrix_shape(self, matrix, op):
        """Check that a matrix attributed to an operator has the correct shape."""
        if matrix is None:
            return
        if not matrix.shape[0] == matrix.shape[1]:
            self.print_(
                f"The operator {self.tmp['name']} defines a non-square matrix.", "fatal_error"
            )
            raise CheckerError("Fatal error: Subsequent checks will not be possible.")
        mat_num_wires = int(qml.math.log2(matrix.shape[0]))
        if not mat_num_wires == op.num_wires and op.num_wires != AnyWires:
            self.print_(
                f"The operator {self.tmp['name']} defines a matrix for {mat_num_wires} wires but "
    ...
    def _check_eigvals(self, eigvals, matrix):
        """Check that produced eigvals for an operator coincide with the
        eigvals of a matrix representation of the same operator."""
        if matrix is None or eigvals is None:
            return
        mat_eigvals = qml.math.sort(qml.math.round(qml.math.linalg.eigvals(matrix), 14))
        if not qml.math.allclose(mat_eigvals, qml.math.sort(qml.math.round(eigvals, 14))):
            self.print_(
    ...
    def _check_diag_gates(self, diag_gates, matrix, eigvals):
        ...
        if not is_diagonal(diagonalized, atol=self.tol):
            ...
            return
        if eigvals is not None and not qml.math.allclose(
            qml.math.sort(qml.math.round(eigvals, 14)),
            qml.math.sort(qml.math.round(qml.math.diag(diagonalized), 14)),
        ):
            self.print_(
    ...
    def _check_basis(self, matrix, instance):
        """Check that a matrix attributed to an operator is diagonal in the basis
        indicated by that operator's ``basis`` property."""
        try:
        ...
        if not is_diagonal(diag_mat @ matrix @ diag_mat.conj().T, atol=self.tol):
            self.print_(
                f"The operator {self.tmp['name']} is not diagonal in the provided basis",
                "error",
            )

    def _check_derivatives(self, op, parameters, wires):
        """Check that the matrix representation of an operator obtained via qml.matrix
    ...
    def _check_parameter_shift(self, op, parameters, wires):
        """Check that an operator is differentiable if it is marked to be, and that
        ...
        if self.tmp["num_params_known"] and op.num_params != 1:
            # Operator has more than one parameter. Skip it
            return
        ...
        for par in parameters:
            ...
            try:
                ...
                std_recipe = qml.math.array([coeffs, qml.math.ones_like(coeffs), shifts])
            except qml.operation.ParameterFrequenciesUndefinedError:
                std_recipe = None
            recipe = qml.math.array(op.grad_recipe[0]).T if has_grad_recipe else std_recipe
            ...
            if has_grad_recipe and qml.math.allclose(recipe, std_recipe):
                self.print_(
                    ...
                    f"to the operator instead in order to allow for flexible shift values.",
                    "hint",
                )
