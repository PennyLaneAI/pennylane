...
class PauliVSpace:
    ...
    def add(self, other, tol=None):
        ...
        new_pws = list(reduce(set.__or__, [set(ps.keys()) for ps in other]))

        new_pws = [pw for pw in new_pws if pw not in pw_to_idx.keys()]
        new_num_pw = num_pw + len(new_pws)
        ...
        # Add new PauliSentence entries to matrix
        for i, pauli_sentence in enumerate(other):
            for pw, value in pauli_sentence.items():
