...
class QuantumScript:
    ...
    **Example:**
        ...
        **Examples**
        ...
        with QueuingManager.stop_recording():
            for observable in _get_base_obs(self.observables):
                ...
                with contextlib.suppress(DiagGatesUndefinedError):
        ...
        **Example**
        ...
        for op_idx, p_indices in op_indices.items():
            ...
            if isinstance(op, Operator):
                ...
                new_obs = bind_new_parameters(op.obs, new_params)
    ...
    def adjoint(self) -> "QuantumScript":
        ...
        with QueuingManager.stop_recording():
            ops_adj = [adjoint(op, lazy=False) for op in reversed(ops)]
    ...
    @property
    def graph(self) -> "pennylane.CircuitGraph":
    ...
    @classmethod
    def from_queue(cls: type[QS], queue: AnnotatedQueue, shots: ShotsLike | None = None) -> QS:
    ...
    def map_to_standard_wires(self) -> "QuantumScript":
        ...
        **Example:**
        ...
        [MultiControlledX(wires=[0, 1, 2, 3], control_values=[ True  True  True], work_wires=[6, 5], work_wire_type=borrowed), probs(wires=[3, 4, 5])]
        ...
        if wire_map is None:
            return self
        new_ops = [op.map_wires(wire_map) for op in self.operations]
        new_mps = [mp.map_wires(wire_map) for mp in self.measurements]
        return self.copy(operations=new_ops, measurements=new_mps)
...
def _get_base_obs(observables):
    ...
    while any(isinstance(o, (CompositeOp, SymbolicOp)) for o in observables):
        ...
        for observable in observables:
            ...
            elif isinstance(observable, SymbolicOp):
