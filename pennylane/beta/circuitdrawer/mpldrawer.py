...
class MPLDrawer:
    ...
    def __init__(self, n_layers, n_wires, figsize=None):
        ...
        self.ax = self.fig.add_axes([0,0,1,1],
                    xlim=(-2,self.n_layers+1), ylim=(-1,self.n_wires),
                    **self.axes_kwargs
                    )
    ...
    def wires(self, n_wires=None, n_layers=None):
        ...
        for wire in range(n_wires): 
            line = plt.Line2D((-1, n_layers), (wire, wire),
                            zorder=1, **self.wire_kwargs)
    ...
    def box_gate(self, layer, wires, text="", extra_width=0, rotate_text=False, zorder_base=0, color=None):
        ...
        box = plt.Rectangle((layer-self.box_dx-extra_width/2, box_min-self.box_dx),
                            2*self.box_dx+extra_width, (box_len+2*self.box_dx), zorder=2+zorder_base,
                            **temp_kwargs
                        )
        self.ax.add_patch(box)
        self.ax.text(layer, box_center, text, zorder=3+zorder_base,
            rotation=rotation, **self.text_kwargs)
    ...
    def ctrl(self, layer, wire_ctrl, wire_target=tuple()):
        ...
        line = plt.Line2D((layer, layer), (min_wire, max_wire), 
                        zorder=2,
                        **self.ctrl_line_kwargs)
        self.ax.add_line(line)

        for wire in wire_ctrl:
            circ_ctrl = plt.Circle((layer, wire), radius=self.ctrl_rad,
                    zorder=2, **self.ctrl_kwargs)
    ...
    def CNOT(self, layer, wires):
        ...
        self.target_x(layer, target)

        
    def target_x(self, layer, wire):
        ...
        """
        target_circ = plt.Circle((layer, wire), radius=self.circ_rad,
                            zorder=3,
                            **self.targetx_kwargs)
        target_v = plt.Line2D((layer, layer),
    ...
    def SWAP(self, layer, wires):
        ...
        """
        line = plt.Line2D((layer, layer), wires,
                        zorder=2, **self.swapx_kwargs)
        self.ax.add_line(line)
    ...
    def _swap_x(self, layer, wire):
        ...
        Args:
            ...
            wire (Int): the wire
        """
        l1 = plt.Line2D((layer-self.swap_dx, layer+self.swap_dx),
                        (wire-self.swap_dx, wire+self.swap_dx), 
                        zorder=2, **self.swapx_kwargs)
        l2 = plt.Line2D((layer-self.swap_dx, layer+self.swap_dx),
                        (wire+self.swap_dx, wire-self.swap_dx), 
                        zorder=2, **self.swapx_kwargs)
    ...
    def measure(self, layer, wire, zorder_base=0):
        ...
        box = plt.Rectangle((layer-self.box_dx, wire-self.box_dx), 2*self.box_dx, 2*self.box_dx,
                           zorder = 2+zorder_base)
        self.ax.add_patch(box)
        
                
        arc = patches.Arc((layer,wire -self.box_dx/8), 1.2*self.box_dx, 1.1*self.box_dx,
                            theta1 = 0, theta2=180, zorder=3+zorder_base)
        ...
        arrow = plt.arrow(arrow_start_x, arrow_start_y, arrow_width, arrow_height,
                          head_width=self.box_dx/4, zorder=4+zorder_base)
