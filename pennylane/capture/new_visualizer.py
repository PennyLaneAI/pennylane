...
class PlxprVisualizerNew(PlxprInterpreter):
    ...
    def __copy__(self):
        copied_visualizer = PlxprVisualizerNew.__new__(PlxprVisualizerNew)
        for attr, value in vars(self).items():
            if attr not in (
                "graph",
                "unique_id_counter",
                "dyn_vars",
                "map_wire_to_node_uid",
                "last_seen_dynamic_wire",
                "current_cluster",
                "map_operator_outvar_to_mp_invar",
            ):
                setattr(copied_visualizer, attr, value)

        copied_visualizer.graph = copy(self.graph)
        copied_visualizer.unique_id_counter = self.unique_id_counter
        copied_visualizer.dyn_vars = self.dyn_vars.copy()
        copied_visualizer.map_wire_to_node_uid = self.map_wire_to_node_uid.copy()
        copied_visualizer.last_seen_dynamic_wire = self.last_seen_dynamic_wire
        copied_visualizer.current_cluster = self.current_cluster
        copied_visualizer.map_operator_outvar_to_mp_invar = (
            self.map_operator_outvar_to_mp_invar.copy()
        )

        return copied_visualizer
    ...
    def interpret_operation_eqn(self, eqn):
        ...
        if isinstance(eqn.outvars[0], jax.core.DropVar):
            ...
            operator_node_uid = self.convert_name_to_uid(op.name)
            ...
            op_wires_ascii = self.convert_wires_to_ascii(real_wires_with_id)
            ...
            for wire, wire_ascii in zip(op.wires, op_wires_ascii, strict=True):
                ...
                if qml.math.is_abstract(wire):
                    ...
                    for _, seen_node_uid in self.map_wire_to_node_uid.items():
                        if not self.graph.get_edge(seen_node_uid, operator_node_uid):
                    ...
                    self.map_wire_to_node_uid.clear()
                    self.map_wire_to_node_uid[wire_ascii] = operator_node_uid
                    continue

                print(self.map_wire_to_node_uid)
                last_dynamic_node_uid = self.map_wire_to_node_uid.get(
                    self.last_seen_dynamic_wire, None
                )
                previous_node_uid_on_this_wire = self.map_wire_to_node_uid.get(
                ...
                self.map_wire_to_node_uid[str(wire)] = operator_node_uid
        ...
        for outvar in eqn.outvars:
            self.map_operator_outvar_to_mp_invar[str(outvar)] = {
    ...
    def interpret_measurement_eqn(self, eqn):
        ...
        print(eqn.outvars, eqn.invars, eqn.params)
        measurement_node_uid = self.convert_name_to_uid(str(mp._shortname))

        real_wires = self.map_operator_outvar_to_mp_invar.get(str(eqn.invars[0]))["wires"]
        print("Real wires for measurement: ", real_wires)
        mp_wires_ascii = self.convert_wires_to_ascii(real_wires)
        self.current_cluster.add_node(
            pydot.Node(
        ...
        for wire, wire_ascii in zip(mp.obs.wires, mp_wires_ascii, strict=True):
            ...
            if qml.math.is_abstract(wire):
                ...
                # Detected a dynamic wire, need to connect all seen nodes to this node
                for _, seen_node_uid in self.map_wire_to_node_uid.items():
                    if not self.graph.get_edge(seen_node_uid, measurement_node_uid):
                        ...
                        self.graph.add_edge(edge)
                # self.map_wire_to_node_uid.clear()
                # self.map_wire_to_node_uid[wire_ascii] = measurement_node_uid
                continue

            print("Inside measurement and mapping dict: ", self.map_wire_to_node_uid)
            print("Dynamic wire seen: ", self.dyn_vars)
            last_dynamic_node_uid = self.map_wire_to_node_uid.get(self.last_seen_dynamic_wire, None)
            previous_node_uid_on_this_wire = self.map_wire_to_node_uid.get(
                str(wire), last_dynamic_node_uid
            )
        ...
        # Map the wire to the node UID
        # self.map_wire_to_node_uid[str(wire)] = measurement_node_uid
...
def handle_qnode(self, *invals, shots, qnode, device, execution_config, qfunc_jaxpr, n_consts):
    ...
    self.current_cluster.add_node(wires_node)
    for wire in device.wires:
        self.map_wire_to_node_uid[str(wire)] = wires_node_uid
    ...
    self.current_cluster.add_subgraph(qnode_cluster)

    copy_ = copy(self)
    copy_.current_cluster = qnode_cluster
...
def handle_for_loop(
    ...
    # Visualization part
    for_loop_uid = self.convert_name_to_uid("for_loop")
    loop_var = jaxpr_body_fn.invars
    print(consts)
    loop_var_ascii = self.convert_wires_to_ascii(loop_var)
    print(self._env)
    ...
    self.current_cluster.add_subgraph(for_loop_cluster)

    copy_ = copy(self)
    copy_.current_cluster = for_loop_cluster
    new_jaxpr_body_fn = jaxpr_to_jaxpr(
        copy_, jaxpr_body_fn, consts, *abstract_shapes, start, *init_state
    )

    self.graph = copy(copy_.graph)
    self.unique_id_counter = copy_.unique_id_counter
    self.dyn_vars = copy_.dyn_vars.copy()
    self.map_wire_to_node_uid = copy_.map_wire_to_node_uid.copy()
    self.last_seen_dynamic_wire = copy_.last_seen_dynamic_wire
    self.map_operator_outvar_to_mp_invar = copy_.map_operator_outvar_to_mp_invar.copy()
...
def handle_while_loop(
    ...
    *invals,
    ...
    self.current_cluster.add_subgraph(while_loop_cluster)

    copy_ = copy(self)
    copy_.current_cluster = while_loop_cluster
    new_jaxpr_body_fn = jaxpr_to_jaxpr(copy_, jaxpr_body_fn, consts_body, *init_state)
    copy_of_copy = copy(copy_)

    copy_of_copy.graph = copy(copy_.graph)
    copy_of_copy.unique_id_counter = copy_.unique_id_counter
    copy_of_copy.dyn_vars = copy_.dyn_vars.copy()
    copy_of_copy.map_wire_to_node_uid = copy_.map_wire_to_node_uid.copy()
    copy_of_copy.last_seen_dynamic_wire = copy_.last_seen_dynamic_wire
    ...
    self.dyn_vars = copy_of_copy.dyn_vars.copy()
...
def handle_cond(self, *invals, jaxpr_branches, consts_slices, args_slice):
    ...
    self.current_cluster.add_subgraph(cond_cluster)
    ...
    for const_slice, jaxpr in zip(consts_slices, jaxpr_branches):
        ...
        if jaxpr is None:
            ...
            branch_uid = self.convert_name_to_uid(f"branch_{branch_count}")
            ...
            copy_ = copy(self)
            copy_.current_cluster = branch_cluster
