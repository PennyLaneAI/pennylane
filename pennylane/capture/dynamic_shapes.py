...
def register_custom_staging_rule(
    ...
    create_initial_env: Callable[[dict, list], dict] | None = None,
) -> None:
    ...
    Args:
        ...
        create_initial_env (Callable[[dict, list], dict] | None): An optional function that takes in
            the equation's ``params`` and the primitive's input variables, and returns a dictionary
            mapping inner jaxpr variables to outer jaxpr variables. This is needed when output shapes
            reference variables from the inner jaxpr that correspond to input variables.
    ...
    ``create_initial_env`` is used when the output shapes contain variables from the inner jaxpr
    that correspond to input variables (e.g., the ``transform`` primitive with dynamic shots).
    """
    ...
    def custom_staging_rule(jaxpr_trace, source_info, *tracers, **params):
        ...
        # Start with initial env mapping inner→outer vars (if provided)
        env: dict[jax.extend.core.Var, jax.extend.core.Var] = {}
        if create_initial_env is not None:
