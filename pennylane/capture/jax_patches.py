...
try:
    ...
    if version.parse(jax_version) >= version.parse("0.7.0") and version.parse(
        ...
        from pennyjax._src import config as jax_config
        from pennyjax._src import core, pjit, source_info_util
        from pennyjax._src.core import JaxprEqnContext, Var
        from pennyjax._src.interpreters import partial_eval as pe
        from pennyjax._src.interpreters.partial_eval import (
            DynamicJaxprTracer,
            TracingEqn,
            compute_on,
            xla_metadata_lib,
        )
        from pennyjax._src.lax import lax
...
def _patch_pjit_infer_params():
    ...
    # in case infinite recursion
    original_infer_params_internal = pjit._infer_params_internal
    ...
    def patched_infer_params_internal(fun, ji, args, kwargs):
        ...
        if not jax_config.dynamic_shapes.value:  # pragma: no cover
            # Then we just fallback to the original jax pjit
            return original_infer_params_internal(fun, ji, args, kwargs)
