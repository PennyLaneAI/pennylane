...
TracingEqn objects, which JAX 0.7.1 requires but doesn't provide in all code paths.
...
def _add_make_eqn_helper():
    ...
    This helper properly creates TracingEqn objects, which is needed for JAX 0.7.1
...
def _patch_dyn_shape_staging_rule():
    ...
    The bug in JAX 0.7.1's lax/lax.py lines 267-275 is that it uses:
...
def _patch_pjit_staging_rule():
    ...
    The bug in JAX 0.7.1's pjit.py lines 1894-1898 is that it uses:
