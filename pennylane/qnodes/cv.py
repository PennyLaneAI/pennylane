...
class CVQNode(JacobianQNode):
    ...
    def _pd_analytic(self, idx, args, kwargs, **options):
        ...
        for op, p_idx in self.variable_deps[idx]:
            ...
            pos_multiplier, pos_shift, neg_multiplier, neg_shift = op.get_parameter_shift(p_idx)

            # shifted parameter values
            shift_p1 = np.r_[args, args[idx] + pos_shift]
            shift_p2 = np.r_[args, args[idx] - neg_shift]
            ...
            if not force_order2 and op.use_method != "B":
                ...
                Z0 = op.heisenberg_tr(self.device.wires, inverse=True)
                Z = Z @ Z0
                ...
                for BB in self._op_descendants(op, "G"):
                    if not BB.supports_heisenberg:
                        ...
                        continue
                    B = BB.heisenberg_tr(self.device.wires) @ B
                    B_inv = B_inv @ BB.heisenberg_tr(self.device.wires, inverse=True)
                Z = B @ Z @ B_inv  # conjugation

                # transform the descendant observables into their derivatives using Z
                desc = self._op_descendants(op, "O")
                obs = [self._transform_observable(x, w, Z, self.device.wires) for x in desc]
