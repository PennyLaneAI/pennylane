...
class RydbergSettings:
    ...
    def __add__(self, other):
        if other is not None:
            raise ValueError(
                "Cannot add two ``HardwareHamiltonian`` instaces with an interaction term"
                f"Obtained tow instances with settings {self} and {other}"
                "You most likely try to add two terms generated by help(qml.pulse.rydberg_interaction)"
            )
