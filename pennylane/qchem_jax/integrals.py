...
def primitive_norm(l, alpha):
    ...
    Args:
        l (tuple[int]): Angular momentum quantum number of the basis function.
                        Note that the angular momentum variable is not differentiable. Any JIT of
                        functions should mark it so with `jax.jit(..., static_argnums=0)`
    ...
    **Example**
    ...
    n = (
        (2 * alpha / jnp.pi) ** 0.75
        * (4 * alpha) ** (sum(l) / 2)
        / jnp.sqrt(fac2(2 * lx - 1) * fac2(2 * ly - 1) * fac2(2 * lz - 1))
    )
    return n
...
def contracted_norm(l, alpha, a):
    ...
    **Example**
    ...
    """
    lx, ly, lz = l
    c = jnp.pi**1.5 / 2 ** sum(l) * fac2(2 * lx - 1) * fac2(2 * ly - 1) * fac2(2 * lz - 1)
    s = (
...
def expansion(la, lb, ra, rb, alpha, beta, t):
    ...
    **Example**
    ...
    q = jnp.array(alpha * beta / p)
    r = jnp.array(ra - rb)

    if la == lb == t == 0:
        return jnp.exp(-q * r**2)
...
def _diff2(i, j, ri, rj, alpha, beta):
    ...
    d1 = j * (j - 1) * jnp.sqrt(jnp.pi / p) * expansion(i, j - 2, ri, rj, alpha, beta, 0)
    d2 = -2 * beta * (2 * j + 1) * jnp.sqrt(jnp.pi / p) * expansion(i, j, ri, rj, alpha, beta, 0)
    d3 = 4 * beta**2 * jnp.sqrt(jnp.pi / p) * expansion(i, j + 2, ri, rj, alpha, beta, 0)
...
def _boys(n, t):
    ...
    return jnp.where(
        t == 0.0,
        1 / (2 * n + 1),
        jsp.special.gammainc(n + 0.5, t + (t == 0.0))
        # TODO: verify directly if the following two definitions are equivalent
        # * asp.special.gamma(n + 0.5)
        * jnp.exp(jsp.special.gammaln(n + 0.5)) / (2 * (t + (t == 0.0)) ** (n + 0.5)),
...
def nuclear_attraction(la, lb, ra, rb, alpha, beta, r):
    ...
    l2, m2, n2 = lb
    p = alpha + beta
    rgp = (alpha * ra[:, jnp.newaxis, jnp.newaxis] + beta * rb[:, jnp.newaxis, jnp.newaxis]) / (
        alpha + beta
    )
    dr = rgp - jnp.array(r)[:, jnp.newaxis, jnp.newaxis]
...
def electron_repulsion(la, lb, lc, ld, ra, rb, rc, rd, alpha, beta, gamma, delta):
    ...
    p_ab = (
        alpha * ra[:, jnp.newaxis, jnp.newaxis, jnp.newaxis, jnp.newaxis]
        + beta * rb[:, jnp.newaxis, jnp.newaxis, jnp.newaxis, jnp.newaxis]
    ) / (alpha + beta)

    p_cd = (
        gamma * rc[:, jnp.newaxis, jnp.newaxis, jnp.newaxis, jnp.newaxis]
        + delta * rd[:, jnp.newaxis, jnp.newaxis, jnp.newaxis, jnp.newaxis]
    ) / (gamma + delta)
    ...
    g = g * 2 * (jnp.pi**2.5) / (p * q * jnp.sqrt(p + q))
