...
"""This module contains a TensorFlow implementation of the :class:`~.DefaultMixed`
reference plugin.
"""
import numpy as np
...
try:
    ...
    if tf.__version__[0] == "1":
        raise ImportError("default.mixed.tf device requires TensorFlow>=2.0")
    ...
    raise ImportError("default.mixed.tf device requires TensorFlow>=2.0") from e
...
from . import DefaultMixed


class DefaultMixedTF(DefaultMixed):
    """Simulator plugin based on ``"default.mixed"``, written using TensorFlow.

    **Short name:** ``default.mixed.tf``

    This device provides a mixed-state qubit simulator written using TensorFlow.
    ...
    **Example**
    ...
    The ``default.mixed.tf`` is designed to be used with end-to-end classical backpropagation
    ...
    >>> dev = qml.device("default.mixed.tf", wires=1)
    ...
    * Only exact expectation values, variances, and probabilities are
    ...
    consider using the ``default.mixed`` device instead.
    ...
    def _create_basis_state(self, index):
        ...
        rho[index, index] = 1
        rho = tf.convert_to_tensor(rho, dtype=self.C_DTYPE)
