...
class DefaultMixedAutograd(DefaultMixed):
    """Simulator plugin based on ``"default.mixed"``, written using Autograd.

    **Short name:** ``default.mixed.autograd``
    ...
    **Example**
    ...
    The ``default.mixed.autograd`` is designed to be used with end-to-end classical backpropagation
    ...
    * Only exact expectation values, variances, and probabilities are differentiable.
    ...
    R_DTYPE = np.float64
    _dot = staticmethod(anp.dot)
    _abs = staticmethod(anp.abs)
    _reduce_sum = staticmethod(lambda array, axes: qnp.sum(array, axis=tuple(axes)))
    _reshape = staticmethod(anp.reshape)
    _flatten = staticmethod(lambda array: array.flatten())
    _gather = staticmethod(lambda array, indices: array[indices])
    _einsum = staticmethod(anp.einsum)
    _cast = staticmethod(lambda array, dtype: ar.astype(array, dtype_name=dtype))
    _transpose = staticmethod(anp.transpose)
    _tensordot = staticmethod(anp.tensordot)
    _conj = staticmethod(anp.conj)
    _imag = staticmethod(anp.imag)
    _roll = staticmethod(anp.roll)
    _stack = staticmethod(anp.stack)

    @classmethod
    def _asarray(cls, array, dtype=None):
        if dtype is None and array is list:
            dtype = array[0].dtype
        if array is list:
            shape = [len(array)] + array[0].shape
            res = qnp.asarray(ar.reshape(array, shape), dtype=dtype)
        else:
    ...
    def _scatter(indices, array, new_dimensions):
        new_array = qnp.zeros(new_dimensions, dtype=array.dtype.type)
