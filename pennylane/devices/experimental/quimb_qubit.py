...
from ..quimb.simulate import simulate
from ..quimb.preprocess import preprocess, validate_and_expand_adjoint
from ..quimb.adjoint_jacobian import adjoint_jacobian
...
class QuimbQubit(Device):
    """A PennyLane device written in Python and using Quimb as a backend to story and
    handle data as tensor networks.
    ...
    **Example:**
    ...
    def supports_derivatives(
        ...
        ``QuimbQubit`` supports backpropagation derivatives with analytic results, as well as
    ...
    def execute(
        ...
        if self.tracker.active:
            ...
            self.tracker.record()
        results = tuple(
            simulate(c, rng=self._rng.integers(low=0, high=2**31 - 1)) for c in circuits
        )
