...
def simulate(
    ...
    **execution_kwargs,
    ...
    >>> qs = QuantumScript([qml.RX(1.2, wires=0)], [qml.expval(qml.Z(0)), qml.probs(wires=(0,1))])
    >>> simulate(qs)
    (0.36235775447667357,
    ...
    if has_mcm:
        ...
        if math.get_deep_interface(circuit.data) == "jax" and prng_key is not None:
            ...
            num_valid_shots = 0
            num_mcms = sum(1 for op in circuit.operations if isinstance(op, MidMeasureMP))
            while num_valid_shots < circuit.shots.total_shots:
                result = simulate_one_shot_native_mcm(
                    aux_circ,
                    debugger=debugger,
                    prng_key=keys[num_valid_shots],
                    **execution_kwargs,
                )
                if not any(math.isnan(res) for res in result[-num_mcms:]):
...
def prepend_state_prep(circuit, state, interface, wires):
    ...
    new_ops = [
        ops.StatePrep(math.ravel(state), wires=wires, validate_norm=False)
    ] + circuit.operations
...
def branch_state(state, branch, mcm):
    ...
    if isinstance(state, np.ndarray):
        ...
        # SLOWER
        state = apply_operation(ops.Projector([branch], mcm.wires), state)
        state = state / math.norm(state)

    if mcm.reset and branch == 1:
        state = apply_operation(ops.PauliX(mcm.wires), state)
    return state
