...
class DefaultQubitInterpreter(FlattenedInterpreter):
    ...
    Args:
        ...
        key (None, qpjax.numpy.ndarray): the ``PRNGKey`` to use for random number generation.
    ...
    >>> qpjax.grad(dq(f))(qpjax.numpy.array(0.5))
    Array(-1.68294197, dtype=float64, weak_type=True)
    >>> qpjax.jit(dq(f))(qpjax.numpy.array(0.5))
    Array(0.54030231, dtype=float64)
    """
    ...
    def __init__(
        ...
        num_wires: int,
        shots: int | None = None,
        key: None | qpjax.numpy.ndarray = None,
        ...
        if key is None:
            key = qpjax.random.PRNGKey(np.random.randint(100000))
    ...
    def interpret_operation(self, op):
        ...
        if isinstance(op, Projector):
            self.key, new_key = qpjax.random.split(self.key, 2)
...
def _(self, *invals, reset, postselect):
    ...
    if mp.postselect is not None:
        ...
        self.state = self.state / (1 - qpjax.numpy.abs(mp.postselect - mcms[mp]))
    return mcms[mp]
