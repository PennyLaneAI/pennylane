...
def apply_mid_measure(
    ...
    is_valid = op.postselect is None or sample == op.postselect
    mid_measurements[op] = sample if is_valid else np.nan
...
def apply_snapshot(
    ...
    if shots:
        snapshot = qubit.measure_with_samples(
            [measurement],
            state,
            shots,
            is_state_batched,
        ...
        snapshot = qubit.measure(measurement, state, is_state_batched)
...
def apply_parametrized_evolution(
    op: pulse.ParametrizedEvolution,
    state,
    is_state_batched: bool = False,
    debugger=None,
    **_,
...
def _evolve_state_vector_under_parametrized_evolution(
    operation: pulse.ParametrizedEvolution, state, num_wires, is_state_batched
):
