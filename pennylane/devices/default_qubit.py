...
class DefaultQubit(QubitDevice):
    ...
    def __init__(self, wires, *, shots=None, analytic=None, dtype=np.float64):
        super().__init__(wires, shots, analytic=analytic, dtype=dtype)
        self._debugger = None
    ...
    def _apply_operation(self, state, operation):
        ...
        matrix = self._get_unitary_matrix(operation).astype(self.C_DTYPE, copy=False)
    ...
    def _apply_hadamard(self, state, axes, **kwargs):
        ...
        return np.multiply(SQRT2INV, state_x + state_z, dtype=state.dtype)
    ...
    def _apply_phase(self, state, axes, parameters, inverse=False):
        ...
        return self._stack([state[sl_0], np.multiply(state[sl_1], phase, dtype=state.dtype)], axis=axes[0])
