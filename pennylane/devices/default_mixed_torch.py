...
"""This module contains a PyTorch implementation of the :class:`~.DefaultMixed`
reference plugin.
"""
...
try:
    ...
    raise ImportError("default.mixed.torch device requires Torch>=1.8.1") from e
...
class DefaultMixedTorch(DefaultMixed):
    """Simulator plugin based on ``"default.mixed"``, written using PyTorch.

    **Short name:** ``default.mixed.torch``

    This device provides a mixed-state qubit simulator written using PyTorch.
    ...
    **Example**
    ...
    The ``default.mixed.torch`` is designed to be used with end-to-end classical backpropagation
    ...
    .. code-block:: python
        ...
        dev = qml.device("default.mixed.torch", wires=1)
    ...
    .. code-block:: python
        ...
        dev = qml.device("default.mixed.torch", wires=1, torch_device='cuda')
    ...
    * Only exact expectation values, variances, and probabilities are differentiable.
    ...
    _transpose = staticmethod(lambda a, axes=[1, 0]: a.permute(*axes))
    ...
    def _create_basis_state(self, index):
        ...
        """
        rho = torch.zeros((2 ** self.num_wires, 2 ** self.num_wires), dtype=self.C_DTYPE)
        rho[index, index] = 1
