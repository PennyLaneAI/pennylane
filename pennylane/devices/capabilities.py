...
class DeviceCapabilities:  # pylint: disable=too-many-instance-attributes
    ...
    Attributes:
        ...
        supported_mcm_methods (list[str]): List of mid-circuit measurement methods supported by the
            backend device. This may include "one-shot", "tree-traversal", etc. An empty list indicates
            that the device does not support mid-circuit measurements.
        options (dict[str, any]): Additional device-specific options.
    """
...
def _get_compilation_options(document: dict, prefix: str = "") -> dict:
    """Gets the boolean capabilities in the compilation section."""

    section = _get_toml_section(document, "compilation", prefix)
    if unknowns := set(section) - VALID_COMPILATION_OPTIONS.keys():
        raise InvalidCapabilitiesError(
            f"The compilation section has unknown options: {list(unknowns)}"
        )
    flags = {
        flag: section.get(flag, default) for flag, default in VALID_COMPILATION_OPTIONS.items()
    }
...
def update_device_capabilities(
    ...
    compilation_flags = _get_compilation_options(document, runtime_interface)
