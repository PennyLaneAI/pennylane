...
def flatten_np(x):
    ...
    if isinstance(x, np.ndarray):
        yield from flatten_np(x.flat)  # should we allow object arrays? or just "yield from x.flat"?
...
def unflatten_np(flat, model):
    ...
    res, tail = _unflatten_np(np.asarray(flat), model)
    if len(tail) != 0:
