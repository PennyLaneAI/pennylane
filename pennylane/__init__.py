...
if _find_spec("jax") is not None:
    if (jax_version := _Version(_metadata_version("jax"))) > _Version("0.7.2"):  # pragma: no cover
        warnings.warn(
            "PennyLane is not yet compatible with JAX versions > 0.7.2. "
            f"You have version {jax_version} installed. "
            "Please downgrade JAX to 0.7.2 to avoid runtime errors using "
            "python -m pip install jax~=0.7.2 jaxlib~=0.7.2",
            RuntimeWarning,
        )
