...
For example, ``qp.RZ`` is a composable rotation. Applying ``qp.RZ(0.1,
wires=0)`` followed by ``qp.RZ(0.2, wires=0)`` is equivalent to performing
a single rotation ``qp.RZ(0.3, wires=0)``.

An example for an alternative accumulation is the ``qp.Rot`` gate: although the three
...
For example, the generator of ``qp.RZ`` is Pauli :math:`Z` with a prefactor of
:math:`-1/2`, and Pauli :math:`Z` is unitary. Contrary, the generator of
``qp.PhaseShift`` is ``np.array([[0, 0], [0, 1]])`` with a prefactor of 1,
which is not unitary. This attribute is used for decompositions in algorithms
using the Hadamard test like ``qp.metric_tensor`` when used without
approximation.
"""
...
For example, ``qp.CZ(wires=[0, 1])`` has the same effect as ``qp.CZ(wires=[1,
...
For example, ``qp.Toffoli(wires=[0, 1, 2])`` has the same effect as
``qp.Toffoli(wires=[1, 0, 2])``, but neither are the same as
``qp.Toffoli(wires=[0, 2, 1])``.
"""
