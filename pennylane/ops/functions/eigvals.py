...
def eigvals(op: qp.operation.Operator, k=1, which="SA") -> TensorLike:
    ...
    **Example**
    ...
    """
    if not isinstance(op, qp.operation.Operator):
        if not isinstance(op, (qp.tape.QuantumScript, qp.QNode)) and not callable(op):
            raise TransformError("Input is not an Operator, tape, QNode, or quantum function")
        return _eigvals_transform(op, k=k, which=which)

    if isinstance(op, qp.SparseHamiltonian):
        ...
        return qp.math.linalg.eigvalsh(sparse_matrix.toarray())

    # TODO: make `eigvals` take a `wire_order` argument to mimic `matrix`
    try:
        return op.eigvals()
    except qp.operation.EigvalsUndefinedError:
        return eigvals(qp.tape.QuantumScript(op.decomposition()), k=k, which=which)
...
def _eigvals_transform(
    ...
    def processing_fn(res):
        ...
        all_wires = qp.wires.Wires.all_wires(op_wires).tolist()
        unique_wires = qp.wires.Wires.unique_wires(op_wires).tolist()
        ...
        if len(all_wires) != len(unique_wires):
            ...
            matrix = qp.matrix(qs, wire_order=qs.wires)
            return qp.math.linalg.eigvals(matrix)
