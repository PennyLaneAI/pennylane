...
def _get_generators(trainable_op):
    ...
    # For rotation it is possible to only use PauliZ by applying some other rotations in the main function
        ...
        gen = trainable_op.generator()
        generators = [gen.base] if isinstance(gen, (qml.ops.SProd)) else gen.ops
        coeffs = [gen.scalar] if isinstance(gen, (qml.ops.SProd)) else gen.coeffs
