...
from pennylane.ops import adjoint, ctrl, prod
from pennylane.ops.functions import evolve, matrix, simplify
...
def _hadamard_test(tape, trainable_param_idx, aux_wire) -> tuple[list, list]:
    ...
    for gen in generators:
        ctrl_gen = [ctrl(gen, control=aux_wire)]
...
def _direct_hadamard_test(tape, trainable_param_idx, aux_wire) -> tuple[list, list]:
    ...
    for idx, gen in enumerate(generators):
        pos_rot = [evolve(gen, np.pi / 4)]
        neg_rot = [evolve(gen, -np.pi / 4)]
...
def _reversed_hadamard_test(tape, trainable_param_idx, aux_wire) -> tuple[list, list]:
    ...
    ops_after_trainable_op = [adjoint(op) for op in reversed(tape.operations[idx + 1 :])]
    ...
    for obs in observables:
        ctrl_obs = [ctrl(obs, control=aux_wire)]
        hadamard = [ops.H(wires=aux_wire)]
        operators = (
...
def _reversed_direct_hadamard_test(tape, trainable_param_idx, aux_wire) -> tuple[list, list]:
    ...
    for idx, obs in enumerate(observables):
        pos_rot = [evolve(obs, np.pi / 4)]
        neg_rot = [evolve(obs, -np.pi / 4)]
...
def _new_measurement(mp, aux_wire, all_wires: Wires):
    obs = mp.obs or prod(*(ops.Z(w) for w in mp.wires or all_wires))
    new_obs = simplify(obs @ ops.Y(aux_wire))
...
def _get_pauli_terms(op):
    ...
    if op.pauli_rep is None:
        mat = matrix(op, wire_order=op.wires)
    ...
    return (
        ...
        if isinstance(pauli_rep.operation(), Sum)
