...
def generate_shifted_tapes(tape, index, shifts, multipliers=None, broadcast=False):
    ...
    def _copy_and_shift_params(tape, params, idx, shift, mult):
        ...
        new_params[idx] = new_params[idx] * qml.math.cast_like(
            qml.math.convert_like(mult, new_params[idx]), new_params[idx]
        ) + qml.math.cast_like(qml.math.convert_like(shift, new_params[idx]), new_params[idx])
