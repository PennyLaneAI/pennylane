...
from pennylane.measurements import Expectation, Probability, State, Variance
...
def _grad_method(tape, idx):
    ...
    for m in tape.measurements:
        ...
        if isinstance(m, Probability) or (m.obs.ev_order not in (1, 2)):
...
def var_param_shift(tape, dev_wires, argnum=None, shifts=None, gradient_recipes=None, f0=None):
    ...
    var_mask = [isinstance(m, Variance) for m in tape.measurements]
...
def param_shift_cv(
    ...
    **Example**
    ...
    var_present = any(isinstance(m, Variance) for m in tape.measurements)
