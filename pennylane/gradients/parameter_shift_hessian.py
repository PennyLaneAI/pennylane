...
from pennylane.measurements import Probability, State, Variance
...
def _all_zero_grad_new(tape):
    ...
    for m in tape.measurements:
        shape = 2 ** len(m.wires) if isinstance(m, Probability) else ()
...
def param_shift_hessian(tape, argnum=None, diagonal_shifts=None, off_diagonal_shifts=None, f0=None):
    ...
    **Example**
    ...
    if any(isinstance(m, State) for m in tape.measurements):
        raise ValueError(
    ...
    if any(isinstance(m, Variance) for m in tape.measurements):
        raise ValueError(
