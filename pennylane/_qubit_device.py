...
def _check_adjoint_diffability(tape, device):
    """Check that only expectation values are returned by a tape, and that no Hamiltonians
    are measured. Also adds the ``base_name`` attribute to measured observables if necessary
    and warns that adjoint differentiation does not use finite shots even if the device has
    ``shots!=None``.

    This is a helper method to ``adjoint_jacobian`` and ``adjoint_hessian_diagonal`` of
...
def _get_trainable_params_wo_obs(tape):
    ...
    If parameters of an observable are found to be trainable, a warning is raised.
    This is a helper method to ``adjoint_jacobian`` and ``adjoint_hessian_diagonal`` of
    the ``QubitDevice`` class below.
    """
    trainable_params = []
...
def _check_gates_adjoint_hessian(tape, trainable_params):
    """Check that all gates with trainable parameters are single-parameter gates
    that are generated by a single operator that squares to the identity. This is a
    helper method to ``adjoint_hessian_diagonal`` of the ``QubitDevice`` class below.
    """

    squares_to_identity = {"PauliX", "PauliY", "PauliZ", "Hadamard"}

    def op_squares_to_identity(op):
        """Check whether an operator squares to the identity."""
        if isinstance(op, qml.operation.Tensor):
            return all(ob.name in squares_to_identity for ob in op.obs)
    ...
    for idx in trainable_params:
        ...
        if len(op.data) > 1:
            raise qml.QuantumFunctionError(
                "Only single-parameter gates can be differentiated with adjoint_hessian_diagonal."
            )
        # Check that the generator is simple enough and the op squares to the identity
        gen = op.generator()
        ...
        if (
            ...
            or len(gen.coeffs) != 1
            or not op_squares_to_identity(gen.ops[0])
        ):
            raise qml.QuantumFunctionError(
                "Only gates that are generated by a single operator that squares to the identity "
                "can be differentiated with adjoint_hessian_diagonal. "
                "Consider decomposing the tape first."
            )
...
class QubitDevice(Device):
    ...
    * :meth:`~.expval`, :meth:`~.var`, :meth:`~.sample`: return expectation values,
    ...
    def adjoint_jacobian(self, tape, starting_state=None, use_device_state=False):
        ...
        trainable_params = _get_trainable_params_wo_obs(tape)
    ...
    def _apply_operation_multistate(self, states, op, use_broadcasting):
        ...
        is the first axis of ``states`` if multiple states are passed. Uses broadcasting if the
        respective flag is activated. Requires an iterable ``states`` input and
        always returns a list.
        """
        if self._ndim(states) == self.num_wires:
            # Only got passed a single state
            return self._apply_operation(states, op)

        if use_broadcasting:
            return list(self._apply_operation(self._stack(states), op))
        return [self._apply_operation(state, op) for state in states]

    def _compute_derivative_states(self, tape, trainable_params):
        r"""Compute the derivative of the full circuit state by inserting generators of
        parametrized gates.
        **Misses a prefactor (-i), i.e. computes :math:`i\partial_k |\psi(\theta)>`**
        """
        use_broadcasting = self.capabilities()["supports_broadcasting"]

        self.reset()
        ops = tape.operations
        if isinstance(ops[0], (qml.QubitStateVector, qml.BasisState)):
            self.apply(ops[:1])
            ops = ops[1:]
            param_count = 1
        else:
        ...
        for op in ops:
            ...
            if d_kets:
                d_kets = self._apply_operation_multistate(d_kets, op, use_broadcasting)
            num_op_params = len(op.data)

            if num_op_params == 1:
                if param_count in trainable_params:
                    coeffs, ops = op.generator().terms()

                    d_kets.append(
                        coeffs[0]
                        * self._apply_unitary(
                            ket, qml.matrix(ops[0], wire_order=ops[0].wires), ops[0].wires
                        )
                    )
                    prefactors.append(coeffs[0])
            elif num_op_params > 1:
                if any(param_count + i in trainable_params for i in range(num_op_params)):
                    raise ValueError("THis should not happen")

            param_count += num_op_params
    ...
    def _contract_with_observable(self, state, observables):
        ...
        number of stated passed via the argument ``state``.
        """
        use_broadcasting = self.capabilities()["supports_broadcasting"]
    ...
    def adjoint_hessian_diagonal(self, tape, starting_state=None, use_device_state=False):
        ...
        This method requires :math:`\mathcal{O}(n_T)` additional copies of the state, and executes
        :math:`\mathcal(O)(n_T n)` operations, where :math:`n` (:math:`n_T`) is the number of
        ...
        Args:
            tape (.QuantumTape): circuit that the function takes the second derivative of

        Keyword Args:
            starting_state (tensor_like): post-forward pass state used to evaluate parts of the
                derivative. Should be complex-valued. Takes precedence over ``use_device_state``.
            use_device_state (bool): use current device state to initialize. A forward pass of the
                same circuit should be the last thing the device has executed. If a
                ``starting_state`` is provided, that takes precedence.

        Returns:
            array: the second derivatives of the tape with respect to trainable parameters,
            corresponding to the diagonal entries of the Hessian.
            Dimensions are ``(len(observables), len(trainable_params))``.

        Raises:
            QuantumFunctionError: if the tape has measurements that are not expectation values
                or contains parametrized gates that are not generated by a Pauli word.
        """
        _check_adjoint_diffability(tape, self)
        ...
        # Reuse fully evolved state if it is available, else compute it.
        if starting_state is not None:
            ket = self._reshape(starting_state, [2] * self.num_wires)
        else:
            if not use_device_state:
        ...
        # Compute the "standard" expectation value of the circuit
        expval = self._contract_with_observable(ket, tape.observables)

        # Compute the derivative of the full circuit state by inserting generators.
        # Misses a prefactor of i, i.e. computes (-i)\partial_k |\psi(\theta)>.
        # Also returns the coefficient between the generators and their Pauli words.
        # Todo: One could potentially include the execute(tape) step above in the following
        # subroutine whenever it is used.
        d_kets, gen_prefactors = self._compute_derivative_states(tape, trainable_params)

        # Compute 2 * i * (-i) * Re{<\partial_k \psi| H |\partial_k \psi>}, where the two complex
        # units in the beginning cancel.
        first_terms = 2 * self._contract_with_observable(d_kets, tape.observables)

        # Square the prefactor c_k such that |\partial_k^2 \psi> = -c_k^2 |\psi>
        # and multiply with the "standard" expectation values to obtain
        # 2 * Re{<\psi | H | \partial_k^2\psi>} = -2 * Re{<\psi | H | \psi>} * c_k^2
        second_terms = self._tensordot(-2 * expval, gen_prefactors**2, axes=0)
