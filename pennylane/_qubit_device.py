...
def _assert_non_Hamiltonian_expval(tape):
...
def _check_gates_adjoint_hessian(tape, trainable_params):
    ...
    that are generated by a single Pauli word. This is a helper method to
    ``adjoint_hessian_diagonal`` of the ``QubitDevice`` class below.
    """
    for idx in trainable_params:
        ...
        # Check that the generator is simple enough
        gen = op.generator()
        # Assuming that Hamiltonians always are decomposed into Pauli words, checking
        # for length=1 is enough here. Projector-generated operations are not supported.
        if not isinstance(gen, qml.Hamiltonian) or len(gen.coeffs) != 1:
            raise qml.QuantumFunctionError(
                "Only gates that are generated by a single Pauli word can be differentiated "
                "with adjoint_hessian_diagonal. Consider decomposing the tape first."
            )
...
class QubitDevice(Device):
    ...
    * :meth:`~.expval`, :meth:`~.var`, :meth:`~.sample`: return expectation values,
    ...
    def adjoint_jacobian(self, tape, starting_state=None, use_device_state=False):
        ...
        _assert_non_Hamiltonian_expval(tape)
    ...
    def _compute_derivative_states(self, tape, trainable_params):
        ...
        parametrized gates. Misses a prefactor (-i), i.e. computes
        :math:`i\partial_k |\psi(\theta)>`
        """
        ...
        for op in tape.operations:
            ...
            if len(op.data) == 1:
                if param_count in trainable_params:
                    ...
                    d_kets.append(coeffs[0] * self._apply_operation(ket, ops[0]))
                    prefactors.append(coeffs[0])
                param_count += 1
            elif len(op.data) > 1:
                if any(param_count + i in trainable_params for i in range(len(op.data))):
                    raise ValueError("Expected trainable single-parameter gates only.")
                param_count += len(op.data)
    ...
    def adjoint_hessian_diagonal(self, tape, starting_state=None, use_device_state=False):
        ...
        Returns:
            array: the second derivative of the tape with respect to trainable parameters.
            Dimensions are ``(len(observables), len(trainable_params))``.
