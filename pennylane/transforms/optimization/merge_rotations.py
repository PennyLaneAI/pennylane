...
from pennylane.ops.qubit.attributes import composable_rotation
...
def merge_rotations(tape, atol=1e-8, include_gates=None):
    ...
    Args:
        ...
        include_gates (None or list[str]): A list of specific operations to merge. If
            ...
            `.pennylane.ops.qubit.attributes.composable_rotation` attribute will be merged. Otherwise,
    ...
    **Example**
    ...
    while len(list_copy) > 0:
        ...
        if not current_gate in composable_rotation:
            apply(current_gate)
            list_copy.pop(0)
            continue
