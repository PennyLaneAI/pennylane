...
from pennylane.ops.qubit.attributes import is_composable_rotation
...
def merge_rotations(tape, atol=1e-8, include_gates=None):
    ...
    Args:
        ...
        include_gates (None or list[str]): A list of specific operations to merge. If
            set to ``None`` (default), all operations in the ``is_composable_rotation``
            attribute will be merged. Otherwise, only the operations whose
