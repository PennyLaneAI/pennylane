...
def _operator_decomposition_gen(
    ...
    if max_expansion is not None and max_expansion <= curr_depth:
...
def _get_plxpr_decompose():  # pylint: disable=missing-docstring, too-many-statements
    ...
    class DecomposeInterpreter(qml.capture.PlxprInterpreter):
        ...
        def decompose_operation(self, op: qml.operation.Operator, curr_depth: int = 0):
        ...
        def _evaluate_jaxpr_decomposition(self, op: qml.operation.Operator, curr_depth: int = 0):
            ...
            if self.max_expansion is not None and curr_depth >= self.max_expansion:
                return self.interpret_operation(op)
            ...
            return self.eval(jaxpr_decomp.jaxpr, jaxpr_decomp.consts, *args, curr_depth=curr_depth)

        def eval(
            self, jaxpr: "jax.core.Jaxpr", consts: Sequence, *args, curr_depth: int = 0
        ) -> list:
            """
            ...
            for eq in jaxpr.eqns:
                ...
                if custom_handler:
                    ...
                    outvals = self.interpret_operation_eqn(eq, curr_depth)
        ...
        def interpret_operation_eqn(self, eqn, curr_depth: int = 0):
            ...
            if not op.has_plxpr_decomposition:
                return self.decompose_operation(op, curr_depth)

            return self._evaluate_jaxpr_decomposition(op, curr_depth)

    def jaxpr_to_jaxpr_decomp(
        interpreter: DecomposeInterpreter, jaxpr: "jax.core.Jaxpr", consts, *args, curr_depth
    ) -> "jax.core.Jaxpr":

        f = partial(interpreter.eval, jaxpr, consts, curr_depth=curr_depth)
    ...
    def handle_cond(self, *invals, jaxpr_branches, consts_slices, args_slice, curr_depth=0):
        ...
        for const_slice, jaxpr in zip(consts_slices, jaxpr_branches):
            ...
            if jaxpr is None:
                ...
                new_jaxpr = jaxpr_to_jaxpr_decomp(
                    copy(self), jaxpr, consts, *args, curr_depth=curr_depth
                )
