...
def compile(
    ...
    Args:
        ...
        basis_set (Sequence[str, type]): A list of basis gates, specified either as
            ...
            (e.g., ``qp.RX``). When expanding the tape, expansion will continue
    ...
    with QueuingManager.stop_recording():
        ...
        for op in basis_set:
            if not isinstance(op, str) and not (
                isinstance(op, type) and issubclass(op, qp.operation.Operator)
            ):
                raise ValueError(
                    f"Elements of basis_set must be strings or Operator subclasses, got {op!r}"
                )

        basis_set_names = {op for op in basis_set if isinstance(op, str)}
        basis_set_types = tuple(op for op in basis_set if isinstance(op, type))
