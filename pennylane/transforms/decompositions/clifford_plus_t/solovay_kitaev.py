...
class TreeSet:
    """Implements tree structure using GateSet for approximation step in Solovay-Kitaev
    ...
    def process_node(self, node, seqs):
        """Adds nodes in the tree structure"""
...
def _unitary_bloch(mat, eps=1e-10):
    ...
    if sine < eps:
        ...
        ny = qml.math.real((mat[1, 0] - mat[0, 1]) / (2 * sine))
...
def solovay_kitaev_decomposition(op, depth, basis_set=None, approximate_set=None):
    ...
    if approximate_set is None:
        approximate_set = build_approximate_set(basis_set)
