...
def _control_controlled_operation(op, control_wire):
    """Control transform for the ControlledOperation."""
    assert isinstance(op, ControlledOperation)
    # No return here since this transform expects operations to be queued.
    ControlledOperation(op._tape, Wires(control_wire) + op._control_wires, do_queue=True)


register_control(ControlledOperation, _control_controlled_operation)
