...
def generate_lattice(dims: Sequence[int], lattice: str) -> Lattice:
    r"""Generates a :class:`~pennylane.ftqc.Lattice` object with a given geometrical parameters and its shape name.

    Args:
        dims(List[int]): Geometrical parameters for lattice generation. For lattices generated by `nx.grid_graph` ( ``'chain'``, ``'rectangle'``,  ``'square'``, ``'cubic'``),
    ...
    if lattice_shape not in _LATTICE_GENERATOR_MAP:
        raise ValueError(
            f"The Lattice generator does not support the given {lattice_shape} lattice shape."
        )
