...
@pytest.mark.parametrize(('mol_name', 'n_act_electrons', 'n_act_orbitals',
...
def test_active_spaces(mol_name, n_act_electrons, n_act_orbitals, docc_indices_ref, act_indices_ref):

    r"""Test the correctness of the generated active spaces"""

    docc_indices, active_indices = qchem.gen_active_space(mol_name, ref_dir, n_act_electrons, n_act_orbitals)

    assert docc_indices == docc_indices_ref
    assert active_indices == act_indices_ref


@pytest.mark.parametrize(('mol_name', 'n_act_electrons', 'n_act_orbitals', 'message_match'),
                         [('lih',         6,  5, 'greater than the total number of electrons'),
                          ('lih',         1,  5, 'should be even'),
                          ('lih',        -1,  5, 'has to be greater than 0.'),
                          ('lih',         2,  6, 'can not be greater than the number '
                                                 'of molecular orbitals'),
                          ('lih',         2,  1, 'there are no virtual orbitals'),
                          ('lih_anion',   2,  5, 'should be odd'),
                          ('lih_anion',   3, -2, 'has to be greater than 0.'),
                          ('lih_anion',   3,  6, 'can not be greater than the number '
                                                 'of molecular orbitals'),
