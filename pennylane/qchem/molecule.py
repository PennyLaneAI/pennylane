...
class Molecule:
    ...
    Args:
        ...
        argnum (int | list(int) | None): index (indices) of the positional argument(s) - [``coordinates``, ``alpha``, ``coeff``] that should support differentiation. For example, ``argnums=[0, 2]`` would mean derivatives can be computed with respect to both ``coordinates`` and ``coeff``.
