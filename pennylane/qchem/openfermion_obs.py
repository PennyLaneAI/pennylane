...
def molecular_hamiltonian(
    ...
    Args:
        molecule (Molecule object): Object used to store all the information about the molecule.
    ...
    **Example**
    ...
    >>> H, qubits = molecular_hamiltonian(molecule)
    >>> print(qubits)
    4
    >>> print(H)
    ...
    if len(molecule.coordinates) == len(molecule.symbols) * 3:
        geometry_dhf = qml.numpy.array(molecule.coordinates.reshape(len(molecule.symbols), 3))
        geometry_hf = molecule.coordinates
    elif len(molecule.coordinates) == len(molecule.symbols):
        geometry_dhf = qml.numpy.array(molecule.coordinates)
        geometry_hf = molecule.coordinates.flatten()
    ...
    if method == "dhf":
        ...
        core, active = qml.qchem.active_space(
            mol.n_electrons, mol.n_orbitals, molecule.mult, active_electrons, active_orbitals
        )
    ...
    if method == "pyscf" and mapping.strip().lower() == "jordan_wigner":
        core_constant, one_mo, two_mo = _pyscf_integrals(
            molecule.symbols, geometry_hf, molecule.charge, molecule.mult, molecule.basis_name, active_electrons, active_orbitals
        )
    ...
    hf_file = meanfield(molecule.symbols, geometry_hf, molecule.name, molecule.charge, molecule.mult, molecule.basis_name, "pyscf", outpath)
