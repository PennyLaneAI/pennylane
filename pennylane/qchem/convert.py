...
def _qiskit_nature_to_pennylane(qubit_operator, wires=None):
    """Convert Qiskit-Nature SparsePauliOp to 2-tuple of coeffs and PennyLane Paulis.
    ...
    **Example**
    ...
    def _get_op(term, wires):
        ...
        if len(term) == n_wires:
            ...
            if active_new_opmath():
                return qml.prod(qml.pauli.string_to_pauli_word(term, wire_map=wire_map))

            return Tensor(qml.pauli.string_to_pauli_word(term, wire_map=wire_map))

        return qml.Identity(wires[0])

    coeffs, ops = zip(*[(coef, _get_op(term, wires)) for term, coef in qubit_operator.to_list()])
...
def _pennylane_to_qiskit_nature(coeffs, ops, wires):
    ...
    Returns:
        SparsePauliOp: an instance of Qiskit-Nature's ``SparsePauliOp``.

    Notes:
        Instead of having to provide the full list of wires,
        e.g., wires=[0, 1, 2, 3, ..., n_wires],
        this function could be alternatively implemented using
    ...
    **Example**
    ...
    try:
        ...
        raise ImportError(
            "This feature requires qiskit. It can be installed with: pip install qiskit"
        ) from Error
...
def _qiskit_nature_pennylane_equivalent(
    ...
    Args:
        qiskit_qubit_operator (SparsePauliOp): Qiskit-Nature qubit operator
    ...
    """
    coeffs, ops = pennylane_qubit_operator.terms()
    return qiskit_qubit_operator == _pennylane_to_qiskit_nature(coeffs, ops, wires=wires)
...
def import_operator(qubit_observable, format="openfermion", wires=None, tol=1e010):
    ...
    We currently support:
        - `OpenFermion <https://quantumai.google/openfermion>`__ operators: the function accepts most types of
            OpenFermion qubit operators, such as those corresponding to Pauli words and sums of Pauli words.
        - `SparsePauliOp <https://docs.quantum.ibm.com/api/qiskit/qiskit.quantum_info.SparsePauliOp>`__ operators from Qiskit and Qiskit-Nature.
    ...
    **Example**
    ...
    >>> h_pl = import_operator(h_qt, format='qiskit-nature')
    >>> print(h_pl)
    (0.14297) [Z1 Z3]
    + (-0.0548) [Y0 X1 Y2 Y3]
    ...
    + (-0.0548) [X0 X1 Y2 Y3]
    """
    if format not in ["openfermion", "qiskit-nature"]:
        raise TypeError(f"Converter does not exist for {format} format.")

    if format == "qiskit-nature":
        # dealing with qiskit `SparsePauliOp`
        coeffs = np.array([np.real_if_close(coef, tol=tol) for coef in qubit_observable.coeffs])
    else:
        # dealing with default openfermion `QubitOperator`
        coeffs = np.array(
            [np.real_if_close(coef, tol=tol) for coef in qubit_observable.terms.values()]
        )
    ...
    if format == "qiskit-nature":
        if active_new_opmath():
            return qml.dot(*_qiskit_nature_to_pennylane(qubit_observable, wires=wires))

        return qml.Hamiltonian(*_qiskit_nature_to_pennylane(qubit_observable, wires=wires))

    # return default openfermion `QubitOperator`
    if active_new_opmath():
