...
def _rccsd_state(ccsd_solver, tol=1e-15):
    ...
    **Example**
    ...
    t2aa = t2aa - 0.5 * np.kron(t1a, t1a).reshape(nelec_a, nvir_a, nelec_a, nvir_a).transpose(0,2,1,3).numpy()
    t2bb = t2bb - 0.5 * np.kron(t1b, t1b).reshape(nelec_b, nvir_b, nelec_b, nvir_b).transpose(0,2,1,3).numpy()
...
def _uccsd_state(ccsd_solver, tol=1e-15):
    ...
    **Example**
    ...
    t2bb = t2bb - 0.5 * np.kron(t1b, t1b).reshape(nelec_b, nvir_b, nelec_b, nvir_b).transpose(0,2,1,3).numpy()
    # this just aligns the entries with how the excitations are ordered when generated by _excited_configurations()
    t2ab = t2ab.transpose(0,2,1,3) - 0.5 * np.kron(t1a, t1b).reshape(nelec_a, nvir_a, nelec_b, nvir_b).numpy()
