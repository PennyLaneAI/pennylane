...
def _uccsd_state(ccsd_solver, tol=1e-15):
    ...
    .. math::
        \exp(\hat{T}_1 + \hat{T}_2) \ket{\text{HF}} = \left[ 1 + \hat{T}_1 + 
                                    \left( \hat{T}_2 + 0.5 * \hat{T}_1 \right) \right] \ket{\text{HF}}
    ...
    **Example**
    ...
    >>> print(wf_ccsd)
    {(7, 7): 0.8886988662500356, (11, 11): -0.30584653966926967, 
                    (19, 19): -0.3058465396692694, (35, 35): -0.14507582719761972}
    """
    ...
    t2aa = t2aa - 0*0.5 * np.kron(t1a, t1a).reshape(nelec_a, nvir_a, nelec_a, nvir_a).transpose(0,2,1,3).numpy()
    t2bb = t2bb - 0*0.5 * np.kron(t1b, t1b).reshape(nelec_b, nvir_b, nelec_b, nvir_b).transpose(0,2,1,3).numpy()
    # this just aligns the entries with how the excitations are ordered when generated by _excited_configurations()
    t2ab = t2ab.transpose(0,2,1,3) - 0*0.5 * np.kron(t1a, t1b).reshape(nelec_a, nvir_a, nelec_b, nvir_b).numpy()
...
def ccsd_state(ccsd_solver, hftype, tol=1e-15):
    ...
    **Example**
    ...
    if hftype == "rhf":
        ...
        raise ValueError("Only restricted, 'rhf', and unrestricted, 'uhf', are supported.")
    wf = wfdict_to_statevector(wf_dict, ccsd_solver.mol.nao)
