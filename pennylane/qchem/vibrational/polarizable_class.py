...
@dataclass
class PolarizablePES:
    r"""Data class to store information needed to construct a vibrational Hamiltonian for a molecule with polarizability tensor.
    ...
    Args:
        ...
        placzek_data (list[TensorLike[float]]): list of  one-mode, two-mode and three-mode dipole
            moment data, with shapes ``(m, l, 6)``, ``(m, m, l, l, 6)`` ``(m, m, m, l, l, l, 6)``,
            respectively, where ``m = len(freqs)`` and ``l > 0``
    ...
    **Example**
    ...
    def __init__(
        ...
        self.placzek_onemode = placzek_data[0] if placzek_data else None
        self.placzek_twomode = placzek_data[1] if placzek_level >= 2 else None
        self.placzek_threemode = placzek_data[2] if placzek_level >= 3 else None
        self.localized = localized
        self.placzek_level = placzek_level
...
def _excited_states(scf_result, excited_method='tddft', num_states=10):
    if es_method == "tddft":
        ...
        raise ValueError(f"Trying to obtain excited electronic energies with method {excited_method}, not implemented!")

    return excitation_energies, transition_dipoles

def _get_placzek(scf_result, scf_method, excited_method='tddft', omega_I=None, num_states=10):
    r"""Obtain the Placzek polarizability tensor (PPT) at the nuclear geometry specified in scf object

    Args:
        scf_result (pyscf.scf object): pyscf object from electronic structure calculations
        scf_method (str): Electronic structure method that can be either restricted and unrestricted
            Hartree-Fock,  ``'rhf'`` and ``'uhf'``, respectively. Default is ``'rhf'``. This was
            used in the pyscf.scf calculation
        es_method (str): Method for calculating electronic excitation energies. Default is ``'tddft'``.
        omega_I (float): Value of incoming photon frequency. Calculates frequency-independent PPT if None is provided
        num_states (int): Number of excited states to incorporate in calculation

    Returns:
        ndarray (float) with dimensions 3x3: PPT values for given nuclear geometry
    """

    excitation_energies, transition_dipoles = _excited_states(scf_result, excited_method, num_states)

    energy_quotients = np.zeros(num_states)
    for pp in range(num_states):
        if omega_I is None:
            energy_quotients[pp] = -1/(excitation_energies[pp]**2)
        else:
            energy_quotients[pp] = 1/(omega_I**2 - excitation_energies[pp]**2)

    placzek_tensor = np.zeros(6)

    for pp,mu in enumerate(transition_dipoles):
        for aa in range(3):
            placzek_tensor[aa] += np.abs(mu[aa])**2 * energy_quotients[pp]

    for pp,mu in enumerate(transition_dipoles):
        idx = 3
        for aa in range(3):
            for bb in range(aa):
                placzek_tensor[idx] += mu[aa] * mu[bb] * energy_quotients[pp]
                idx += 1

    if omega_I is not None:
        placzek_tensor *= 2*omega_I
    else:
        placzek_tensor *= 2

    return placzek_tensor