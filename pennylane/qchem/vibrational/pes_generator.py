...
def _pes_onemode(
    ...
    jobs_on_rank = np.array_split(all_jobs, num_workers-1)
...
def _local_pes_onemode(
    ...
    __local_pes_onemode(
        rank, jobs_on_rank, molecule, scf_result, freqs, vectors, grid, path, method=method, dipole=dipole
    )
