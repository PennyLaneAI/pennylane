...
\DeclareMathOperator{\Span}{Span}  % linear span of vectors
...
\newcommand{\Admap}[1]{\tilde{#1}} % mapping observables by adjoint action
...
%quantum and classical node styles for tikz
\usetikzlibrary{calc}
...
\email{Author order to be discussed }
...
Recent progress in the development and commercialization of quantum technologies has had a profound impact on the landscape of quantum algorithms. One important design paradigm for algorithms running on near-term quantum devices is the idea of hybrid architectures that integrate quantum and classical processing to harvest the best of both worlds.
Most prominently, \textit{variational circuits} are shallow, parameter-dependent quantum circuits that can be optimized with regards to a given objective by a classical optimization strategy. Variational circuits have found numerous applications in quantum chemistry [Romero,...], quantum optimization [Farhi,...] and, more recently, in quantum machine learning [REFs]. \\

Hybrid quantum algorithms and variational circuits have opened up new research avenues for application of near term quantum technologies. However, and unusual for the field of quantum computing, a lot of research for these mostly heuristic algorithms necessarily focuses on numerical experiments rather than rigorous mathematical analysis. Luckily, there are various publicly accessible platforms to simulate quantum algorithms or even run them on real quantum devices through a cloud service. However, even though some frameworks are designed with variational circuit optimization in mind [SF, QMLT], there is at this stage no unified framework for the optimization of hybrid algorithms that run on quantum simulators or devices. \\

PennyLane is an open source framework that facilitates the optimization of hybrid quantum-classical algorithms. It can in principle be used in combination with any quantum computing platform. PennyLane is coded with quantum machine learning applications in mind, which tend to be more complex than pure optimization tasks since they may require data inputs to be fed into the quantum circuit, and can involve high-dimensional optimization landscapes. \maria{Advertise features.}

\section{Hybrid optimization}

The goal of optimization is to find minima or maxima of an objective function that quantifies the quality of a solution for a certain task. (Here we follow the convention that the objective is a \textit{cost} that has to be minimized.) In hybrid quantum-classical optimization tasks, the scalar output of the cost function is a result of both classical and quantum processing. We call these submodules \textit{classical} and \textit{quantum nodes}. A node can take inputs from one or more previous nodes or ``external inputs'', and produces an output of some sorts. The cost can therefore be expressed by a Directed Acyclic Graph (DAG) of quantum and classical nodes feeding into each other (see Figure XXX). Both classical and quantum nodes can depend on tunable parameters that we call \textit{variables}. The variables are optimized to minimize the cost.\\    

While classical nodes can simply be understood as functions that map an input to an output, quantum nodes consist of quantum circuits that either run on a real quantum device, or are simulated by a classical computer. In both cases we assume that the input of the quantum node are adaptable gate parameters, while its output is the estimate of an expectation value which is obtained by running the circuit for $R$ times and measuring a set of non-commuting observables. For example, these could be the Pauli-z observable of each qubit in a register, or the photon number of a quantum mode.\\

Hybrid optimization can be used as a scheme to optimize  variational quantum circuits in applications such as Quantum Approximate Optimization [REF] or Variational Quantum Eigensolvers [REF]. However it can also be used in more complex tasks such as training a hybrid quantum-classical machine learning model in a supervised fashion. In the following we will introduce the make-up of quantum and classical nodes in more detail.


\subsection{Quantum nodes}
As mentioned above, a quantum node is a module in the cost function DAG which consists of a quantum circuit $U(\gamma)$ wich depends on a set of tunable gate parameters $\gamma$. The circuit acts on an initial state $|\psi_0 \rangle$ which is usually the vacuum or ground state $\ket{0}$.\\

The circuit parameters $\gamma$ can be of three different types: a) they can be trainable parameters or variables $\theta$, b) they can be fixed values throughout the application, or c) they can be used to take inputs $x$ from preceding nodes. After applying the variational circuit, we get a final quantum state $\ket{\psi (x, \theta) }$ from which we draw sample observations with regards to an observable $O$ (i.e., a compuational basis measurement returns samples of bit strings, and a photon number resolution measurement returns strings of integers). The samples are used to estimate the expectation value
...
A classical node can be any differentiable function $y = h(x, \theta)$ that maps inputs $x$ and variables $\theta$ to real \maria{any need for complex outputs?} output vectors or scalars $y$.\\

We depict classical nodes by the following symbol:\\
\begin{figure}[h]
\centering
\begin{tikzpicture}
\node[classical node] (c) at (0,0) {$h(x, \theta) $};
\draw[->] (c) -- (2.5,0)  node[out label] {$y$};
\end{tikzpicture}
\end{figure}
...
\begin{table*}[t]
\begin{tabular}{lll}
Gate $\G$ & linear representation & derivative of representation\\   \hline \hline
\cell{Phase rotation\\ $R(\phi)$}
&  
$A_R(\phi) = \begin{pmatrix} 
1 & 0          & 0 \\ 
0 & \cos(\phi) & -\sin(\phi) \\ 
0 & \sin(\phi) & \cos(\phi) 
\end{pmatrix}$ 
&
$\partial_{\phi} \; A_R(\phi) =  
\frac{1}{2} A_R(\phi + \frac{\pi}{2}) 
- \frac{1}{2} A_R(\phi + \frac{3\pi}{2})$\\   \hline 
\cell{Displacement\\ $D(\alpha_{\text{Im}}, \alpha_{\text{Re}})$} 
&  
$A_D(\alpha_{\text{Re}}, \alpha_{\text{Im}}) = \begin{pmatrix}
1 & 0 & 0 \\
2\alpha_{\text{Re}} & 1 & 0  \\
2\alpha_{\text{Im}} & 0 & 1 \\
\end{pmatrix}$ 
& 
\cell{$\partial_{\alpha_{\text{Re}}} A = \frac{1}{2}A(1,0) - \frac{1}{2}A(-1,0)$\\
$\partial_{\alpha_{\text{Im}}} A = \frac{1}{2}A(0,1) - \frac{1}{2}A(0,-1)$
}\\  \hline 
\cell{Squeezing\footnote{The squeezing gate $\tilde{S}$ in SF has an imaginary parameter $\phi$ which defines the angle of the squeezing. $\tilde{S}(r, \phi) = R(\frac{\phi}{2})S(r)R(\frac{\phi}{2})^{\dagger}$. The derivation of the rotations can be executed as specified in this table} \\ $S(r)$}
&  
$A_S(r) = \begin{pmatrix} 
\e^{-r} & 0 
\\ 0 & \e^{r} 
\end{pmatrix}$
& 
\cell{$\partial_{r} A(r) =   A(r + \log (\frac{\sqrt{5} + 1}{2}) ) -  A(r + \log (\frac{\sqrt{5} - 1}{2})) $\\
}
\\ \hline 
\cell{Beam splitter\\ $B(\theta, \phi)$} 
& 
$A_B(\theta, \phi) = \begin{pmatrix} 
\mathbbm{1} & 0 & 0\\
0 & \cos(\theta) \mathbbm{1} & -e^{-i\phi}\sin(\theta) \mathbbm{1}\\
0 & e^{i\phi}\sin(\theta) \mathbbm{1} & \cos(\theta) \mathbbm{1} \end{pmatrix} $
& 
\cell{$\partial_{\theta} A_B(\theta, \phi) =  \frac{1}{2} A_B(\theta + \frac{\pi}{2}, \phi) - \frac{1}{2} A_B(\theta + \frac{3\pi}{2}, \phi)$ \\ 
$\partial_{\phi} A(\theta, \phi) =  \frac{1}{2} A_B(\theta, \phi+ \frac{\pi}{2}) - \frac{1}{2} A_B(\theta, \phi + \frac{3\pi}{2})$}
\\   \hline \hline 
\end{tabular}
...
%= \sum_{xy} \Admap{U}_{ix} \Admap{U}_{jy} E_x E_y\\
%&= \sum_{xy} (\Admap{U} \otimes \Admap{U})_{ij,xy} E_x E_y.
\end{align*}
yielding
\begin{align*}
...
%&=
%(\pd{}{\theta} \Admap{U}_{\vt \: ik} E_k) (\Admap{U}_{\vt \: jl} E_l)
%+(\Admap{U}_{\vt \: ik} E_k) (\pd{}{\theta} \Admap{U}_{\vt \: jl} E_l)\\
&=
\sum_x c_x ((\Ad_{U_{\vt+\vR_x}^\dagger} E_i) (\Ad_{U_{\vt}^\dagger} E_j)
+(\Ad_{U_{\vt}^\dagger} E_i) (\Ad_{U_{\vt+\vR_x}^\dagger} E_j))
%\sum_{xy}\pd{}{\theta}(\Admap{U}_{\vt} \otimes \Admap{U}_{\vt})_{ij,xy} E_x E_y
%&=
%\sum_{xyj}(c_j \Admap{U}_{\vt+\vR_j} \otimes \Admap{U}_{\vt})_{ij,xy} E_x E_y
\end{align*}
and thus
\begin{widetext}
\begin{align}
