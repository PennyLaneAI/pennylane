...
\end{eqnarray*}
THAT $i$ IS STILL IN THE WAY IF WE DO NOT WORK WITH GENERATORS!


We try to show now how to apply a transformation of the form $\G \pm i A_k$ on a CV quantum computer with the help of a vacuum mode and beam splitters, using the Heisenberg picture. 

Starting with $\ket{\psi}$ in the first mode, and vacuum in the second, we apply the circuit shown in Fig. \ref{Fig:der_eval_cv}. The circuit consists of a 50-50 beam splitter with relative phase shift of $\pi/2$, a Gaussian transformation on each mode separately, as well as another 50-50 beam splitter. We measure the expectation of the vector of the transformed quadrature operators $\c, \d$. 
...
CAN WE USE IT TO GET THE DESIRED EXPECTATIONS?
