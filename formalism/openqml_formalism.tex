...
\begin{tikzpicture}
\node[input node] (o) at (-0.5, 0) {x};
\node[quantum node] (c) at (0,0) {$U(x, \theta) $ \nodepart{two} $O $};
\draw[->] (c) -- (3.5,0)  node[out label] {$y$} node[samples label] {$R$};
\draw[->] (o) -- (c);
\end{tikzpicture}
\end{figure}

\subsection{The hybrid graph}

\begin{figure}[t]
\begin{tikzpicture}
\node[classical node, anchor=center] (c1) at (0,0) {\phantom{.}};
\node[quantum node, anchor=center] (q1) at (1,0) {\phantom{.} \nodepart{two} \phantom{.}};
\node[quantum node, anchor=center] (q2) at (1,0.75) {\phantom{.} \nodepart{two} \phantom{.}};
...
\node[output node, anchor=center] (o2) at (6,0) {$C(\Theta)$ };
\draw[->] (c1) -- (q1);
\draw[->] (q1) -- (c2);
\draw[->, gray] (q2) -- (q3);
\draw[->] (c2) -| (mid1down)|- (c3);
...
\node[classical node, anchor=center] (c5) at (5,0) {\phantom{.} };
\end{tikzpicture}
\caption{Illustration of the cost function as a Directed Acyclic Graph of quantum and classical nodes.}
\label{Fig:dag}
\end{figure}

The inputs to a classical or quantum node are either externally given (such as data samples fed into a model) or they are the output from one or more previous nodes. The cost can therefore be expressed by a Directed Acyclic Graph (DAG) of quantum and classical nodes feeding into each other (see Figure XXX). 

\subsection{Backpropagating throught the graph}

PennyLane focuses on gradient-based algorithms such as gradient descent and its variations. In gradient descent, to minimize the cost in every step the parameters are updated according to the following simple loop:\\
...
\begin{flushleft}
\begin{tikzpicture}
\node[align=left, anchor=west] at (-1,1) {(a) Variational Quantum Eigensolver};
...
\node[output node] (o) at (7,-1) {$C$};
\draw[] (q1.east) -- (3.7,0)  node[out label] {$y_{1}$} node[samples label] {$R$};
\draw[->] (3.7,0) -- (3.7,-1)-- (c);
\draw[] (qK.east) -- (3.7,-1.5) node[out label] {$y_{i}$} node[samples label] {$R$};
\draw[->] (3.7,-1.5) -- (3.7,-1)-- (c);
\draw[->] (c) -- (o);
%\draw[red] (q1) -| (mid) |- (c.west);
\end{tikzpicture}
 ~~~~~~~~~~~~~~~ \begin{tikzpicture}
\node[align=left, anchor=west] at (-1,0.5) {(b) Generative quantum model};
\node[align=left, anchor=west] at (-0.5,0) {Training:};
...
\node[align=left, anchor=west] at (-0.5,-1.5) {Data generation:};
\node[quantum node] (q) at (0,-2.25) {$ U(\theta) $ \nodepart{two} $\sigma_z \otimes \hdots \otimes\sigma_z$};
\node[output node] (o) at (4.25,-2.25) {$y$};
...
\node[classical node] (c1) at (0,0) {$h_1(\bx, \theta_{1}) $};
\node[] (dots) at (3,0) {$\hdots $};
\node[classical node] (cK) at (4.5,0) {$h_K(\mathbf{y}_{{K-1}}, \theta_{K}) $};
\node[quantum node] (q) at (8.5,0) {$ U(\mathbf{y}_K, \theta_{K+1}) $ \nodepart{two} $\sigma_z$};
\node[output node] (o) at (12.5,0) {$y$};
\draw[-] (q) -- (o) node[samples label] {$R$};
\draw[->] (i) -- (c1);
\draw[->] (c1) -- (dots) node[out label] {$\mathbf{y}_{1}$};
\draw[->] (dots) -- (cK) node[out label] {$\mathbf{y}_{{K-1}}$};
\draw[->] (cK) -- (q) node[out label] {$\mathbf{y}_{K}$};
\end{tikzpicture}
...
\bibliography{pennylane_formalism}
...
where the matrix $q$~is real and symmetric.\footnote{In general $q$ could be hermitian, but the imaginary components always cancel out via commutators.}
We obtain
\begin{align*}
...
\subsubsection{Beamsplitter: $B(\theta, \phi) = e^{\theta (e^{i\phi} a_1 a_2^\dagger -e^{-i\phi} a_1^\dagger a_2)}$}

Conjugation by two-mode gaussian gates maps
$\Span\{\I, x_1, p_1, x_2, p_2\}$ to itself.

Since
$B(\theta, \phi) = R_1^\dagger(\phi) B(\theta, 0) R_1(\phi)$,
we have
\begin{align*}
\Admap{B}(\theta, \phi) &=
...
\begin{pmatrix}
  1 & 0 & 0 & 0 & 0\\
  0 & \block(2,2){\cos(\theta)\I_2} & \block(2,2){-\sin(\theta)\hat{R}^\dagger(\phi)}\\
  0\\
  0 & \block(2,2){\sin(\theta)\hat{R}(\phi)} & \block(2,2){\cos(\theta)\I_2}\\
  0
\end{pmatrix},
\end{align*}
since
...
\begin{pmatrix}
  0 & 0 & 0 & 0 & 0\\
  0 & \block(2,2){0} & \block(2,2){-\sin(\theta)\partial_\phi\hat{R}^\dagger(\phi)}\\
  0\\
  0 & \block(2,2){\sin(\theta)\partial_\phi\hat{R}(\phi)} & \block(2,2){0}\\
  0
\end{pmatrix}\\
&=
