...
\[\bra{\psi} A^{\dagger}  \hat{Q} \G \ket{\psi} +  \bra{\psi}\G^{\dagger} \hat{Q} A  \ket{\psi} = 2 p_0 \tilde{E}_0 + 1 p_1 \tilde{E}_1.\]
...
The Gaussian gates applied to each mode can be expressed by symplectic transformations applied to the subspaces of each mode. Let gate $A$ acting on the first mode be given by the Bogoliubov transform
...
and let gate $\G$ acting on the second mode be given by the transform
...
\begin{equation}
	\partial_{\phi} \; \mathcal{R}(\phi) = \mathcal{R}(\phi + \frac{\pi}{2}).
...
\end{pmatrix}.
\end{equation}
The derivative of the Bogoliubov transform $D$ with respect to $b_{\mathrm{im}}$ and $b_{\mathrm{re}}$ is given by 
