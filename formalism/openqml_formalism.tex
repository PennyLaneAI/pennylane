...
\section{Linear combination of unitaries}\label{App:gatederiv}


An example is the special case for Equation (\ref{eq:decomposition_derivative}) when $\G(\mu) = \e^{i\mu D}$ for a generator $D$. Then, formally,
\begin{equation}
	\partial_{\mu}\G(\mu) = i D \e^{i\mu D}.
	\label{Eq:der_of_generator_gate}
\end{equation}
Since $D$ might not be a unitary operator, we have to decompose it into unitaries $D_k$ via $D = \sum_k \delta_k D_k$, so that
\[
\partial_{\mu}\G(\mu) = \sum_k \underbrace{i \delta_k}_{a_k} \underbrace{D_k \e^{i\mu D}}_{A_k}.
\]
In other words, the derivative sequence $A_k$ consists of $D_k$ as well as the original gate. We call this the \textit{generator decomposition rule}.\\

In many situations, the derivative sequence is a linear combination of the original gate but with a transformed parameter:
\[ \partial \G(\mu) = \sum_k  a_k \underbrace{\G(r_k (\mu))}_{A_k}. \]
For example, the transformation $r_k (\mu)$ can be a parameter shift by $\frac{\pi}{2}$, or replacing the parameter by a constant. We call this the \textit{parameter shift rule}. \\

We will look at some concrete gates and formulate their derivatives.
...
We have seen above how the derivative of $\G$ can be decomposed into $\sum_{k=1}^K a_k A_k$ where $A_k$ are also Gaussian gates. We consequently need to compute the values
\begin{eqnarray}
	&&\bra{\psi} A_k^{\dagger} \hat{c} \G \ket{\psi} +  \bra{\psi}\G^{\dagger} \hat{c} A_k  \ket{\psi} \label{Eq:simplf_exp_der1}  \\
	&&\bra{\psi} A_k^{\dagger} \hat{d} \G \ket{\psi} +  \bra{\psi}\G^{\dagger} \hat{d} A_k  \ket{\psi}
	\label{Eq:simplf_exp_der2} 
\end{eqnarray} 
for $k=1,...,K$. For the position quadrature we have that
\begin{eqnarray*}
&&\frac{1}{2}\bra{\psi} (\G + i A_k)^{\dagger} \hat{c} (\G + i A_k) \ket{\psi} \\
&-& \frac{1}{2}  \bra{\psi}(\G - i A_k)^{\dagger}  \hat{c} (\G - i A_k) \ket{\psi}  \\
&=& i \bra{\psi} A_k^{\dagger} \hat{c} \G \ket{\psi} - i \bra{\psi}\G^{\dagger} \hat{c} A_k  \ket{\psi}
\end{eqnarray*}



We show now (hopefully) how to compute Eq. (\ref{Eq:simplf_exp_der1}) and (\ref{Eq:simplf_exp_der2}) on a CV quantum computer with the help of a vacuum mode and beam splitters, using the Heisenberg picture. 

Starting with $\ket{\psi}$ in the first mode (after applying $U$, and vacuum in the second, we apply the circuit shown in Fig. \ref{Fig:der_eval_cv}. The circuit consists of a 50-50 beam splitter with relative phase shift of $\pi/2$, a Gaussian transformation on each mode separately, as well as another 50-50 beam splitter. We measure the expectation of the vector of the transformed quadrature operators $\c, \d$. 


\begin{figure}[t]
$$
\Qcircuit @C=1em @R=.7em {
\lstick{\ket{\psi}} &  \multigate{1}{BS(\frac{\pi}{4}, \frac{\pi}{2})}  & \gate{A}&  \multigate{1}{BS(\frac{\pi}{4}, 0)} &  \meter\\
\lstick{\ket{0}} &   \ghost{BS(\frac{\pi}{4},\frac{\pi}{2})}   & \gate{\G}&\ghost{BS(\frac{\pi}{4}, 0)} &  \meter\\
}
$$
...
\[\frac{1}{\sqrt{2}}\begin{pmatrix} \c + i \x \\ \d + i \p \\ \x - i\c \\ \p - i \d \end{pmatrix}.\]
The Gaussian gates applied to each mode can be expressed by symplectic transformations applied to the subspaces of each mode:
\[\underbrace{\begin{pmatrix} s_{11} & s_{12}\\ s_{21} & s_{22} \end{pmatrix}}_{S} \begin{pmatrix} \c + i \x \\ \d + i \p \end{pmatrix} + \underbrace{\begin{pmatrix} \alpha_1 \\ \alpha_2 \end{pmatrix}}_{\alpha}, \]
and
\[\underbrace{\begin{pmatrix} r_{11} & r_{12}\\ r_{21} & r_{22} \end{pmatrix}}_{R} \begin{pmatrix} \x - i\c \\ \p - i \d\end{pmatrix} + \underbrace{\begin{pmatrix} \beta_1 \\ \beta_2 \end{pmatrix}}_{\beta}.\]
After the second beam splitter, we get the following overall transformation on the first mode:
\[\begin{pmatrix} \c \\ \d \end{pmatrix} \rightarrow \frac{1}{2} \bigg[ (S-iR) \begin{pmatrix} \c \\ \d \end{pmatrix} + (R+iS) \begin{pmatrix} \x \\ \p \end{pmatrix} + \alpha + \beta \bigg]\]
...
\[\frac{1}{2} \bigg[ (s_{11}-ir_{11}) \bra{\psi}\c\ket{\psi} + (s_{12}-ir_{12}) \bra{\psi}\d\ket{\psi} + \alpha_1 + \beta_1
%+(r_{11}+is_{11}) \bra{0}\x\ket{0} +(r_{12}+is_{12}) \bra{0}\p\ket{0}
\bigg]\]
for the ``position'' quadrature, and
\[\frac{1}{2} \bigg[ (s_{21}-ir_{21}) \bra{\psi}\c\ket{\psi} + (s_{22}-ir_{22}) \bra{\psi}\d\ket{\psi} + \alpha_2 + \beta_2
%+(r_{21}+is_{21}) \bra{0}\x\ket{0} +(r_{22}+is_{22}) \bra{0}\p\ket{0}
\bigg]\]
for the ``momentum'' quadrature, since the expectation $\bra{0}\x\ket{0}$ and $\bra{0}\p\ket{0}$ evaluate to zero. This is equivalent to the expectation of $(\c, \d)^{\dagger}$ after the (non-symplectic) transformation 
\[(S-iR)\begin{pmatrix} \c \\ \d \end{pmatrix} + \alpha + \beta.\]
