...
Here $\mathcal{D}$ is the dataset associated with the cost function for learning problems. If $C$ depends on a dataset $\mathcal{D}$, stochastic gradient descent refers to a gradient descent algorithm where in each step $\mathcal{D}$ is a random subsample of the full training data set.\\
...
We can always express the derivative of a gate as a weighed sum of ``allowed gates'' $A_k(\mu)$,
\begin{equation}
        \partial_{\mu} \G(\mu) = \sum_k a_k A_k(\mu),
    \label{eq:decomposition_derivative}
\end{equation}
with complex coefficients $a_k$. Any
...
\end{eqnarray}
where $U_{A_k}(\theta)$ is the original circuit but with the gate $\G_r^l(\mu)$ replaced by $A_k(\mu)$. We show examples for the decomposition in Appendix \ref{App:gatederiv}.\\


Summarizing the above, we get the formula
\begin{multline}
        \partial_{\mu} y_q(\theta) = \sum_k a^*_k  \bra{0} U_{A_k}(\theta)^{\dagger} \hat{O} U(\theta) \ket{0} \\
        + a_k  \bra{0} U(\theta)^{\dagger} \hat{O} U_{A_k}(\theta) \ket{0}.
        \label{Eq:expgrad}
\end{multline}
The terms $\bra{0} U_{A_k}(\theta)^{\dagger} \hat{O} U(\theta) \ket{0}$ are not expectation values and can therefore not be evaluated with the quantum device, unless we consult tricks. 

\subsection{Evaluating the gradients on a quantum computer}

Next we need to show how to estimate this value with a quantum device. 



\subsubsection{Simplification for Pauli gates}

Let us consider for ease of notation that the variational circuit only consists of the gate $\G(\mu) = \e^{i \mu G}$, where $G$ is a generator of a one-parameter subgroup. Let us assume $G^2 = r^2\I$. This is equivalent to the two eigenvalues of $\mathcal{G}$ being $\lambda_i = \pm r$. Then 
\begin{multline}
\partial_{\mu} y_q =  \bra{0}(\partial_{\mu}\G^{\dagger}(\mu)) \hat{O} \G(\mu) \ket{0} + \bra{0}\G^{\dagger}(\mu) \hat{O} (\partial_{\mu} \G(\mu)) \ket{0},
\label{Eq:deriv_pauli}
\end{multline}
is equivalent to
\begin{multline}
\frac{1}{2}\bra{0}\G^{\dagger}(\mu)\G^{\dagger}(\frac{\pi}{4r}) \hat{O} \G(\frac{\pi}{4r})\G(\mu) \ket{0} \\
- \frac{1}{2}\bra{0}\G^{\dagger}(\mu)\G^{\dagger}(-\frac{\pi}{4r}) \hat{O}\G(-\frac{\pi}{4r}) \G(\mu)\ket{0}.
\label{Eq:insertpifourth}
\end{multline}
...
\begin{align*}
\mathcal{G}(\mu) &= \exp(-iG\mu) = \sum_{k=0}^\infty \frac{(-i\mu)^k G^k}{k!}\\
&=
\sum_{k=0}^\infty \frac{(-i\mu)^{2k} G^{2k}}{(2k)!}
+\sum_{k=0}^\infty \frac{(-i\mu)^{2k+1} G^{2k+1}}{(2k+1)!}\\
&=
\I \sum_{k=0}^\infty \frac{(-1)^{k} (\mu r)^{2k}}{(2k)!}
-i G/r \sum_{k=0}^\infty \frac{(-1)^{k} (\mu r)^{2k+1}}{(2k+1)!}\\
&=
\I \cos(r \mu)
-i G/r \sin(r \mu).
\end{align*}
This way, the gate with $\mu = \frac{\pi}{4r}$ is given by
\be
\G(\pm \frac{\pi}{4r}) = \frac{1}{\sqrt{2}}(\I \pm ir^{-1}G).
\ee
Inserting this into Equation (\ref{Eq:insertpifourth}), we find that some terms cancel out to arrive at Equation (\ref{Eq:deriv_pauli}). 
\end{proof}
...
where $|\vec{a}|=1$. We obtain $G^2 = \frac{1}{4} \I$, and thus $r=1/2$.

\maria{Check imaginary unit prefactor.}
...
\subsection{Derivatives of Pauli generated single qubit gates}
A standard example for the generator decomposition rule is the case when $D$ is a Pauli operator. Then, the sum over $k$ only has one term, since $D$ is already unitary and can be applied as a gate.  
...
Using the quadrature operator formalism with the Heisenberg picture, we show that for most Gaussian gates the derivative of a gate can be expressed by using the same gate with a simple transformation of the parameters. In the case of squeezing we require a classical linear combination of such gates. \\

\paragraph{Beam splitter}
The beam splitter with $\theta$ and phase $\phi$ (using Strawberry Fields conventions) applied to modes $i$ and $j$ with quadrature vector $\xi = (x_i, p_i, x_j, p_j )^T$ implements the transformation
\[  \begin{pmatrix} \x'_i \\ \p'_i \\ \x'_j \\ \p'_j \end{pmatrix} = 
\begin{pmatrix} \cos(\theta) \mathbbm{1} & -e^{-i\phi}\sin(\theta) \mathbbm{1}\\
e^{i\phi}\sin(\theta) \mathbbm{1} & \cos(\theta) \mathbbm{1} \end{pmatrix} \begin{pmatrix} \x_i \\ \p_i \\ \x_j \\ \p_j \end{pmatrix},
\]
or
\[ \xi' = B(\theta, \phi) \xi .\]
...
Compared to $B(\theta, \phi)$, this transformation changes the sign of the quadratures of the first mode and `swaps' the quadratures of the two modes.\\

We have that
\[ \partial_{\theta} B(\theta, \phi) =B(\frac{\pi}{2} , \phi ) B(\theta , \phi ) = B(\theta +\frac{\pi}{2}, \phi ), \]
in other words, we either have to insert a constant beam splitter gate, or alternatively shift the first parameter by $\frac{\pi}{2}$ to get the derivative circuit.\\
...
When considering unitary transformations and universal quantum computing, we also have a phase rotation gate,
\[
\begin{pmatrix}
\x'\\
\p'
\end{pmatrix} =  \begin{pmatrix} \cos(\phi) & \sin(\phi) \\
-\sin(\phi) & \cos(\phi) \end{pmatrix}\begin{pmatrix} \x\\ \p \end{pmatrix}, \]
or summarised, 
\[ \xi' = R(\phi ) \xi. \]
The derivative of $R(\phi)$ with respect to $\phi$ is
...
\subsection{Derivatives of ``Gaussian expectations''}

Assume we want to take the derivative of the expectation values
\[ \bra{0}U^{\dagger} \G V^{\dagger} \hat{x} V \G U \ket{0}\]
and
\[ \bra{0}U^{\dagger} \G V^{\dagger} \hat{p} V \G U \ket{0}\]
of a single mode with respect to the parameter of a Gaussian gate $\G$, where $U,V$ are CV circuits applied before and after this gate. To simplify the notation, we absorb $U$ into the quantum state, and $V$ into the operator and compute $ \bra{\psi} \G \hat{c} \G \ket{\psi}$ and $\bra{\psi} \G \hat{d} \G \ket{\psi} $, instead, with $\hat{c} = V^{\dagger} \x V$, $\hat{d} = V^{\dagger} \p V$, and $\ket{\psi}= U \ket{0}$.\\

We have seen above how the derivative of $\G$ can always be decomposed into $\sum_{k=1}^K a_k A_k$ where $A_k$ are Gaussian gates. We consequently need to compute the values
\begin{eqnarray*}
	&&a_k\sum_{k=1}^{K} \bra{\psi} A_k^{\dagger} \hat{c} \G \ket{\psi} +  \bra{\psi}\G^{\dagger} \hat{c} A_k  \ket{\psi}  \\
	&&a_k \sum_{k=1}^{K} \bra{\psi} A_k^{\dagger} \hat{d} \G \ket{\psi} +   \bra{\psi}\G^{\dagger} \hat{d} A_k  \ket{\psi}
\end{eqnarray*} 
for $k=1,...,K$. Let us look at one of these terms for the position operator, 
\begin{equation}
	\bra{\psi} A_k^{\dagger} \hat{c} \G \ket{\psi} +  \bra{\psi}\G^{\dagger} \hat{c} A_k  \ket{\psi}.
	\label{Eq:term_to_evaluate}
\end{equation}
We can estimate the term on a quantum computer as follows. Evaluate  
\[E_1 = \bra{\psi} (\G + A_k)^{\dagger} \hat{c} (\G + A_k) \ket{\psi},\]
and
\[E_2= \bra{\psi}(\G - A_k)^{\dagger}  \hat{c} (\G - A_k) \ket{\psi} \]
on the quantum device. Classically compute $\frac{1}{2}E_1 - \frac{1}{2} E_2$. This is exactly the term (\ref{Eq:term_to_evaluate}) we seek to estimate.
\begin{proof}
We have 
\begin{eqnarray*}
	E_1 = &&\bra{\psi} \G^{\dagger} \hat{c} \G \ket{\psi} + \bra{\psi} A_k^{\dagger} \hat{c} A_k \ket{\psi} \\
	&+& \bra{\psi} \G^{\dagger} \hat{c} A_k \ket{\psi}+ \bra{\psi} A_k^{\dagger} \hat{c} \G \ket{\psi}
\end{eqnarray*}
and
\begin{eqnarray*}
	E_2 = && \bra{\psi} \G^{\dagger} \hat{c} \G \ket{\psi} + \bra{\psi} A_k^{\dagger} \hat{c} A_k \ket{\psi} \\
	&-& \bra{\psi} \G^{\dagger} \hat{c} A_k \ket{\psi}- \bra{\psi} A_k^{\dagger} \hat{c} \G \ket{\psi}
\end{eqnarray*}
Subtracting the terms cancels the first two expectation values in $E_1$ and $E_2$ and adds the second ones, and the normalisation of $\frac{1}{2}$ ensures that we have a prefactor of $1$.
\end{proof}

We show now how to estimate the expectations $E_1, E_2$ for single qubit gates on a CV quantum computer with the help of an additional vacuum mode and beam splitters. We use the Heisenberg picture. \\

Starting with $\ket{\psi}$ in the first mode (which just means that we already applied circuit $U$), and vacuum in the second, we apply the circuit shown in Fig. \ref{Fig:der_eval_cv}. The circuit consists of a 50-50 beam splitter, a Gaussian transformation on each mode separately, as well as another 50-50 beam splitter. We measure the expectation of the vector of the transformed quadrature operators $\c, \d$. \\


\begin{figure}[t]
$$
\Qcircuit @C=1em @R=.7em {
\lstick{\ket{\psi}} &  \multigate{1}{BS(\frac{\pi}{4})}  & \gate{A}&  \multigate{1}{BS(\frac{\pi}{4})} &  \meter\\
\lstick{\ket{0}} &   \ghost{BS(\frac{\pi}{4})}   & \gate{\G}&\ghost{BS(\frac{\pi}{4})} &  \\
}
$$
...
In the Heisenberg picture, the two modes are described by their quadrature operators,
\[\begin{pmatrix} \c \\ \d \\ \x \\ \p \end{pmatrix}, \]
where we use $\x$ and $\p$ to describe the quadratures of the second mode (which does not get transformed by a subsequent circuit $V$). 
The first beam splitter tranforms this vector to
\[\frac{1}{\sqrt{2}}\begin{pmatrix} \c - \x \\ \d - \p \\ \c + \x \\ \d + \p \end{pmatrix}.\]
...
\[\underbrace{\begin{pmatrix} s_{11} & s_{12}\\ s_{21} & s_{22} \end{pmatrix}}_{S} \begin{pmatrix} \c - \x \\ \d - \p \end{pmatrix} + \underbrace{\begin{pmatrix} \alpha_1 \\ \alpha_2 \end{pmatrix}}_{\alpha}, \]
and let the trafo describing gate $\G$ acting on the second mode be given by
\[\underbrace{\begin{pmatrix} r_{11} & r_{12}\\ r_{21} & r_{22} \end{pmatrix}}_{R} \begin{pmatrix} \c + \x \\ \d + \p \end{pmatrix} + \underbrace{\begin{pmatrix} \beta_1 \\ \beta_2 \end{pmatrix}}_{\beta}.\]
Overall, the evolution is given by
\[\frac{1}{\sqrt{2}}\begin{pmatrix} S & 0 \\ 0 & R \end{pmatrix}
\begin{pmatrix} \c - \x \\ \d - \p \\ \c + \x \\ \d + \p \end{pmatrix} + \begin{pmatrix}\alpha_1\\ \alpha_2 \\ \beta_1 \\ \beta_2 \end{pmatrix}.\]
This yields
\[\frac{1}{\sqrt{2}}\begin{pmatrix} s_{11}(\c - \x) + s_{12}(\d - \p) + \alpha_1 \\ s_{21}(\c - \x) + s_{22}(\d - \p)  + \alpha_2 \\ r_{11}(\c + \x) + r_{12}(\d + \p)  + \beta_1 \\ r_{21}(\c + \x) + r_{22}(\d + \p) + \beta_2  \end{pmatrix}.\]
After the second beam splitter, we get
\begin{widetext}
\[\frac{1}{2}\begin{pmatrix}(s_{11}- r_{11})\c +(s_{12}- r_{12})\d  -(s_{11}+ r_{11})\x - (s_{12}+ r_{12}) \p  +  \alpha_1 - \beta_1\\ 
(s_{21}- r_{21}) \c+ (s_{22}- r_{22}) \d - (s_{21}+ r_{21}) \x -(s_{22}+ r_{22}) \p  + \alpha_2  - \beta_2 \\ 
(s_{11}+r_{11}) \c + (s_{12}+r_{12}) \d + (r_{11}-s_{11}) \x + (r_{12}-s_{12}) \p + \alpha_1   + \beta_1\\ 
(s_{21}+r_{21}) \c + ( s_{22}+ r_{22}) \d + (r_{21}-s_{21}) \x + ( r_{22}- s_{22}) \p + \alpha_2  + \beta_2  \end{pmatrix}\]
\end{widetext}
which corresponds to following overall transformation on the first mode:
\[\begin{pmatrix} \c \\ \d \end{pmatrix} \rightarrow \frac{1}{2} \bigg[ (S-R) \begin{pmatrix} \c \\ \d \end{pmatrix} - (S+R) \begin{pmatrix} \x \\ \p \end{pmatrix} + \alpha - \beta \bigg]\]
...
\begin{multline*}
	\frac{1}{2} \bigg[ (s_{11}-r_{11}) \bra{\psi}\c\ket{\psi} + (s_{12}-r_{12}) \bra{\psi}\d\ket{\psi} + \alpha_1 - \beta_1 \\+ (s_{11}+r_{11}) \bra{0}\x\ket{0} + (s_{12}+r_{12}) \bra{0}\p\ket{0} \bigg]
\end{multline*}
for the ``position'' quadrature, and
\begin{multline*}
	\frac{1}{2} \bigg[ (s_{21}-r_{21}) \bra{\psi}\c\ket{\psi} + (s_{22}-r_{22}) \bra{\psi}\d\ket{\psi} + \alpha_1 - \beta_1 \\+ (s_{21}+r_{21}) \bra{0}\x\ket{0} + (s_{22}+r_{22}) \bra{0}\p\ket{0} \bigg]
\end{multline*}
for the ``momentum'' quadrature. The expectations $\bra{0}\x\ket{0}$ and $\bra{0}\p\ket{0}$ evaluate to zero and the operation is therefore equivalent to the expectation of $(\c, \d)^{\dagger}$ with respect to $\ket{\psi} \otimes \ket{0}$ after the (non-symplectic) transformation 
\[(S-R)\begin{pmatrix} \c \\ \d \end{pmatrix} + \alpha - \beta.\]
To get the sum of the two, we simply consider the expectation of the second mode. This trick should work for any other operator $\hat{O}$ with $\bra{0} \hat{O} \ket{0} = \bra{0} \hat{O}^{\dagger} \ket{0}=0$.\\

IS THERE A NORMALIZATION SOMEWHERE? HOW DOES POSTSELECTION WITH PATH-ENCODED ANCILLAS WORK?\\
