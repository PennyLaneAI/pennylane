...
What happens if $W, V \neq \mathbbm{1}$ ? $W$, the circuit applied before the gate $\G$ that we derive for, can be arbitrary, since the derivation of the coefficients $c_{ij}$ in Equation (\ref{Eq:omega_gaussian}) does not change if the $\hat{O}_j$ are linear combinations of other monomials. Howeverm $V$, the circuit applied after $\G$, plays a role. If $V$ is non-Gaussian, it mixes higher-order monomials $j>2$ for which the rules in Table \ref{Tbl:cv_gate_derivatives} are not valid since we did not consider higher order coefficients $c_{ij}$. If the variational circuit is parametrised in all gates, we can only use this trick to compute derivatives for the parameters up to the first non-Gaussian gate in the circuit. \\
