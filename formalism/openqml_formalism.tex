...
\subsection{Gaussian circuits}

We now show that for Gaussian gates and homodyne measurements, the derivative of the quadrature expectation can be expressed by estimating the expectations of slightly different circuits. This is done using the quadrature operator formalism with the Heisenberg picture, where a Gaussian gate applied to the (evolved) quadratures $\hat{q} =\hat{c}, \hat{d}$ is expressed by 
\[ \G(\mu) \hat{q} \G^{\dagger}(\mu)  = a(\mu)  \c + b(\mu) \d + \alpha (\mu). \]
Therefore, 
\[\partial_{\mu} \left( \G(\mu) \hat{q} \G^{\dagger}(\mu)\right)  = \partial_{\mu}a(\mu)  \c + \partial_{\mu}b(\mu) \d + \partial_{\mu}\alpha (\mu). \]
If we can express the right side as
\[ \sum_{k=1}^K \gamma_k (\tilde{a}_k(\mu)  \c + \tilde{b}_k(\mu) \d + \tilde{\alpha}_k (\mu)) = \sum_{k=1}^K \gamma_k (A_k(\mu)^{\dagger} \hat{q} A_k), \]
we get
\[\partial_{\mu}\bra{\psi} \G(\mu)^{\dagger} \hat{q} \G(\mu) \ket{\psi} = \sum_{k=1}^K \gamma_k \bra{\psi} A_k(\mu)^{\dagger} \hat{q} A_k(\mu) \ket{\psi}, \]
and, as in the case of Pauli gates, the derivative of the expectation is a wighed sum of expectations with slightly different gates.\\

We will now show that for standard Gaussian gates one can indeed  express their derivative as a linear combination. For this we use the Bogoliubov transform of the entire quadrature vector.


\paragraph{Phase rotation}
The Bogoliubov transform $ \mathcal{B}_R(\phi)$ of the phase rotation gate, acting as
\[
\begin{pmatrix}
\x'\\
\p'
...
0 & 1 & b_{\mathrm{re}} \\
0 & 0 & 1
\end{pmatrix}}_{\mathcal{B}_D}
\begin{pmatrix}
\x \\ \p \\ \mathbbm{1}
\end{pmatrix}.
\end{equation}
The derivative of the Bogoliubov transform $\mathcal{B}_D$ with respect to $b_{\mathrm{im}}$ and $b_{\mathrm{re}}$ is given by 
\begin{eqnarray*}
\partial_{b_{\mathrm{re}}} \mathcal{B}_D(b_{\mathrm{re}}, b_{\mathrm{im}}) &=& \begin{pmatrix}
0 & 0 & 0  \\
0 & 0 & 1 \\
0 & 0 & 0
\end{pmatrix},\\
\partial_{b_{\mathrm{im}}} \mathcal{B}_D(b_{\mathrm{re}}, b_{\mathrm{im}}) &=& \begin{pmatrix}
0 & 0 & 1  \\
0 & 0 & 0 \\
0 & 0 & 0
\end{pmatrix}.
...
\begin{eqnarray*}
\partial_{b_{\mathrm{re}}} \mathcal{B}_D(b_{\mathrm{re}}, b_{\mathrm{im}}) &=& \frac{1}{2} \mathcal{B}_D(1, 0) - \frac{1}{2} \mathcal{B}_D(-1, 0) \\
\partial_{b_{\mathrm{im}}} D(b_{\mathrm{re}}, b_{\mathrm{im}}) &=& \frac{1}{2} \mathcal{B}_D(0, 1) - \frac{1}{2} \mathcal{B}_D(0, -1)
\end{eqnarray*}
