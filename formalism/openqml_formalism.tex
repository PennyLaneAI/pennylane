...
$G = \theta B$, the commutator $\ad_G B = 0$
and the series~$Q$ terminates after the first term, yielding
\be
\pd{}{\theta} \Ad_{U_{\theta}} \rho
  = -i \ad_B \Ad_{U_{\theta}} \rho.
\ee
...
\ville{The usefulness of this formula depends on whether we can sum the series~$Q_k$ easily enough.
  For example, for the displacement gate~$D(\re{z}, \im{z})$ it terminates after the second term
  (and even that second term is proportional to identity, so it does not matter to the $\ad$).
