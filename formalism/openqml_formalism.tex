...
The unitary and its generator commute, and thus we can obtain
the derivative of a propagated state~$\rho$ as
\begin{align}
  \notag
  \pd{}{\theta} \Ad_{U_\theta} \rho
  &= \pd{}{\theta} (U_\theta \rho U_\theta^\dagger) = -i [G, U_\theta \rho U_\theta^\dagger]\\
  &= -i \ad_G \Ad_{U_\theta} \rho = -i \Ad_{U_\theta} \ad_G \rho.
\end{align}
...
\ee
With $U=B U_\theta A$, the derivative of the expectation value is given by
\begin{align}
  \label{eq:d_ev}
  \notag
  \pd{}{\theta} \expect{Q}_{B U_\theta A}
  &= \tr(Q \Ad_B (\pd{}{\theta} \Ad_{U_\theta}) \Ad_A \rho)\\
  &= \tr(Q \Ad_B (-i \ad_G) \Ad_{U_\theta A} \rho).
\end{align}

\ville{The reason why I'm looking into the periodicity of the subgroup $U_\theta$ is that I was originally
  overoptimistic and though that periodicity might be sufficient to make the gradient trick work. It probably isn't.
...
In quantum mechanics the global phase of a propagator is meaningless, so we may always add
