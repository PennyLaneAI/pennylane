...
\tikzset{quantum node/.style={draw, align=center, anchor=west,rounded corners=4pt, inner sep=5pt, rectangle split, rectangle split horizontal, rectangle split parts=2, rectangle split part fill={quantum1,quantum2}, every two node part/.style={text = white}}}
\tikzset{classical node/.style={draw, rectangle,align=center, anchor=west,rounded corners=4pt, thin, fill=classical, inner sep=5pt}}
...
\draw[->] (c) -- (3,0)  node[out label] {$y_{c}$};
\end{tikzpicture}
\end{figure}
...
\centering
\begin{tikzpicture}
\node[quantum node] (c) at (0,0) {$\ket{\psi(\x_c, \theta_c)} $ \nodepart{two} $O $};
\draw[->] (c) -- (3,0)  node[out label] {$y_{q}$};
\end{tikzpicture}
\end{figure}
...
\begin{figure}[h]
\begin{tikzpicture}
\node[draw, thick, dotted, rectangle, anchor=west, minimum width = 6cm, minimum height = 3cm] (q1) at (-0.3,-1) {};
\node[quantum node] (q1) at (0,0) {$\ket{\psi}$ \nodepart{two} $O$};
\node[]  at (0.5,-1) {$\vdots $};
\node[quantum node] (qK) at (0,-2) {$\ket{\psi}$ \nodepart{two} $O$};
\node[classical node] (c) at (3,-1) {$\sum_{k=1}^{K} a_k y_{q_k} $};
\node[output node] (o) at (5.5,-1) {$y$};
\draw[] (q1) -- (2.5,0)  node[out label] {$y_{q_1}$};
\draw[->] (2.5,0) -- (2.5,-1)-- (c);
\draw[] (qK) -- (2.5,-2) node[out label] {$y_{q_K}$};
\draw[->] (2.5,-2) -- (2.5,-1)-- (c);
\draw[->] (c) -- (o);
\node (=) at (6.2,-1) {$=$};
\node[expectation node] (q1) at (6.5,-1) {$\ket{\psi}$ \nodepart{two} $O$};
...
\node[expectation node] (q1) at (0,0) {$\ket{\psi(\theta_1)} $ \nodepart{two} $\sigma_x$};
\node[]  at (1,-0.75) {$\vdots $};
\node[expectation node] (qK) at (0,-1.5) {$\ket{\psi(\theta_K)} $ \nodepart{two} $\sigma_x \otimes \sigma_y$};
\node[]  at (1,-2.25) {$\vdots $};

\node[classical node] (c) at (4.25,-1) {$\sum_k h_k y_{q_k} $};
\node[output node] (o) at (6.5,-1) {$y$};
\draw[->] (i1) -- (q1);
\draw[->] (iK) -- (qK) ;
\draw[] (q1) -- (3.7,0)  node[out label] {$y_{q_1}$};
\draw[->] (3.7,0) -- (3.7,-1)-- (c);
\draw[] (qK) -- (3.7,-1.5) node[out label] {$y_{q_K}$};
\draw[->] (3.7,-1.5) -- (3.7,-1)-- (c);
\draw[->] (c) -- (o);
%\draw[->] (3,-0.75) -- (3.7,-0.75)-- (3.7,-1) --(c);
%\draw[->] (3,-2.25) -- (3.7,-2.25)-- (3.7,-1) -- (c);
...
\node[expectation node] (q) at (8.5,0) {$ \ket{\psi(\mathbf{y}_c, \theta_q)} $ \nodepart{two} $\sigma_z$};
\node[output node] (o) at (12,0) {$y$};
\draw[->] (i) -- (c1);
...
(c)\\ 
\begin{tikzpicture}
\node[input node] (i) at (-0.5,0) {$\emptyset$};
\node[quantum node] (q) at (0,0) {$ \ket{\psi(\theta)} $ \nodepart{two} $\sigma_z$};
\node[output node] (o) at (3,0) {$y$};
\draw[->] (i) -- (q);
\draw[->] (q) -- (o);
\end{tikzpicture}

\end{flushleft}
\caption{Examples of model graphs. Turqois nodes depict quantum nodes, and orange nodes depict classical nodes. (a) Model graph of a variational quantum eigensolver. (b) Model graph of a classical neural network with a final quantum layer. (c) Generative quantum model. Explain...}
