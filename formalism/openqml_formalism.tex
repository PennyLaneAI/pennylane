...
For second-order observables $Q = \sum_{ij} q_{ij} E_i E_j$ we obtain
\begin{align*}
...
\notag
&= \sum_{ijx} c_x q_{ij} \tr(\Ad_{B U_{\vt} A} \rho \ ((\Ad_{(B U_{\vR_x} B^\dagger)^\dagger} E_i) E_j +E_i (\Ad_{(B U_{\vR_x} B^\dagger)^\dagger} E_j)))\\
\notag
...
&= \sum_{ijkx} c_x q_{ij} \expect{(\Admap{Z_x}_{ik} E_k) E_j +E_i (\Admap{Z_x}_{jk} E_k)}_{B U_{\vt} A},
\end{align}
\end{widetext}
where $Z_x = B U_{\vR_x} B^\dagger$.
This formula is not as easy to use as Eq.~\eqref{eq:d_ev1_cv}
since we have to transform the observables using $\Admap{Z_x}$
...
Let us denote the 2D rotation matrix by
\be
\hat{R}(\phi) =
\begin{pmatrix}
  \cos(\phi) & -\sin(\phi)\\
...
\begin{align*}
\Admap{D}(\sqrt{\frac{\lambda}{2}} \: z) = D(r, \phi) =
\begin{pmatrix}
  1 & 0 & 0\\
  r \cos \phi & 1 & 0\\
  r \sin \phi & 0 & 1
...
\begin{align*}
\pd{}{\re z} \Admap{D}(\sqrt{\frac{\lambda}{2}} \: z)
=
\begin{pmatrix}
  0 & 0 & 0\\
  1 & 0 & 0\\
  0 & 0 & 0
\end{pmatrix}
= \frac{1}{2s} (\Admap{D}(\sqrt{\frac{\lambda}{2}} \: (s, 0)) -\Admap{D}(\sqrt{\frac{\lambda}{2}} \: (-s, 0)))
\end{align*}
Likewise for $\im z$.
Alternatively, using the polar coordinates,
\begin{align*}
\pd{}{r} \Admap{D}(r, \phi)
=
\begin{pmatrix}
...
\begin{align*}
\pd{}{r} \Admap{S}(r, \phi)
&= \frac{1}{2 \sinh a} (S(r+a, \phi) -S(r-a, \phi)).
\end{align*}
We also obtain
\begin{align*}
\pd{}{\phi} \Admap{S}(r, \phi)
&= \frac{1}{2}(S(r, \phi+\pi/2) -S(r, \phi-\pi/2)).
\end{align*}
...
\begin{pmatrix}
  1 & 0 & 0 & 0 & 0\\
  0 & \block(2,2){\cos(\theta)\I} & \block(2,2){\sin(\theta)\hat{R}(\phi)}\\
  0\\
  0 & \block(2,2){-\sin(\theta)\hat{R}^\dagger(\phi)} & \block(2,2){\cos(\theta)\I}\\
  0
\end{pmatrix},
\end{align*}
since
...
\begin{pmatrix}
  1 & 0 & 0 & 0 & 0\\
  0 & \block(2,2){\hat{R}(\phi)} & \block(2,2){0}\\
  0\\
  0 & \block(2,2){0} & \block(2,2){\I_2}\\
  0
\end{pmatrix}.
\end{align*}
Now we obtain
