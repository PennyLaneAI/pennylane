...
from pennylane.measurements import MeasurementValue, Sample
...
class TestSample:
    ...
    def test_sample_dimension(self, mocker, n_sample):
        ...
        assert len(output) == 2
        assert circuit._qfunc_output[0].shape(n_sample, 2) == (
            (n_sample,) if not n_sample == 1 else ()
        )
        assert circuit._qfunc_output[1].shape(n_sample, 2) == (
            (n_sample,) if not n_sample == 1 else ()
        )
    ...
    def test_sample_combination(self, mocker):
        ...
        assert circuit._qfunc_output[0].shape(n_sample, 3) == (n_sample,)
    ...
    def test_single_wire_sample(self, mocker):
        ...
        assert circuit._qfunc_output.shape(n_sample, 1) == (n_sample,)
    ...
    def test_multi_wire_sample_regular_shape(self, mocker):
        ...
        assert circuit._qfunc_output[0].shape(n_sample, 3) == (n_sample,)
        assert circuit._qfunc_output[1].shape(n_sample, 3) == (n_sample,)
        assert circuit._qfunc_output[2].shape(n_sample, 3) == (n_sample,)
