...
class TestProbs:
    ...
    def test_probs_empty_wires(self):
        """Test that using ``qp.probs`` with an empty wire list raises an error."""
        with pytest.raises(ValueError, match="Cannot set an empty list of wires."):
            qp.probs(wires=[])

    @pytest.mark.parametrize("shots", [None, 100])
    def test_probs_no_arguments(self, shots):
        """Test that using ``qp.probs`` with no arguments returns the probabilities of all wires."""
        ...
        assert qp.math.allequal(res, [1, 0, 0, 0, 0, 0, 0, 0])
    ...
    def test_full_prob(self, init_state, tol, seed):
        ...
        @qp.qnode(dev)
        def circuit():
            qp.StatePrep(state, wires=list(range(4)))
    ...
    def test_process_state(self, interface, subset_wires, expected):
        ...
        state = qp.math.array([1 / 2, 0] * 4, like=interface)
        wires = qp.wires.Wires(range(3))
        subset_probs = qp.probs(wires=subset_wires).process_state(state, wires)
        assert subset_probs.shape == (len(expected),)
        assert qp.math.allclose(subset_probs, expected)
    ...
    def test_process_state_batched(self, interface, subset_wires, expected):
        ...
        states = qp.math.array([[1 / 2, 0] * 4, [0, 1 / 2] * 4], like=interface)
        wires = qp.wires.Wires(range(3))
        subset_probs = qp.probs(wires=subset_wires).process_state(states, wires)
        assert subset_probs.shape == qp.math.shape(expected)
        assert qp.math.allclose(subset_probs, expected)
    ...
    def test_process_density_matrix_basic(self, interface):
        ...
        dm = qp.math.array([[0.5, 0], [0, 0.5]], like=interface)
        wires = qp.wires.Wires(range(1))
        expected = qp.math.array([0.5, 0.5], like=interface)
        calculated_probs = qp.probs().process_density_matrix(dm, wires)
        assert qp.math.get_interface(calculated_probs) == interface
        assert qp.math.allclose(calculated_probs, expected)
    ...
    def test_process_density_matrix_subsets(self, interface, subset_wires, expected):
        ...
        subset_probs = qp.probs(wires=subset_wires).process_density_matrix(dm, wires)
        assert qp.math.get_interface(subset_probs) == interface
        assert subset_probs.shape == qp.math.shape(expected)
        assert qp.math.allclose(subset_probs, expected)
    ...
    def test_process_density_matrix_batched(self, interface, subset_wires, expected):
        ...
        subset_probs = qp.probs(wires=subset_wires).process_density_matrix(dm_batch, wires)

        expected = qp.math.array(expected, like=interface)
    ...
    def test_integration_jax(self, tol_stochastic, shots, obs, params, seed):
        ...
        def circuit(x):
            qp.PhaseShift(x, wires=1)
            qp.RX(x, wires=1)
            qp.PhaseShift(x, wires=1)
            qp.CNOT(wires=[0, 1])
            if isinstance(obs, Sequence):
                return qp.probs(wires=obs)
            return qp.probs(op=obs)
    ...
    def test_composite_measurement_value_not_allowed(self):
        ...
        with pytest.raises(ValueError, match=r"Cannot use qp.probs\(\) when measuring multiple"):
            _ = qp.probs(op=m0 + m1)
    ...
    @pytest.mark.parametrize(
        ...
        [
            (None, [1, 0]),
            (qp.StatePrep([[0, 1], [1, 0]], 0), [[0, 1], [1, 0]]),
        ],
    )
    def test_process_state_tf_autograph(self, prep_op, expected):
        ...
        def probs_from_state(state):
            return qp.probs(wires=wires).process_state(state, wires)

        state = qp.devices.qubit.create_initial_state(wires, prep_operation=prep_op)
    ...
    def test_numerical_analytic_diff_agree(self, tol):
        ...
        w = 4
        dev = qp.device("default.qubit", wires=w)

        def circuit(x, y, z):
            for i in range(w):
                qp.RX(x, wires=i)
                qp.PhaseShift(z, wires=i)
                qp.RY(y, wires=i)
        ...
        circuit_F = qp.QNode(circuit, dev, diff_method="finite-diff")
        circuit_A = qp.QNode(circuit, dev, diff_method="parameter-shift")
        res_F = qp.jacobian(circuit_F)(*params)
        res_A = qp.jacobian(circuit_A)(*params)
    ...
    def test_prob_generalize_param_one_qubit(self, hermitian, tol):
        ...
        def circuit(x):
            qp.RZ(x, wires=0)
            return qp.probs(op=qp.Hermitian(hermitian, wires=0))

        res = circuit(0.56)

        def circuit_rotated(x):
            qp.RZ(x, wires=0)
            qp.Hermitian(hermitian, wires=0).diagonalizing_gates()

        state = np.array([1, 0])
        matrix = qp.matrix(circuit_rotated, wire_order=[0])(0.56)
    ...
    def test_prob_generalize_param(self, hermitian, tol):
        ...
        matrix = qp.matrix(circuit_rotated, wire_order=[0, 1, 2])(0.56, 0.1)
        state = np.dot(matrix, state)
    ...
    def test_prob_generalize_param_multiple(self, hermitian, tol):
        ...
        def circuit(x, y):
            ...
            qp.CNOT(wires=[0, 2])
            return (
                qp.probs(op=qp.Hermitian(hermitian, wires=0)),
                qp.probs(wires=[1]),
                qp.probs(wires=[2]),
            )
    ...
    def test_prob_generalize_initial_state(self, hermitian, wire, init_state, tol, seed):
        ...
        def circuit():
            ...
            qp.PauliX(wires=2)
            qp.PauliX(wires=3)
            return qp.probs(op=qp.Hermitian(hermitian, wires=wire))
        ...
        def circuit_rotated():
            ...
            qp.PauliX(wires=2)
            qp.PauliX(wires=3)
            qp.Hermitian(hermitian, wires=wire).diagonalizing_gates()

        matrix = qp.matrix(circuit_rotated, wire_order=[0, 1, 2, 3])()
        state = np.dot(matrix, state)
        expected = np.reshape(np.abs(state) ** 2, [2] * 4)
    ...
    def test_warning_hermitian(self):
        ...
        def circuit():
            qp.H(0)
            return qp.probs(op=qp.Hermitian(H, wires=0))

        with pytest.warns(
            UserWarning,
            match="Using qp.probs with a Hermitian observable might return different results than expected",
        ):
            circuit()

    @pytest.mark.parametrize("operation", [qp.PauliX, qp.PauliY, qp.Hadamard])
    @pytest.mark.parametrize("wire", [0, 1, 2, 3])
    def test_operation_prob(self, operation, wire, init_state, tol, seed):
        ...
        def circuit():
            ...
            qp.PauliY(wires=2)
            qp.PauliZ(wires=3)
            return qp.probs(op=operation(wires=wire))
    ...
    @pytest.mark.parametrize("observable", [(qp.PauliX, qp.PauliY)])
    def test_observable_tensor_prob(self, observable, init_state, tol, seed):
        ...
        def circuit():
            ...
            qp.PauliY(wires=2)
            qp.PauliZ(wires=3)
            return qp.probs(op=observable[0](wires=0) @ observable[1](wires=1))
    ...
    @pytest.mark.parametrize("coeffs, obs", [([1, 1], [qp.PauliX(wires=0), qp.PauliX(wires=1)])])
    ...
    @pytest.mark.parametrize(
        "operation", [qp.SingleExcitation, qp.SingleExcitationPlus, qp.SingleExcitationMinus]
    )
    def test_generalize_prob_not_hermitian(self, operation):
        ...
        def circuit():
            qp.PauliX(wires=0)
            qp.PauliZ(wires=1)
            return qp.probs(op=operation(0.56, wires=[0, 1]))
    ...
    def test_prob_wires_and_hermitian(self, hermitian):
        ...
        def circuit():
            qp.PauliX(wires=0)
            return qp.probs(op=qp.Hermitian(hermitian, wires=0), wires=1)
    ...
    def test_estimate_probability_with_binsize_with_broadcasting(self, wires, expected):
        ...
        res = qp.probs(wires=wires).process_samples(
    ...
    def test_process_samples_shot_range(self):
        ...
        _ = qp.probs(wires=0).process_samples(
            samples, wire_order=qp.wires.Wires((0, 1)), shot_range=(0, 10)
        )
    ...
    def test_estimate_probability_with_counts(self, wires, expected):
        ...
        wire_order = qp.wires.Wires((2, 1, 0))

        res = qp.probs(wires=wires).process_counts(counts=counts, wire_order=wire_order)
    ...
    def test_non_commuting_probs_does_not_raises_error(self):
        ...
        def circuit(x, y):
            ...
            qp.RY(y, wires=[1])
            qp.CNOT(wires=[0, 1])
            return qp.expval(qp.PauliX(1)), qp.probs(wires=[0, 1])

        res = circuit(1, 2)
        assert isinstance(res, tuple) and len(res) == 2

    def test_commuting_probs_in_computational_basis(self):
        """Test that `qp.probs` can be used in the computational basis with other commuting observables."""
        dev = qp.device("default.qubit", wires=5)
        ...
        assert res[0] == res2
        assert qp.math.allequal(res[1:], res3)
