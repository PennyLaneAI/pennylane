...
class TestSample:
    ...
    def test_sample_dimension(self, n_sample):
        ...
        assert qp.sample(qp.PauliZ(0)).shape(shots=n_sample, num_device_wires=2) == ((n_sample,))
        assert qp.sample(qp.PauliX(1)).shape(shots=n_sample, num_device_wires=2) == ((n_sample,))
    ...
    def test_sample_output_type_in_combination(self):
        ...
        @qp.qnode(dev, diff_method="parameter-shift")
        def circuit():
            return qp.expval(qp.PauliZ(0)), qp.var(qp.PauliZ(1)), qp.sample(qp.PauliZ(2))
    ...
    def test_providing_observable_and_wires(self):
        ...
        def circuit():
            qp.Hadamard(wires=0)
            return qp.sample(qp.PauliZ(0), wires=[0, 1])
    ...
    def test_providing_no_observable_and_no_wires_shot_vector(self):
        ...
        @qp.set_shots([shots1, shots2, shots3])
        @qp.qnode(dev)
        def circuit():
            qp.Hadamard(wires=0)
            qp.CNOT(wires=[0, 1])
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (qp.Hadamard(0)),
            (qp.Identity(0)),
            (qp.Hermitian(np.diag([1, 2]), 0)),
            (qp.Hermitian(np.diag([1.0, 2.0]), 0)),
            ...
            (qp.Projector([0, 1], wires=[0, 1]) @ qp.PauliZ(2)),
            (qp.Hermitian(np.array(np.eye(2)), wires=[0]) @ qp.PauliZ(2)),
            (qp.Projector([0, 1], wires=[0, 1]) @ qp.Hermitian(np.array(np.eye(2)), wires=[2])),
        ],
    )
    def test_numeric_type(self, obs):
        ...
        res = qp.sample(obs) if obs is not None else qp.sample()
    ...
    def test_sample_empty_wires(self):
        """Test that using ``qp.sample`` with an empty wire list raises an error."""
    ...
    def test_sample_no_arguments(self, shots):
        """Test that using ``qp.sample`` with no arguments returns the samples of all wires."""
        dev = qp.device("default.qubit", wires=3)
    ...
    def test_new_sample_with_operator_with_no_eigvals(self):
        ...
        with pytest.raises(EigvalsUndefinedError, match="Cannot compute samples of"):
            qp.sample(op=DummyOp(0)).process_samples(samples=np.array([[1, 0]]), wire_order=[0])
    ...
    def test_process_samples_dtype(self, coeffs, dtype):
        ...
        processed_samples = qp.sample(coeffs * qp.X(0), dtype=dtype).process_samples(
            samples, wire_order=[0]
        )
        assert processed_samples.dtype == np.dtype(dtype)

    def test_sample_allowed_with_parameter_shift(self):
        """Test that qp.sample doesn't raise an error with parameter-shift and autograd."""
        dev = qp.device("default.qubit")
        ...
        assert qp.math.shape(res) == (10,)
        assert all(r in {-1, 0, 1} for r in np.round(res, 13))

    @pytest.mark.jax
    def test_sample_with_jax_jacobian(self):
        """Test that qp.sample executes with parameter-shift and jax."""
        import jax
    ...
    def test_sample_dtype_combined(self, interface, dtype, obs):
        ...
        @qp.set_shots(10)
        @qp.qnode(device=qp.device("default.qubit", wires=1), interface=interface)
        def circuit():
            qp.Hadamard(wires=0)
            return qp.sample(obs, dtype=dtype) if obs is not None else qp.sample(dtype=dtype)

        samples = circuit()
        assert qp.math.get_interface(samples) == interface
        assert qp.math.get_dtype_name(samples) == dtype
...
class TestJAXCompatibility:
    ...
    def test_sample_with_boolean_tracer(self):
        """Test that qp.sample can be used with Catalyst measurement values (Boolean tracer)."""
        import jax
        ...
        def fun(b):
            ...
            assert mp.mv.shape == ()
            assert isinstance(mp.wires, qp.wires.Wires)
            assert mp.wires == ()
    ...
    def test_jitting_with_dtype(self, dtype, obs):
        ...
        @qp.set_shots(10)
        @qp.qnode(device=qp.device("default.qubit", wires=1), interface="jax")
        def circuit(x):
            qp.RX(x, wires=0)
            return qp.sample(obs, dtype=dtype) if obs is not None else qp.sample(dtype=dtype)

        samples = jax.jit(circuit)(jax.numpy.array(0.123))
        assert qp.math.get_interface(samples) == "jax"
        assert qp.math.get_dtype_name(samples) == dtype

    def test_process_samples_with_jax_tracer(self):
        """Test that qp.sample can be used when samples is a JAX Tracer"""

        import jax

        def f(samples):
            return qp.sample(op=2 * qp.X(0)).process_samples(
                samples, wire_order=qp.wires.Wires((0, 1))
            )
...
class TestSampleProcessCounts:
    ...
    def test_process_counts_with_eigen_values(self, wire_order, expected_result):
        ...
        sample_mp = qp.sample(qp.Z(0))
        counts = {"00": 2, "10": 3}
        wire_order = qp.wires.Wires(wire_order)
    ...
    def test_process_counts_with_eigen_values_multiple_wires(self, wire_order, expected_result):
        """Test process_counts method with eigen values."""
        sample_mp = qp.sample(qp.Z(0) @ qp.Z(1))
