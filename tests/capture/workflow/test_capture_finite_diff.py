...
class TestGradients:
    ...
    def test_simple_circuit(self, grad_f, kwargs):
        ...
        @qp.qnode(
            qp.device("default.qubit", wires=1), diff_method="finite-diff", gradient_kwargs=kwargs
        )
        def circuit(x):
            qp.RX(x, 0)
            return qp.expval(qp.Z(0))
    ...
    def test_hessian(self):
        ...
        @qp.qnode(
            qp.device("default.qubit", wires=4),
        ...
        assert qp.math.allclose(hess, -jnp.cos(x), atol=5e-4)  # gets noisy
    ...
    def test_jaxpr_contents(self, argnums):
        ...
        @qp.qnode(
            qp.device("default.qubit", wires=1),
            diff_method="finite-diff",
