...
class TestCaptureWhileLoop:
    ...
    def test_while_loop_dynamic_array(self, array):
        ...
        def fn(arg):
            ...
            @qp.while_loop(lambda *args: args[-1] < 10)
            def loop(a, b, idx):
...
class TestCaptureCircuitsWhileLoop:
    ...
    def test_circuit_closure_vars(self, arg, expected):
        ...
        result = circuit(arg)
        assert qp.math.allclose(result, expected), f"Expected {expected}, but got {result}"
    ...
    def test_while_loop_nested(self, upper_bound, arg, expected):
        ...
        def circuit(upper_bound, arg):
            ...
            @qp.while_loop(lambda i: i < upper_bound)
            def loop_fn(i):
                qp.Hadamard(wires=i)
                return i + 1
            ...
            @qp.while_loop(lambda _, i: i < upper_bound)
...
def test_pytree_input_output():
    ...
    assert qp.math.allclose(out["x"], 10)
...
class TestCaptureWhileLoopDynamicShapes:
    ...
    def test_while_loop_dynamic_shape_array(self):
        ...
        def f(x):
            @qp.while_loop(lambda res: jnp.sum(res) < 10)
            def g(res):
                return res + res
    ...
    def test_array_initialized_with_size_of_other_arg(self):
        ...
        [a_size, final_i, final_a] = qp.capture.eval_jaxpr(jaxpr.jaxpr, jaxpr.consts, 2)
        assert qp.math.allclose(a_size, 2)  # what it was initialized with
    ...
    def test_error_if_combine_with_dynamic_closure_var(self, allow_array_resizing):
        ...
        def w(i0):
            ...
            @qp.while_loop(lambda a: jnp.sum(a) < 10, allow_array_resizing=allow_array_resizing)
            def f(a):
                return c * a
    ...
    def test_loop_with_argument_combining(self, allow_array_resizing):
        ...
        @qp.while_loop(lambda a, b: jnp.sum(a) < 20, allow_array_resizing=allow_array_resizing)
        def f(a, b):
            return a + b, b + 1
        ...
        jaxpr = jax.make_jaxpr(w)(2)
        [dynamic_shape, a, b] = qp.capture.eval_jaxpr(jaxpr.jaxpr, jaxpr.consts, 2)
        assert qp.math.allclose(dynamic_shape, 2)  # the initial size
    ...
    def test_loop_args_resized_together(self, allow_array_resizing):
        ...
        @qp.while_loop(lambda a, b: jnp.sum(a) < 10, allow_array_resizing=allow_array_resizing)
        def f(x, y):
        ...
        assert qp.math.allclose(y, 2 * x_expected)
    ...
    def test_independent_resizing(self, allow_array_resizing):
        ...
        jaxpr = jax.make_jaxpr(w)(2)
        [shape1, shape2, a, b] = qp.capture.eval_jaxpr(jaxpr.jaxpr, jaxpr.consts, 3)
