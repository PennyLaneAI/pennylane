...
def test_null_if_not_enabled():
    ...
    _ = qpjax.make_jaxpr(f)(jnp.eye(4))
...
class TestDyanmicShapes:
    ...
    def test_single_abstract_shape(self):
        ...
        def f(*args):
            ...
            jaxpr = qpjax.make_jaxpr(lambda *args: 0, abstracted_axes=abstracted_axes)(*args)
            _ = qpjax.core.eval_jaxpr(jaxpr.jaxpr, jaxpr.consts, *abstract_shapes, *args)

        _ = qpjax.make_jaxpr(f, abstracted_axes=initial_abstracted_axes)(jnp.arange(4))
    ...
    def test_single_abstract_shape_multiple_abstract_axes(
        ...
        _ = qpjax.make_jaxpr(f, abstracted_axes=initial_abstracted_axes)(jnp.eye(4))
    ...
    def test_pytree_input(self):
        ...
        def f(*args):
            ...
            _ = qpjax.core.eval_jaxpr(jaxpr.jaxpr, jaxpr.consts, *abstract_shapes, *flat_args)

        _ = qpjax.make_jaxpr(f, abstracted_axes=initial_abstracted_axes)(arg)
    ...
    def test_input_created_with_jnp_ones(self):
        ...
        def f(n):
            m = n + 1
            ones = qpjax.numpy.ones((m, 3))
            zeros = qpjax.numpy.zeros((4, n))
...
def test_custom_staging_rule(enable_disable_dynamic_shapes):
    ...
    jaxpr = qpjax.make_jaxpr(workflow)()
    ...
    assert isinstance(jaxpr.jaxpr.outvars[0].aval, qpjax.core.ShapedArray)
    assert isinstance(jaxpr.jaxpr.outvars[1].aval, qpjax.core.DShapedArray)
