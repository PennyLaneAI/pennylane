...
"""Unit tests for dynamic decompositions."""
import numpy as np
...
def _simple_custom_op_resources():
    return {resource_rep(qml.Hadamard): 2}


@register_resources(_simple_custom_op_resources)
def _simple_custom_op_decomposition(wires):
    qml.Hadamard(wires=wires)
    qml.Hadamard(wires=wires)


add_decomps(SimpleCustomOp, _simple_custom_op_decomposition)
...
def _simple_custom_op_return_resources():
    return {resource_rep(qml.Hadamard): 1}


@register_resources(_simple_custom_op_return_resources)
def _simple_custom_op_return_decomposition(wires):
    return qml.Hadamard(wires=wires)


add_decomps(SimpleCustomOpReturn, _simple_custom_op_return_decomposition)
...
def _custom_op_cont_hyperparams_resources():
    return {
        resource_rep(qml.CNOT): 1,
        resource_rep(qml.RX): 3,
        resource_rep(qml.RY): 1,
        resource_rep(qml.RZ): 1,
    }


@register_resources(_custom_op_cont_hyperparams_resources)
def _custom_op_cont_hyperparams_decomposition(wires, *args, **hyperparameters):
    phi = args[0]
...
add_decomps(CustomOpConstHyperparams, _custom_op_cont_hyperparams_decomposition)
...
def _custom_op_multi_wire_resources():
    return {
        resource_rep(qml.CNOT): 2,
        resource_rep(qml.DoubleExcitation): 1,
        resource_rep(qml.RY): 1,
        resource_rep(qml.RZ): 1,
        resource_rep(qml.RX): 2,
    }


@register_resources(_custom_op_multi_wire_resources)
def _custom_op_multi_wire_decomposition(wires, *args, **hyperparameters):
    phi = args[0]
...
add_decomps(CustomOpMultiWire, _custom_op_multi_wire_decomposition)
...
def _custom_op_cond_resources():
    return {
        resource_rep(qml.RX): 1,
        resource_rep(qml.RY): 1,
    }
...
add_decomps(CustomOpCond, _custom_op_cond_decomposition)
...
def _custom_op_cond_no_false_resources():
    return {
        resource_rep(qml.RX): 1,
    }
...
add_decomps(CustomOpCondNoFalseBranch, _custom_op_cond_no_false_decomposition)
...
def _custom_op_for_loop_resources():
    return {
        resource_rep(qml.RX): 3,
    }
...
add_decomps(CustomOpForLoop, _custom_op_for_loop_decomposition)
...
def _custom_op_while_loop_resources():
    return {
        resource_rep(qml.RX): 3,
    }
...
add_decomps(CustomOpWhileLoop, _custom_op_while_loop_decomposition)
...
def _custom_op_nested_cond_resources():
    return {
        resource_rep(qml.RX): 3,
        resource_rep(qml.RZ): 3,
    }
...
add_decomps(CustomOpNestedCond, _custom_op_nested_cond_decomposition)
...
def _custom_op_autograph_resources():
    return {
        resource_rep(qml.RX): 3,
        resource_rep(qml.RY): 3,
    }
...
add_decomps(CustomOpAutograph, _custom_op_autograph_decomposition)
...
def _custom_op_nested_op_resources():
    return {resource_rep(qml.RX): 1, resource_rep(SimpleCustomOp): 1}
...
add_decomps(CustomOpNestedOp, _custom_op_nested_op_decomposition)
...
def _custom_op_nested_op_ctrl_flow_resources():
    return {
        resource_rep(qml.Rot): 1,
        resource_rep(CustomOpNestedOp): 4,
        resource_rep(SimpleCustomOp): 3,
    }
...
add_decomps(CustomOpNestedOpControlFlow, _custom_op_nested_op_ctrl_flow_decomposition)
