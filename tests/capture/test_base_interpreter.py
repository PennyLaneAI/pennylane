...
def test_default_operator_handling():
    ...
    assert jaxpr.eqns[2].primitive == operator_p
    assert jaxpr.eqns[2].params["op_cls"] is qp.T
...
class TestHigherOrderPrimitiveRegistrations:
    ...
    @pytest.mark.parametrize("grad_f", (qp.grad, qp.jacobian, qp.value_and_grad))
    def test_grad_and_jac(self, grad_f):
        ...
        if grad_f == qp.value_and_grad:
            prim = qp.capture.primitives.value_and_grad_prim
        else:
            prim = qp.capture.primitives.jacobian_prim
