...
class TestSample:
    ...
    def test_sample_dimension(self, tol):
        ...
        assert np.allclose(sample.shape, (2,), atol=0, rtol=0)
        assert np.allclose(sample[0].shape, (n_sample,), atol=0, rtol=0)
        assert np.allclose(sample[1].shape, (2*n_sample,), atol=0, rtol=0)
