...
class TestMeasurementDispatch:
    ...
    def test_measure_nan(self):
        """Check that the nan measurement function is used if the state vector
        has nan values."""
        state = np.array([[0.0, 0.0], [1 / np.sqrt(2), np.NaN]])
        assert get_measurement_function(qml.expval(qml.PauliZ(0)), state) is state_measure_nan
...
class TestBroadcasting:
    ...
    @pytest.mark.parametrize(
        "mp, shape",
        [
            (qml.expval(qml.PauliZ(0)), (3,)),
            (qml.var(qml.PauliZ(0)), (3,)),
            (qml.probs(wires=1), (3, 2)),
        ],
    )
    def test_nan_measurements(self, mp, shape):
        """Test that broadcasting with nan values in the state vector creates
        outcomes with the correct shape."""
        state = qml.math.full((3, 2, 2), np.NaN, like="numpy")
        res = state_measure_nan(mp, state, is_state_batched=True)
