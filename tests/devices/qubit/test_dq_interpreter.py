...
def test_initialization():
    ...
    assert dq.original_shots == qp.measurements.Shots(None)
...
def test_setup_and_cleanup():
    ...
    assert dq.stateref["shots"] == qp.measurements.Shots(2)
    assert dq.shots == qp.measurements.Shots(2)
    ...
    assert qp.math.allclose(dq.state, expected)
...
def test_simple_execution():
    ...
    assert qp.math.allclose(jit_res, res)

    g = jax.grad(f)(jax.numpy.array(0.5))
    assert qp.math.allclose(g, -jax.numpy.sin(0.5))
...
def test_capture_remains_enabled_if_measurement_error():
    ...
    @DefaultQubitInterpreter(num_wires=1, shots=None)
    def g():
        return qp.sample(wires=0)  # sampling with analytic execution.
...
def test_pytree_function_output():
    ...
    res = g()
    assert qp.math.allclose(res["probs"], [1.0, 0.0])
    assert qp.math.allclose(res["state"], [1.0, 0.0 + 0j])
    assert qp.math.allclose(res["var_Z"], 0.0)
    assert qp.math.allclose(res["var_X"], 1.0)
...
def test_mcm_reset():
    ...
    out = f()
    assert qp.math.allclose(out, jnp.array([1.0, 0.0]))  # reset into zero state.
...
def test_projector(basis_state):
    ...
    def circuit(x):
        qp.RX(x, 0)
        qp.Projector(jnp.array([basis_state]), 0)
        return qp.state()

    x = 1.5
    expected_state = qp.math.array([jnp.cos(x / 2), -1j * jnp.sin(x / 2)])
    expected_state[int(not basis_state)] = 0
    expected_state = expected_state / qp.math.norm(expected_state)

    assert qp.math.allclose(circuit(x), expected_state)
...
class TestSampling:
    ...
    def test_same_key_same_results(self, seed):
        ...
        assert qp.math.allclose(res1_first_exec, res2_first_exec)
        assert qp.math.allclose(res1_second_exec, res2_second_exec)
    ...
    def test_classical_transformation_mcm_value(self):
        ...
        assert qp.math.allclose(f(), expected)
    ...
    def test_mcms_not_all_same_key(self, seed):
        ...
        def g():
            ...
            for _ in range(33):
                qp.Hadamard(0)
                ms.append(qp.measure(0, reset=0))
            return ms

        output = g()
        assert not all(qp.math.allclose(output[0], output[i]) for i in range(1, 33))
    ...
    def test_each_measurement_has_different_key(self, seed):
        ...
        res1, res2 = g()
        assert not qp.math.allclose(res1, res2)
    ...
    def test_more_executions_same_interpreter_different_results(self, seed):
        ...
        assert not qp.math.allclose(s1, s2)
...
class TestClassicalComponents:
    ...
    def test_while_loop_with_consts(self):
        ...
        assert qp.math.allclose(output, jnp.cos(2 * 1.2))
    ...
    def test_condition_with_consts(self):
        ...
        assert qp.math.allclose(res0, jnp.cos(x))

        res1 = circuit(x, y, z, False, True)
        assert qp.math.allclose(res1, jnp.cos(z))  # elif branch = z

        res2 = circuit(x, y, z, False, False)
        assert qp.math.allclose(res2, jnp.cos(y))  # false fn = y
...
class TestDynamicShapes:
    ...
    def test_broadcast_in_dim(self, creation_fn):
        ...
        ones = creation_fn(4)
        assert qp.math.allclose(output, jax.numpy.cos(ones))
    ...
    def test_dynamic_shape_arange(self):
        ...
        assert qp.math.allclose(output, jax.numpy.cos(x))
