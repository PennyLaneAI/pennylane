...
def test_shots():
    ...
    assert NullQubit().shots == qp.measurements.Shots(None)
    with pytest.warns(PennyLaneDeprecationWarning, match="shots on device is deprecated"):
        assert NullQubit(shots=100).shots == qp.measurements.Shots(100)
...
def test_wires():
    ...
    assert NullQubit(wires=2).wires == qp.wires.Wires([0, 1])
    assert NullQubit(wires=[0, 2]).wires == qp.wires.Wires([0, 2])
...
def test_set_device_target():
    ...
    to_target = qp.devices.ReferenceQubit()
...
def test_supports_operator_without_decomp(shots):
    ...
    tape = qp.tape.QuantumScript([MyOp(wires=(0, 1))], [qp.expval(qp.Z(0))], shots=shots)
    dev = NullQubit()
...
def test_tracking():
    ...
    qs = qp.tape.QuantumScript(
        [qp.Hadamard(0), qp.FlipSign([1, 0], [0, 1])], [qp.expval(qp.PauliZ(0))]
    )
    dev = NullQubit()
...
class TestBasicCircuit:
    ...
    def test_basic_circuit_numpy(self):
        ...
        dq_res = qp.device("default.qubit").execute(qs)

        assert isinstance(result, tuple)
        assert len(result) == 2
        assert qp.math.shape(dq_res) == qp.math.shape(result)
        assert qp.math.allequal(result, 0)
    ...
    def test_autograd_results_and_backprop(self):
        ...
        assert np.array_equal(qp.jacobian(f)(phi), expected)
    ...
    def test_qnode_returns_correct_interface(self, op, param):
        ...
        assert qp.math.allclose(res, tf.Variable(0.0, dtype=tf.float64))  # input variables float64
...
class TestSampleMeasurements:
    ...
    def test_counts_wires(self, all_outcomes):
        ...
        x = np.array(np.pi / 2)
        qs = qp.tape.QuantumScript(
            [qp.RY(x, wires=0)], [qp.counts(wires=[0, 1], all_outcomes=all_outcomes)], shots=10000
        )
    ...
    def test_counts_wires_batched(self, all_outcomes):
        ...
        qs = qp.tape.QuantumScript(
            [qp.RY(x, wires=0)],
            [qp.counts(wires=[0, 1], all_outcomes=all_outcomes)],
            shots=[50, 100, 150],
        )
...
class TestExecutingBatches:
    ...
    def f(dev, phi):
        ...
        config = ExecutionConfig(interface=qp.math.get_interface(phi), gradient_method="backprop")
    ...
    def f_hashable(phi):
        ...
        config = qp.devices.ExecutionConfig(
            gradient_method="backprop", interface=qp.math.get_interface(phi)
        )
    ...
    def test_autograd(self):
        ...
        assert qp.math.allclose(g1, np.zeros(4))
...
class TestSumOfTermsDifferentiability:
    ...
    def f(dev, scale, n_wires=10, offset=0.1, style="sum"):
        ...
        config = ExecutionConfig(interface=qp.math.get_interface(scale))
    ...
    def test_autograd_backprop(self, style):
        ...
        assert qp.grad(self.f)(dev, x, style=style) == 0
...
class TestClassicalShadows:
    ...
    def test_expval(self, seed):
        ...
        qs = qp.tape.QuantumScript(ops, meas, shots=1000)
    ...
    def test_batching_not_supported(self):
        ...
        ops = [qp.RX([1.1, 2.2], 0)]
        qs = qp.tape.QuantumScript(ops, [qp.classical_shadow([0])], shots=100)
...
class TestIntegration:
    ...
    def test_expected_shape_all_methods(self, diff_method, seed):
        ...
        shape = qp.StronglyEntanglingLayers.shape(n_layers=5, n_wires=n_wires)
        rng = np.random.default_rng(seed=seed)
        params = qp.numpy.array(rng.random(shape))
        ...
        def cost(params):
            out = circuit(params)
            return qp.math.sum(out)

        assert np.array_equal(qp.grad(cost)(params), np.zeros(shape))
...
class TestJacobian:
    ...
    def get_circuit(method, device_vjp):
        @qp.qnode(NullQubit(), diff_method=method, device_vjp=device_vjp)
        def circuit(x):
            qp.RX(x, wires=0)
    ...
    def test_jacobian_autograd(self, method, device_vjp):
        ...
        probs_jac = qp.jacobian(circuit)(x, qp.probs(wires=[0]))
        expval_jac = qp.jacobian(circuit)(x, qp.expval(qp.PauliZ(0)))
...
@pytest.mark.parametrize(
    ...
    [
        ...
        qp.vn_entropy([1]),
        qp.vn_entropy([0, 1]),
        qp.classical_shadow([1]),
        qp.classical_shadow([0, 1]),
        qp.shadow_expval(qp.PauliZ(0)),
        qp.shadow_expval([qp.PauliZ(0), qp.PauliZ(1)]),
    ],
)
...
class TestNullQubitGraphModeExclusive:  # pylint: disable=too-few-public-methods
    ...
    def test_insufficient_work_wires_causes_fallback(self):
        ...
        qp.add_decomps(MyNullQubitOp, decomp_fallback, decomp_with_work_wire)

        tape = qp.tape.QuantumScript([MyNullQubitOp(0)])
        dev = qp.device("null.qubit", wires=1)  # Only 1 wire, but decomp needs 5 burnable
        program = dev.preprocess_transforms()
    ...
    def test_operator_without_graph_decomposition_runs_without_error(self):
        ...
        class CustomOp(qp.operation.Operator):  # pylint: disable=too-few-public-methods
            ...
            def decomposition(self):
                ...
                return [qp.PauliX(self.wires[0]), qp.PauliY(self.wires[1])]

        # Create a tape with this custom operator
        tape = qp.tape.QuantumScript([CustomOp(wires=[0, 1])], [qp.expval(qp.Z(0))])
