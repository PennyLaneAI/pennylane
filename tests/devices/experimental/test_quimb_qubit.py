...
from pennylane.devices.experimental import QuimbQubit, ExecutionConfig
from pennylane.devices.qubit.preprocess import validate_and_expand_adjoint


def test_name():
    """Tests the name of QuimbQubit."""
    assert QuimbQubit().name == "quimb.qubit"


def test_no_jvp_functionality():
    """Test that jvp is not supported on QuimbQubit."""
    dev = QuimbQubit()
...
def test_no_vjp_functionality():
    """Test that vjp is not supported on QuimbQubit."""
    dev = QuimbQubit()
...
def test_no_device_derivatives():
    """Test that QuimbQubit currently doesn't support device derivatives."""
    dev = QuimbQubit()
...
class TestSupportsDerivatives:
    """Test that QuimbQubit states what kind of derivatives it supports."""

    def test_supports_backprop(self):
        """Test that QuimbQubit says that it supports backpropagation."""
        dev = QuimbQubit()
    ...
    def test_supports_adjoint(self):
        """Test that QuimbQubit says that it supports adjoint differentiation."""
        dev = QuimbQubit()
    ...
    def test_doesnt_support_adjoint_with_invalid_tape(self):
        """Tests that QuimbQubit does not support adjoint differentiation with invalid circuits."""
        dev = QuimbQubit()
        config = ExecutionConfig(gradient_method="adjoint")
    ...
    def test_doesnt_support_other_gradient_methods(self, gradient_method):
        """Test that QuimbQubit currently does not support other gradient methods natively."""
        dev = QuimbQubit()
...
class TestExecutingBatches:
    ...
    @staticmethod
    def f(phi):
        """A function that executes a batch of scripts on QuimbQubit without preprocessing."""
        ops = [
            qml.PauliX("a"),
            qml.PauliX("b"),
        ...
        return QuimbQubit().execute((qs1, qs2))
...
class TestAdjointDifferentiation:
    """Tests adjoint differentiation integration with QuimbQubit."""
...
class TestRandomSeed:
    ...
    def test_same_seed(self, measurements):
        ...
        dev1 = QuimbQubit(seed=123)
        result1 = dev1.execute(qs)

        dev2 = QuimbQubit(seed=123)
    ...
    def test_different_seed(self):
        ...
        dev1 = QuimbQubit(seed=None)
        result1 = dev1.execute(qs)

        dev2 = QuimbQubit(seed=123)
        result2 = dev2.execute(qs)

        dev3 = QuimbQubit(seed=456)
...
def test_broadcasted_parameter():
    """Test that QuimbQubit handles broadcasted parameters as expected."""
    dev = QuimbQubit()
