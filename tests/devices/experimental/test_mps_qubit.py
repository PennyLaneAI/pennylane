...
from pennylane.devices.experimental import MPSQubit, ExecutionConfig
from pennylane.devices.qubit.preprocess import validate_and_expand_adjoint


def test_name():
    """Tests the name of MPSQubit."""
    assert MPSQubit().name == "mps.qubit"


def test_no_jvp_functionality():
    """Test that jvp is not supported on MPSQubit."""
    dev = MPSQubit()
...
def test_no_vjp_functionality():
    """Test that vjp is not supported on MPSQubit."""
    dev = MPSQubit()
...
def test_no_device_derivatives():
    """Test that MPSQubit currently doesn't support device derivatives."""
    dev = MPSQubit()
...
class TestSupportsDerivatives:
    """Test that MPSQubit states what kind of derivatives it supports."""

    def test_supports_backprop(self):
        """Test that MPSQubit says that it supports backpropagation."""
        dev = MPSQubit()
    ...
    def test_supports_adjoint(self):
        """Test that MPSQubit says that it supports adjoint differentiation."""
        dev = MPSQubit()
    ...
    def test_doesnt_support_adjoint_with_invalid_tape(self):
        """Tests that MPSQubit does not support adjoint differentiation with invalid circuits."""
        dev = MPSQubit()
        config = ExecutionConfig(gradient_method="adjoint")
    ...
    def test_doesnt_support_other_gradient_methods(self, gradient_method):
        """Test that MPSQubit currently does not support other gradient methods natively."""
        dev = MPSQubit()
...
class TestExecutingBatches:
    ...
    @staticmethod
    def f(phi):
        """A function that executes a batch of scripts on MPSQubit without preprocessing."""
        ops = [
            qml.PauliX("a"),
            qml.PauliX("b"),
        ...
        return MPSQubit().execute((qs1, qs2))
...
class TestAdjointDifferentiation:
    """Tests adjoint differentiation integration with MPSQubit."""
...
class TestRandomSeed:
    ...
    def test_same_seed(self, measurements):
        ...
        dev1 = MPSQubit(seed=123)
        result1 = dev1.execute(qs)

        dev2 = MPSQubit(seed=123)
    ...
    def test_different_seed(self):
        ...
        dev1 = MPSQubit(seed=None)
        result1 = dev1.execute(qs)

        dev2 = MPSQubit(seed=123)
        result2 = dev2.execute(qs)

        dev3 = MPSQubit(seed=456)
...
def test_broadcasted_parameter():
    """Test that MPSQubit handles broadcasted parameters as expected."""
    dev = MPSQubit()
