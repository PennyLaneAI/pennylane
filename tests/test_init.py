...
class TestInit:
    ...
    def test_gate_fabric_init(self, tol):
        """Test the functions 'gate_fabric_uniform' and
        'gate_fabric_normal'."""
        ...
        params = qml.init.gate_fabric_uniform(n_layers, n_wires)
        assert params.shape == exp_shape

        params = qml.init.gate_fabric_normal(n_layers, n_wires)
        assert params.shape == exp_shape

        # check deterministic output for a fixed seed
        seed = 1975
        p1 = qml.init.gate_fabric_uniform(n_layers, n_wires, seed=seed)
        p2 = qml.init.gate_fabric_uniform(n_layers, n_wires, seed=seed)
        assert np.allclose(p1, p2, atol=tol)

        p1 = qml.init.gate_fabric_normal(n_layers, n_wires, seed=seed)
        p2 = qml.init.gate_fabric_normal(n_layers, n_wires, seed=seed)
        assert np.allclose(p1, p2, atol=tol)

        # check that the output is different for different seeds
        p1 = qml.init.gate_fabric_uniform(n_layers, n_wires, seed=seed)
        p2 = qml.init.gate_fabric_uniform(n_layers, n_wires, seed=seed + 1)
        assert not np.allclose(p1, p2, atol=tol)

        p1 = qml.init.gate_fabric_normal(n_layers, n_wires, seed=seed)
        p2 = qml.init.gate_fabric_normal(n_layers, n_wires, seed=seed + 1)
        assert not np.allclose(p1, p2, atol=tol)

    def test_gate_fabric_init_exceptions(self):
        """Test exceptions the functions 'gate_fabric_uniform' and
        'gate_fabric_normal'."""
        ...
        with pytest.raises(ValueError, match=msg_match):
            qml.init.gate_fabric_uniform(n_layers, n_wires)

        with pytest.raises(ValueError, match=msg_match):
            qml.init.gate_fabric_normal(n_layers, n_wires)
