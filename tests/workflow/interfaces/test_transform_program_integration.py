...
class TestCompilePipeline:
    ...
    def test_transform_program_modifies_circuit(self, interface):
        ...
        tape0 = qp.tape.QuantumScript([qp.Rot(1.2, 2.3, 3.4, wires=0)], [qp.expval(qp.PauliZ(0))])
    ...
    def test_ragged_batch_sizes(self, dev, interface):
        ...
        tape3 = qp.tape.QuantumScript([op], [qp.expval(qp.sum(*(qp.PauliZ(i) for i in range(5))))])
        with dev.tracker:
            results = qp.execute(
