...
class TestCaching:
    ...
    def test_cache_maxsize(self, mocker):
        ...
        qpjax.jit(qpjax.grad(cost), static_argnums=1)(params, cachesize=2)
    ...
    def test_custom_cache(self, mocker):
        ...
        custom_cache = {}
        params = qpjax.numpy.array([0.1, 0.2])
        qpjax.grad(cost)(params, cache=custom_cache)
    ...
    def test_custom_cache_multiple(self, mocker):
        ...
        custom_cache = {}
        qpjax.grad(cost)(a, b, cache=custom_cache)
    ...
    def test_caching_param_shift(self, tol):
        ...
        params = qpjax.numpy.array([0.1, 0.2])
        with qml.Tracker(dev) as tracker:
            qpjax.grad(cost)(params, cache=None)
...
class TestJaxExecuteIntegration:
    ...
    def test_scalar_jacobian(self, execute_kwargs, tol):
        ...
        res = qpjax.jit(qpjax.grad(cost))(a)
        assert res.shape == ()
    ...
    def test_reusing_quantum_tape(self, execute_kwargs, tol):
        ...
        jac_fn = qpjax.jit(qpjax.grad(cost))
        jac = jac_fn(a, b)

        a = qpjax.numpy.array(0.54)
        b = qpjax.numpy.array(0.8)
        ...
        jac_fn = qpjax.jit(qpjax.grad(lambda a, b: cost(2 * a, b)))
        jac = jac_fn(a, b)
    ...
    def test_grad_with_backward_mode(self, execute_kwargs):
        ...
        dev = qml.device("default.qubit", wires=2)
        params = qpjax.numpy.array([0.1, 0.2, 0.3])
        expected_results = qpjax.numpy.array([-0.3875172, -0.18884787, -0.38355705])
        ...
        results = qpjax.grad(cost)(params, cache=None)
        for r, e in zip(results, expected_results):
            assert qpjax.numpy.allclose(r, e, atol=1e-7)
    ...
    def test_classical_processing_single_tape(self, execute_kwargs):
        ...
        dev = qml.device("default.qubit", wires=2)
        res = qpjax.jit(qpjax.grad(cost, argnums=(0, 1, 2)), static_argnums=3)(a, b, c, device=dev)
        assert len(res) == 3
    ...
    def test_classical_processing_multiple_tapes(self, execute_kwargs):
        ...
        tapes"""
        dev = qml.device("default.qubit", wires=2)
        params = qpjax.numpy.array([0.3, 0.2])
        ...
        res = qpjax.jit(qpjax.grad(cost_fn))(params)
        assert res.shape == (2,)
    ...
    def test_matrix_parameter(self, execute_kwargs, tol):
        ...
        dev = qml.device("default.qubit", wires=2)
        res = qpjax.jit(cost, static_argnums=2)(a, U, device=dev)
        assert np.allclose(res, -np.cos(a), atol=tol, rtol=0)

        jac_fn = qpjax.grad(cost, argnums=0)
    ...
    def test_differentiable_expand(self, execute_kwargs, tol):
        ...
        with qml.decomposition.local_decomps():
            ...
            with pytest.warns(PennyLaneDeprecationWarning, match="expand"):
                res = qpjax.jit(cost_fn, static_argnums=2)(a, p, device=dev)
            ...
            jac_fn = qpjax.jit(qpjax.grad(cost_fn, argnums=1), static_argnums=2)

            with pytest.warns(PennyLaneDeprecationWarning, match="expand"):
                res = jac_fn(a, p, device=dev)

            expected = qpjax.numpy.array(
                [
                    np.cos(p[1])
    ...
    def test_independent_expval(self, execute_kwargs):
        ...
        res = qpjax.jit(qpjax.grad(cost), static_argnums=1)(params, cache=None)
        assert res.shape == (3,)
...
class TestVectorValuedJIT:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ([qml.probs(wires=[0, 1])], (4,), qpjax.numpy.ndarray),
            ([qml.probs()], (4,), qpjax.numpy.ndarray),
        ],
    )
    def test_shapes(self, execute_kwargs, ret_type, shape, expected_type):
        ...
        res = qpjax.jit(cost)(params, cache=None)
    ...
    ret_and_output_dim = [
        ([qml.probs(wires=0)], (2,), qpjax.numpy.ndarray),
        ([qml.state()], (4,), qpjax.numpy.ndarray),
        ([qml.density_matrix(wires=0)], (2, 2), qpjax.numpy.ndarray),
        # Multi measurements
        ([qml.expval(qml.PauliZ(0)), qml.expval(qml.PauliZ(1))], (), tuple),
    ...
    def test_vector_valued_qnode(self, execute_kwargs, ret, out_dim, expected_type):
        ...
        res = qpjax.jit(cost, static_argnums=1)(params, cache=None)
...
class TestJitAllCounts:
    ...
    def test_jit_allcounts(self, device_name, counts_wires):
        ...
        res = qpjax.jit(qml.execute, static_argnums=(1, 2))(
...
def test_diff_method_None_jit():
    ...
    assert qpjax.numpy.allclose(wrapper(qpjax.numpy.array(0.0))[0], 1.0)
