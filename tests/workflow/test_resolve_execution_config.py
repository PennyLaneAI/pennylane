...
def test_resolve_execution_config_with_gradient_method(empty_tape, empty_tp):
...
def test_metric_tensor_lightning_edge_case(empty_tape, metric_tensor_tp):
...
def test_param_shift_cv_kwargs(empty_tp):
...
def test_mcm_config_validation(empty_tape, empty_tp):
...
@pytest.mark.jax
def test_jax_interface(mcm_method, postselect_mode, tape_with_finite_shots, empty_tp):
...
@pytest.mark.jax
def test_jax_jit_interface(empty_tape, empty_tp):
