...
def test_no_finite_shots():
    ...
    with pytest.raises(ValueError, match="Cannot use the 'one-shot' method"):
        _resolve_mcm_config(mcm_config, device=dev, interface=Interface.AUTO, finite_shots=False)

    mcm_config = MCMConfig(mcm_method=None, postselect_mode="hw-like")
    resolved_mcm_config = _resolve_mcm_config(
        mcm_config, device=dev, interface=Interface.AUTO, finite_shots=False
    )
...
def test_single_branch_statistics():
    ...
    with pytest.raises(
        ...
        _resolve_mcm_config(mcm_config, device=dev, interface=Interface.AUTO, finite_shots=True)
...
def test_resolve_mcm_config_jax_jit_deferred():
    ...
    with pytest.raises(
        ...
        _resolve_mcm_config(mcm_config, device=dev, interface=Interface.JAX_JIT, finite_shots=True)

    mcm_config = MCMConfig(mcm_method="deferred", postselect_mode=None)
    resolved = _resolve_mcm_config(
        mcm_config, device=dev, interface=Interface.JAX_JIT, finite_shots=True
    )
...
def test_resolve_mcm_config_finite_shots_pad_invalid_samples(
    ...
    resolved = _resolve_mcm_config(
        mcm_config, device=dev, interface=Interface.JAX, finite_shots=True
    )
