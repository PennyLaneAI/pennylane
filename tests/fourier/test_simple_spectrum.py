...
from pennylane.fourier.simple_spectrum import simple_spectrum
...
class TestCircuits:
    ...
    def test_spectrum_grows_with_gates(self, n_layers, n_qubits):
        ...
        res = simple_spectrum(circuit)(0.1)
    ...
    def test_encoding_gates(self):
        ...
        res = simple_spectrum(circuit, encoding_gates=["x"])(0.1)
        assert res == {"x": [-1.0, 0.0, 1.0]}

        res = simple_spectrum(circuit, encoding_gates=["x", "other"])(0.1)
        assert res == {"x": [-1.0, 0.0, 1.0], "other": [-1.0, 0.0, 1.0]}

        res = simple_spectrum(circuit)(0.1)
        assert res == {"x": [-1.0, 0.0, 1.0], "other": [-1.0, 0.0, 1.0]}

        res = simple_spectrum(circuit, encoding_gates=["a"])(0.1)
        assert res == {"a": []}
    ...
    def test_spectrum_changes_with_qnode_args(self):
        ...
        res_true = simple_spectrum(circuit)(True)
        assert np.allclose(res_true["x"], range(-3, 4))

        res_false = simple_spectrum(circuit)(False)
