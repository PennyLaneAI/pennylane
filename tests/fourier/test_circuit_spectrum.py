...
class TestCircuits:
    ...
    def test_encoding_gates(self):
        ...
        @qp.qnode(dev)
        def _circuit(x):
            mark(qp.RX(x, wires=0), "x")
            mark(qp.RY(0.4, wires=0), "other")
            return qp.expval(qp.PauliZ(wires=0))
    ...
    def test_spectrum_changes_with_qnode_args(self):
        ...
        def _circuit(last_gate):
            mark(qp.RX(0.1, wires=0), "x")
            mark(qp.RX(0.2, wires=1), "x")
            if last_gate:
                mark(qp.RX(0.3, wires=2), "x")
            return qp.expval(qp.PauliZ(wires=0))
    ...
    def test_input_gates_not_of_correct_form(self):
        ...
        def _circuit():
            mark(qp.RX(0.1, wires=0), "x")
            mark(qp.Rot(0.2, 0.3, 0.4, wires=1), "x")
            return qp.expval(qp.PauliZ(wires=0))
...
def circuit(x, w):
    ...
    for l in range(2):
        for i in range(3):
            mark(qp.RX(x[i], wires=0), "x" + str(i))
            qp.RY(w[l][i], wires=0)
            qp.CNOT(wires=[0, 1])
            qp.CNOT(wires=[1, 2])
