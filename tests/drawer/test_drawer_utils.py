...
class TestDefaultBitMap:
    ...
    def test_subroutine(self):
        ...
        op = f.operator((0, 1, 2))
        cond1 = qp.ops.Conditional(op.output[1], qp.S(0))
        mp = qp.expval(op.output[2])
        queue = [op, cond1, mp]
...
class TestCwireConnections:
    ...
    def test_subroutine_with_output(self):
        ...
        f_op = f.operator((0, 1))
        conds = [
            qp.ops.Conditional(f_op.output[1], qp.S(0)),
            qp.ops.Conditional(f_op.output[2], qp.T(0)),
            qp.ops.Conditional(f_op.output[1], qp.SX(2)),
        ]
