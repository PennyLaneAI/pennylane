...
class TestMidMeasure:
    ...
    def test_can_reuse_wire_after_conditional(self):
        """Tests that a wire can be re-used after using a mid-circuit measurement."""
        with AnnotatedQueue() as q:
            m0 = qml.measure(0)
            qml.cond(m0, qml.PauliX)(1)
            qml.Hadamard(0)
        ...
        ops = q.queue
        assert drawable_layers(ops) == [[ops[0]], [ops[1]], [ops[2]]]
