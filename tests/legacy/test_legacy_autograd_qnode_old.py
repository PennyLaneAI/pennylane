...
class TestQubitIntegration:
    ...
    def test_chained_gradient_value(self, interface, dev_name, diff_method, mode, tol):
        ...
        is correct."""
        kwargs = dict(diff_method=diff_method)
        if diff_method == "spsa":
            kwargs["sampler_rng"] = np.random.default_rng(SEED_FOR_SPSA)
            tol = TOL_FOR_SPSA
...
class TestTapeExpansion:
    ...
    def test_hamiltonian_expansion_analytic(self, dev_name, diff_method, mode, max_diff, tol):
        ...
        kwargs = dict(diff_method=diff_method, mode=mode, max_diff=max_diff)
        if diff_method == "adjoint":
    ...
    def test_hamiltonian_expansion_finite_shots(
        ...
        if diff_method in ("adjoint", "backprop"):
            ...
            gradient_kwargs = dict(
                h=H_FOR_SPSA, sampler_rng=np.random.default_rng(SEED_FOR_SPSA), num_directions=20
            )
            tol = TOL_FOR_SPSA
        elif diff_method == "finite-diff":
