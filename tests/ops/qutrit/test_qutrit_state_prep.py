...
def test_QutritBasisState_decomposition():
    ...
    n = np.array([0, 1, 0])
    wires = (0, 1, 2)
    ops1 = qp.QutritBasisState.compute_decomposition(n, wires)
    ops2 = qp.QutritBasisState(n, wires=wires).decomposition()

    assert len(ops1) == len(ops2) == 1
    assert isinstance(ops1[0], qp.QutritBasisStatePreparation)
    assert isinstance(ops2[0], qp.QutritBasisStatePreparation)
...
class TestStateVector:
    ...
    def test_QutritBasisState_state_vector(self, num_wires, wire_order, one_position):
        ...
        basis_op = qp.QutritBasisState([0, 1], wires=[1, 2])
        ket = basis_op.state_vector(wire_order=wire_order)
        assert qp.math.shape(ket) == (3,) * num_wires
    ...
    def test_QutritBasisState_state_vector_computed(self, state, device_wires, op_wires):
        ...
        basis_op = qp.QutritBasisState(state, wires=op_wires)
    ...
    def test_QutritBasisState_state_vector_preserves_parameter_type(self, interface):
        ...
        basis_op = qp.QutritBasisState(qp.math.array([0, 1], like=interface), wires=[1, 2])
    ...
    def test_QutritBasisState_state_vector_bad_wire_order(self):
        ...
        basis_op = qp.QutritBasisState([0, 1], wires=[0, 1])
    ...
    def test_QutritBasisState_explicitly_checks_0_1_2(self):
        ...
        op = qp.QutritBasisState([3, 1], wires=[0, 1])
        with pytest.raises(
    ...
    def test_QutritBasisState_wrong_param_size(self):
        ...
        op = qp.QutritBasisState([0], wires=[0, 1])
        with pytest.raises(
