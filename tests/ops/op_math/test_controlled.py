...
special_non_par_op_decomps = [
    ...
    (
        ...
        [0],
        qp.CSWAP,
        [qp.Toffoli(wires=[0, 2, 1]), qp.Toffoli(wires=[0, 1, 2]), qp.Toffoli(wires=[0, 2, 1])],
    ),
]
...
class TestCtrl:
    ...
    @pytest.mark.parametrize(
        ...
        [
            (qp.PauliX(wires=[0]), [1], [1], qp.CNOT([1, 0])),
            (
                qp.PauliX(wires=[2]),
                [0, 1],
                [1, 1],
    ...
    def test_pauli_x_based_ctrl_ops(self, op, ctrl_wires, ctrl_values, expected_op):
        ...
        op = qp.ctrl(op, control=ctrl_wires, control_values=ctrl_values, work_wires=["aux"])
    ...
    def test_ctrl_values_sanity_check(self):
        ...
        expected = [
            ...
            *qp.CRot(0.111, 0.222, 0.333, wires=[1, 2]).decomposition(),
            qp.CNOT(wires=[1, 2]),
            *qp.CY(wires=[1, 4]).decomposition(),
            *qp.CZ(wires=[1, 0]).decomposition(),
            qp.PauliX(wires=1),
        ]
        assert len(tape) == 9
