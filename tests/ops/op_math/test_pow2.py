...
def test_has_sparse_matrix():
    ...
    assert pow2(SX2(0), z=2).has_sparse_matrix

    # base has a sparse matrix but z is not an integer
    assert not pow2(SX2(0), z=0.5).has_sparse_matrix

    # base does not have a sparse matrix
    assert not pow2(RX2(0.5, wires=0), z=2).has_sparse_matrix
...
def test_compute_sparse_matrix():
    ...
    with pytest.raises(SparseMatrixUndefinedError):
        pow2(SX2(0), z=0.5).sparse_matrix()
...
def test_has_decomposition():
    ...
    assert pow2(NoPowOp(0), z=2).has_decomposition

    # base.pow succeeds for a non-integer power
    assert pow2(RealPowOp(0.5, wires=0), z=0.5).has_decomposition

    # base.pow raises PowUndefinedError
    assert not pow2(NoPowOp(0), z=0.5).has_decomposition

    # base.pow raises a generic error with a batched (non-scalar) z
    assert not pow2(BadPowOp(0), z=[0.5, 0.5]).has_decomposition

    # base.pow raises a generic error with a scalar z, which is re-raised
    with pytest.raises(ValueError, match="bad pow"):
        _ = pow2(BadPowOp(0), z=0.5).has_decomposition
...
def test_decomposition():
    ...
    decomp = pow2(RealPowOp(2.0, wires=0), z=3).decomposition()
    ...
    with qp.queuing.AnnotatedQueue():
        repeated = pow2(NoPowOp(0), z=3).decomposition()
...
def test_diagonalizing_gates():
    ...
    assert not pow2(RX2(0.5, wires=0), z=2).has_diagonalizing_gates
...
def test_has_adjoint_and_adjoint():
    ...
    frac = pow2(RX2(0.5, wires=0), z=0.5)
...
def test_simplify_single_op():
    ...
    op = pow2(RealPowOp(2.0, wires=0), z=0.5)
