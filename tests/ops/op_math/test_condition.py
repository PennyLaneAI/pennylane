...
class TestCondTypePromotion:
    """Tests for type promotion in conditional operations."""
    
    def test_int_float_promotion(self):
        """Test that int and float branches get promoted correctly."""
        ...
        @qml.qjit
        def circuit(c):
            return qml.cond(c, t, f)()
        
        # This should not raise a compilation error
        result_true = circuit(True)
        result_false = circuit(False)
        
        # Both should be promoted to float64
        import jax.numpy as jnp
        assert result_true.dtype == jnp.float64
        assert result_false.dtype == jnp.float64
        assert result_true == 1.0
        assert result_false == 1.0
        
        qml.capture.disable()
    
    def test_multiple_output_promotion(self):
        """Test type promotion with multiple outputs."""
        pytest.importorskip("jax")
        
        qml.capture.enable()
        
        def t():
            return (1, 2.0)  # int, float
        
        def f():
            return (1.5, 2)  # float, int
        ...
        # All outputs should be promoted to float64
        import jax.numpy as jnp
        assert all(x.dtype == jnp.float64 for x in result_true)
        assert all(x.dtype == jnp.float64 for x in result_false)
