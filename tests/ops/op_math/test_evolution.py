...
class TestEvolution:  # pylint: disable=too-many-public-methods
    ...
    def test_bind_new_parameters_sum_generator(self):
        """Test that bind_new_parameters updates only the evolution time for an
        Evolution of a composite (Sum) generator with scalar coefficients, and
        leaves the static generator untouched (regression for issue #6514)."""
        base = 0.5 * qp.X(0) + 0.7 * qp.Z(0)
        op = Evolution(base, 0.123)

        # op.data only exposes the evolution parameter
        assert op.data == (0.123,)

        new_op = qp.ops.functions.bind_new_parameters(op, op.data)
        qp.assert_equal(new_op, op)
        assert new_op is not op
        assert new_op.base is not op.base

        updated = qp.ops.functions.bind_new_parameters(op, (0.456,))
        qp.assert_equal(updated, Evolution(base, 0.456))
    ...
    def test_sum_generator_diff_methods_agree(self, diff_method):
        """Forward value and gradient of evolve over a Sum generator must agree
        across differentiation methods. parameter-shift/adjoint previously raised
        an IndexError while binding parameters (issue #6514)."""
    ...
    def test_qaoa_evolve_matches_approx_time_evolution(self):
        ...
        built from ``qml.evolve`` of ``Sum`` cost/mixer Hamiltonians silently gave
