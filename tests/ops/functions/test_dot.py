...
op1 = qp.prod(qp.PauliX(1), qp.PauliY(2))
op2 = qp.prod(qp.PauliZ(0), qp.PauliZ(2), qp.PauliZ(4))
op3 = qp.prod(qp.PauliX("a"), qp.PauliX("b"), qp.PauliZ("c"))
op4 = qp.prod(qp.PauliY("a"), qp.PauliZ("b"), qp.PauliX("c"))
op5 = qp.prod(qp.PauliX(4), qp.PauliX(5))
...
class TestDotSum:
    ...
    def test_dot_returns_sprod(self):
        ...
        O = qp.dot(coeffs=[2.0], ops=[qp.PauliX(0)])
    ...
    def test_coeffs_all_ones(self):
        ...
        result = qp.dot([1, 1, 1], [qp.X(0), qp.Y(1), qp.Z(2)])
        assert isinstance(result, Sum)
        assert result[0] == qp.X(0)
        assert result[1] == qp.Y(1)
        assert result[2] == qp.Z(2)
    ...
    def test_dot_does_not_groups_coeffs(self, coeffs):
        ...
        result = qp.dot(coeffs=coeffs, ops=[qp.PauliX(0), qp.PauliX(1), qp.PauliX(2)])
        assert isinstance(result, Sum)
        assert result[0] == qp.s_prod(coeffs[0], qp.PauliX(0))
        assert result[1] == qp.s_prod(coeffs[1], qp.PauliX(1))
        assert result[2] == qp.s_prod(coeffs[2], qp.PauliX(2))
    ...
    def test_dot_autograd(self, dtype):
        ...
        c = qp.numpy.array([1.0, 2.0, 3.0], dtype=dtype)
        o = [qp.PauliX(0), qp.PauliY(1), qp.PauliZ(2)]
        op_sum = qp.dot(c, o)
        op_sum_2 = Sum(
            qp.PauliX(0),
            SProd(qp.numpy.array(2.0, dtype=dtype), qp.PauliY(1)),
            SProd(qp.numpy.array(3.0, dtype=dtype), qp.PauliZ(2)),
        )
    ...
    def test_dot_torch(self, dtype):
        ...
        op_sum_2 = Sum(
            qp.PauliX(0),
            SProd(torch.tensor(2.0, dtype=getattr(torch, dtype)), qp.PauliY(1)),
            SProd(torch.tensor(3.0, dtype=getattr(torch, dtype)), qp.PauliZ(2)),
        )
        qp.assert_equal(op_sum, op_sum_2)
    ...
    def test_dot_jax(self, dtype):
        ...
        op_sum_2 = Sum(
            qp.PauliX(0),
            SProd(jax.numpy.array(2.0, dtype=dtype), qp.PauliY(1)),
            SProd(jax.numpy.array(3.0, dtype=dtype), qp.PauliZ(2)),
        )
        qp.assert_equal(op_sum, op_sum_2)
    ...
    def test_dot_with_just_words_pauli_false(self, coeff, words, ops):
        ...
        dot_res = qp.dot(coeff, words, pauli=False)
        true_res = qp.dot(coeff, ops)
    ...
    def test_dot_with_words_and_sentences_pauli_false(self, coeff, ops):
        ...
        dot_res = qp.dot(coeff, ops, pauli=False)
        true_res = qp.dot(coeff, [op.operation() for op in ops], pauli=False)
        assert dot_res == true_res
    ...
    data_op_words_and_sentences_pauli_false = (
        (
            [1.0, 2.0, 3.0, 1j],
            [pw1, pw2, ps1, op1],
            qp.sum(qp.s_prod(3.0 * 1 + 1 + 1j, op1), qp.s_prod(3 * 2.0 + 2, op2)),
        ),
        (
            [2.0, 3.0, 1j],
            [pw2, ps1, op1],
            qp.sum(qp.s_prod(3.0 * 1 + 1j, op1), qp.s_prod(3 * 2.0 + 2, op2)),
        ),
        (
            [2.0, 3.0, 1j],
            [pw2, ps1, op5],
            qp.sum(qp.s_prod(3.0 * 1, op1), qp.s_prod(3 * 2.0 + 2, op2), qp.s_prod(1j, op5)),
        ),
        (
            [1.0, 2.0, 3.0, 1j],
            [pw3, pw4, ps2, op3],
            qp.sum(qp.s_prod(3.0 * 1 + 1 + 1j, op3), qp.s_prod(3 * 2.0 + 2, op4)),
        ),  # string valued wires
    )
    ...
    def test_dot_with_ops_words_and_sentences(self, coeff, ops, res):
        ...
        dot_res = qp.dot(coeff, ops, pauli=False).simplify()
        assert dot_res == res
    ...
    def test_identities_with_pauli_words_pauli_false(self):
        ...
        res = qp.dot([2.0, 2.0], [pw_id, _pw1], pauli=False)
        true_res = qp.sum(qp.s_prod(2.0, qp.I()), 2.0 * qp.prod(qp.X(1), qp.Y(2)))
        assert res == true_res
    ...
    def test_identities_with_pauli_sentences_pauli_false(self):
        ...
        _pw1 = PauliWord({0: I, 1: X, 2: Y})
        res = qp.dot([2.0, 2.0], [ps_id, _pw1], pauli=False)
        true_res = qp.s_prod(2, qp.I()) + qp.s_prod(2.0, qp.prod(qp.X(1), qp.Y(2)))
        assert res == true_res
...
class TestDotPauliSentence:
    ...
    def test_error_if_ops_PauliWord(self):
        ...
        _pw = qp.pauli.PauliWord({0: "X", 1: "Y"})
    ...
    def test_error_if_ops_PauliSentence(self):
        ...
        _pw1 = qp.pauli.PauliWord({0: "X", 1: "Y"})
        _pw2 = qp.pauli.PauliWord({2: "Z"})
        ps = 2 * _pw1 + 3 * _pw2
    ...
    def test_dot_returns_pauli_sentence(self):
        ...
        ps = qp.dot(coeffs0, ops0, pauli=True)
    ...
    def test_coeffs_and_ops(self):
        ...
        assert qp.math.allequal(hcoeffs, coeffs0)
        for _op1, _op2 in zip(hops, ops0):
            qp.assert_equal(_op1, _op2)

    def test_dot_simplifies_linear_combination(self):
        """Test that the dot function groups equal pauli words."""
        ps = qp.dot(
            coeffs=[0.12, 1.2, 12], ops=[qp.PauliX(0), qp.PauliX(0), qp.PauliX(0)], pauli=True
        )
        assert len(ps) == 1
        h = ps.operation()
        assert isinstance(h, qp.ops.SProd)
        qp.assert_equal(h.base, qp.PauliX(0))
    ...
    def test_dot_returns_hamiltonian_simplified(self):
        ...
        ps = qp.dot(coeffs0, ops0, pauli=True)
        h_ps = ps.operation()
        h = qp.Hamiltonian(coeffs0, ops0)
        h.simplify()
        qp.assert_equal(h_ps, h)

    @pytest.mark.autograd
    def test_dot_autograd(self):
        """Test the dot function with the autograd interface."""
        c = qp.numpy.array([1.0, 2.0, 3.0])
        o = [qp.PauliX(0), qp.PauliY(1), qp.PauliZ(2)]
    ...
    def test_dot_torch(self):
        ...
        ps_2 = qp.pauli.PauliSentence(
            {
                qp.pauli.PauliWord({0: "X"}): torch.tensor(1.0),
                qp.pauli.PauliWord({1: "Y"}): torch.tensor(2.0),
                qp.pauli.PauliWord({2: "Z"}): torch.tensor(3.0),
            }
        )
        assert ps == ps_2
    ...
    def test_dot_jax(self):
        ...
        ps_2 = qp.pauli.PauliSentence(
            {
                qp.pauli.PauliWord({0: "X"}): jax.numpy.array(1.0),
                qp.pauli.PauliWord({1: "Y"}): jax.numpy.array(2.0),
                qp.pauli.PauliWord({2: "Z"}): jax.numpy.array(3.0),
            }
        )
        assert ps == ps_2
    ...
    def test_dot_with_just_words(self, coeff, ops, res):
        """Test operators that are just pauli words"""
        dot_res = qp.dot(coeff, ops, pauli=True)
        assert dot_res == res
