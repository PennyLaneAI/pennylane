...
class TestPauliUtils:
    ...
    def test_pauli_letters(self):
        ...
        for op, mat in zip(
            [qp.Identity(0), qp.PauliX(0), qp.PauliY(0), qp.PauliZ(0)], _pauli_matrices
        ):
...
special_matrix_cases = [
    ...
    (
        2,
        eye[0] + eye[3] + eye[4],
        np.kron(qp.RX(-2, 0).matrix(), qp.RX(-2, 0).matrix()) @ qp.IsingXX(-2, [0, 1]).matrix(),
    ),
    (2, 0.6 * (eye[4] + eye[9]), qp.IsingXY(2.4, [0, 1]).matrix()),
    (2, 0.8 * eye[-1], qp.IsingZZ(-1.6, [0, 1]).matrix()),
]


class TestGetOneParameterGenerators:
    """Tests for the effective generators computing function
    get_one_parameter_generators of qp.SpecialUnitary."""
    ...
    def get_one_parameter_generators(theta, num_wires, interface):
        ...
        return qp.SpecialUnitary(theta, list(range(num_wires))).get_one_parameter_generators(
            interface
        )
    ...
    def test_raises_autograd(self):
        ...
        op = qp.SpecialUnitary(qp.numpy.ones(3), [0])
...
class TestGetOneParameterGeneratorsDiffability:
    """Tests for the effective generators computing function
    get_one_parameter_generators of qp.SpecialUnitary to be differentiable."""
    ...
    def test_raises_autograd(self):
        ...
        with pytest.raises(NotImplementedError, match="expm is not differentiable in Autograd"):
            qp.jacobian(self.get_one_parameter_generators)(qp.numpy.ones(3), 1, "autograd")


class TestGetOneParameterCoeffs:
    """Tests for the coefficients of effective generators computing function
    get_one_parameter_coeffs of qp.SpecialUnitary."""
...
class TestSpecialUnitary:
    ...
    def test_compute_matrix_random_broadcasted(self, n, seed, interface):
        ...
        for matrix in matrices:
            ...
            assert all(np.allclose(m @ qp.math.conj(qp.math.T(m)), I) for m in matrix)
    ...
    def test_compute_matrix_single_param(self, n):
        ...
        for word, theta in zip(words, np.eye(d)):
            matrices = [
                qp.SpecialUnitary(x * theta, list(range(n))).matrix(),
                qp.SpecialUnitary.compute_matrix(x * theta, n),
            ]
            paulirot_matrix = qp.PauliRot(-2 * x, word, list(range(n))).matrix()
            for matrix in matrices:
    ...
    def test_compute_matrix_special_cases(self, n, theta, expected):
        ...
        for matrix in matrices:
            assert qp.math.allclose(matrix, expected)
    ...
    def test_compute_matrix_special_cases_broadcasted(self):
        ...
        for matrix in matrices:
            assert qp.math.allclose(matrix, np.stack(exp))
    ...
    def test_decomposition_numpy(self, n, theta):
        ...
        mat = qp.SpecialUnitary.compute_matrix(theta, n)
    ...
    def test_adjoint(self, theta, n):
        ...
        wires = list(range(n))
        U = qp.SpecialUnitary(theta, wires)
        U_dagger = qp.adjoint(qp.SpecialUnitary)(theta, wires)
        U_dagger_inplace = qp.SpecialUnitary(theta, wires).adjoint()
        U_minustheta = qp.SpecialUnitary(-theta, wires)
        assert qp.math.allclose(U.matrix(), U_dagger.matrix().conj().T)
        assert qp.math.allclose(U.matrix(), U_dagger_inplace.matrix().conj().T)
        assert qp.math.allclose(U_minustheta.matrix(), U_dagger.matrix())
    ...
    def test_wrong_input_shape(self, theta, n):
        ...
        with pytest.raises(ValueError, match="Expected the parameters to have"):
            _ = qp.SpecialUnitary(theta, wires)
    ...
    def test_jax_jit(self):
        ...
        def comparison(x):
            state = qp.SpecialUnitary.compute_matrix(x, 1) @ jnp.array([1, 0])
    ...
    def test_large_wire_jacobian_regression(self, interface):
        ...
        matrix = qp.SpecialUnitary.compute_matrix(theta, num_wires=num_wires)

        expected_shape = (2**num_wires, 2**num_wires)
        assert qp.math.shape(matrix) == expected_shape

        # Test that gradients can be computed (this was the main issue https://github.com/PennyLaneAI/pennylane/issues/7583)

        def g(theta):
            return qp.math.real(qp.SpecialUnitary.compute_matrix(theta, num_wires=num_wires))

        jac = qp.math.jacobian(g)(theta)
        assert qp.math.shape(jac) == expected_shape + (num_params,)
...
class TestSpecialUnitaryIntegration:
    ...
    def paulirot_comp_circuit(x, word=None):
        ...
        qp.PauliRot(-2 * x, word, wires=[0, 1])
        return qp.expval(qp.PauliZ(0) @ qp.PauliX(1))

    x = np.linspace(0.2, 1.5, 15)
    state = qp.SpecialUnitary.compute_matrix(x, 2) @ np.eye(4)[0]
    exp = np.vdot(state, qp.matrix(qp.PauliZ(0) @ qp.PauliX(1)) @ state).real
    ...
    def test_qnode_numpy(self, dev_fn):
        ...
        assert qp.math.isclose(res, self.exp)
    ...
    def test_qnode_jax(self, dev_fn, shots, atol, use_jit):
        ...
        qnode = qp.QNode(self.circuit, dev, interface="jax", diff_method=diff_method, shots=shots)
        if use_jit:
            qnode = jax.jit(qnode)
    ...
    def test_qnode_torch(self, dev_fn, shots, atol):
        ...
        res = qnode(x)
        assert qp.math.shape(res) == ()
        assert qp.math.isclose(res, torch.tensor(self.exp), atol=atol)
...
class TestTmpPauliRot:
    ...
    def test_decomposition_at_zero_autograd(self):
        ...
        x = qp.numpy.array(0.0, requires_grad=True)
        with qp.queuing.AnnotatedQueue() as q:
            qp.grad(self.get_decomposition)(x)
    ...
    def test_decomposition_nonzero(self, word, x):
        ...
        for dec in [decomp, decomp2]:
            assert len(dec) == 1
            qp.assert_equal(dec[0], qp.PauliRot(x, word, wires))
