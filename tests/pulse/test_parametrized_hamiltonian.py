...
example_ph = ParametrizedHamiltonian(
...
class TestInitialization:
    ...
    def test_H_fixed_lists(self):
        ...
        assert example_ph.coeffs_fixed == [1, 2]
        for op1, op2 in zip(example_ph.ops_fixed, [qml.PauliX(0), qml.PauliY(1)]):
            qml.assert_equal(op1, op2)

    def test_H_parametrized_lists(self):
        """Test that attributes H_parametrized_ops and H_parametrized_coeffs are as expected"""
        assert example_ph.coeffs_parametrized == [f1, f2]
        for op1, op2 in zip(example_ph.ops_parametrized, [qml.PauliZ(2), qml.Hadamard(3)]):
            qml.assert_equal(op1, op2)

    def test_H_fixed(self):
        """Test that H_fixed() is an Operator of the expected form"""
        H_fixed = example_ph.H_fixed()
    ...
    def test_H_parametrized(self):
        ...
        H_param = example_ph.H_parametrized
