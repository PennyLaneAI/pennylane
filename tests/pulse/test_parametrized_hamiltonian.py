...
test_example = ParametrizedHamiltonian(
    [1, 2, f1, f2], [qp.PauliX(0), qp.PauliY(1), qp.PauliZ(2), qp.Hadamard(3)]
)
...
class TestInitialization:
    ...
    def test_initialization_via_dot(self):
        """Test that using qp.dot initializes a ParametrizedHamiltonian"""
        ...
        qp.assert_equal(H([1.2, 2.3], 3.4), expected_H([1.2, 2.3], 3.4))
    ...
    def test_H_fixed_lists(self):
        ...
        for op1, op2 in zip(test_example.ops_fixed, [qp.PauliX(0), qp.PauliY(1)]):
    ...
    def test_H_parametrized_lists(self):
        ...
        for op1, op2 in zip(test_example.ops_parametrized, [qp.PauliZ(2), qp.Hadamard(3)]):
            qp.assert_equal(op1, op2)

    def test_H_fixed(self):
        """Test that H_fixed() is an Operator of the expected form"""
        H_fixed = test_example.H_fixed()
        op = qp.sum(qp.s_prod(1, qp.PauliX(0)), qp.s_prod(2, qp.PauliY(1)))
        qp.assert_equal(H_fixed, op)
    ...
    def test_H_parametrized(self):
        ...
        assert isinstance(H[0], qp.ops.SProd)
        assert isinstance(H[1], qp.ops.SProd)
    ...
    def test__repr__with_Sum_and_Prod(self):
        ...
        h0 = qp.sum(*(qp.X(i) for i in range(5)))
        h1 = qp.prod(qp.X(0), qp.sum(qp.X(1), qp.X(2)))
        H = qp.pulse.constant * h0 + qp.pulse.constant * h1 + qp.pulse.constant * qp.X(1)
    ...
    def test_repr_with_class_objects(self):
        ...
        observables = [qp.PauliX(0), qp.PauliY(0), qp.PauliZ(0), qp.PauliX(0)]
    ...
    def test_wire_attribute(self):
        ...
        coeffs = [1, f1, 2, f2]
        ops = [qp.PauliX(3), qp.PauliX(2), qp.PauliX(0), qp.PauliX(1)]
...
class TestInteractionWithOperators:
    ...
    def test_multiply_with_scalar(self):
        ...
        qp.assert_equal(new_H.H_parametrized([1, 2], t=4), expected_H.H_parametrized([1, 2], t=4))
    ...
    def test_adding_two_parametrized_hamiltonians(self):
        ...
        qp.assert_equal(parametric_term[0], pH1.H_parametrized([1.2], 0.5))
        qp.assert_equal(parametric_term[1], pH2.H_parametrized([2.3], 0.5))
...
class TestProperties:
    ...
    def test_ops(self):
        ...
        for op1, op2 in zip(ops, [qp.PauliX(0), qp.PauliY(1), qp.PauliZ(2), qp.Hadamard(3)]):
            qp.assert_equal(op1, op2)
