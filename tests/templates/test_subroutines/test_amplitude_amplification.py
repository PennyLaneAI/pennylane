...
class TestIntegration:
    ...
    def circuit():
        ...
        qml.AmplitudeAmplification(generator(range(3)), oracle([0], range(3)), fixed_point=True, iters=3, work_wire=3)
        return qml.probs(wires=range(3))

    # not calculated analytically, we are only ensuring that the results are consistent accross interfaces

    exp_result = np.array(
        [0.67103818, 0.04699455, 0.04699455, 0.04699455, 0.04699455, 0.04699455, 0.04699455, 0.04699455]
    )
