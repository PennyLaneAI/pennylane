...
class TestMultiplier:
    ...
    def test_operation_result(
        ...
        for x, y in zip(range(len(x_wires)),range(len(y_wires))):
            assert np.allclose(sum(bit * (2 ** i) for i, bit in enumerate(reversed(circuit(x,y)))), (x*y) % max)
    ...
    def test_decomposition(self, x_wires, y_wires, output_wires, mod, work_wires):
        ...
        multiplier_decomposition = OutMultiplier(
            x_wires, y_wires, output_wires, mod, work_wires).compute_decomposition(x_wires, y_wires, output_wires, mod, work_wires)
        op_list = []
        ...
        op_list.append(qml.ControlledSequence(qml.ControlledSequence(PhaseAdder(1, output_wires,mod,work_wires), 
                                                                     control = x_wires),control=y_wires))
