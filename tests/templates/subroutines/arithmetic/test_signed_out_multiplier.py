...
from pennylane.decomposition.resources import controlled_resource_rep, resource_rep
...
def test_signed_out_multiplier_resources():
    ...
    semi_adder_rep = resource_rep(
        SemiAdder,
        num_x_wires=len(output_wires),
        num_y_wires=len(output_wires),
        num_work_wires=len(output_wires) - 1,
    )
