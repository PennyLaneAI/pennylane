...
class TestFable:
    ...
    def test_fable_real_input_matrices(self, input, wires):
        ...
        @qp.qnode(dev)
        def circuit():
            qp.FABLE(input_matrix=input, wires=range(wires), tol=0)
            return qp.state()

        dim = len(input)
        expected = dim * qp.matrix(circuit, wire_order=range(wires))().real[:dim, :dim]
    ...
    def test_fable_imaginary_error(self, input_matrix):
        ...
        with pytest.raises(
            ...
            qp.FABLE(imaginary_matrix, wires=range(5), tol=0.01)
    ...
    def test_fable_wires_error(self, input_matrix):
        ...
        with pytest.raises(ValueError, match="Number of wires is incorrect"):
            qp.FABLE(input_matrix, wires=range(4), tol=0.01)
    ...
    def test_fable_normalization_error(self, input_matrix):
        ...
        with pytest.raises(ValueError, match="The subnormalization factor should be lower than 1."):
            qp.FABLE(input_matrix, wires=range(5), tol=0.01)
    ...
    def test_warning_for_non_square(self):
        ...
        with pytest.warns(Warning, match="The input matrix should be of shape NxN"):
            qp.FABLE(non_square_matrix, wires=range(5), tol=0.01)
    ...
    def test_padding_for_non_square(self):
        ...
        op = qp.FABLE(non_square_matrix, wires=range(5), tol=0.01)
        data = op._flatten()
    ...
    def test_warning_for_not_power(self):
        ...
        with pytest.warns(Warning, match="The input matrix should be of shape NxN"):
            qp.FABLE(two_by_three_array, wires=range(5), tol=0.01)
    ...
    def test_padding_for_not_power(self):
        ...
        op = qp.FABLE(two_by_three_array, wires=range(5), tol=0.01)
        data = op._flatten()
        assert data[0][0].shape == (4, 4)
    ...
    def test_jax(self, input_matrix):
        ...
        circuit_jax = qp.FABLE(jax_matrix, wires=range(5), tol=0)

        assert qp.math.allclose(qp.matrix(circuit_default), qp.matrix(circuit_jax))
    ...
    def test_autograd(self, input_matrix):
        ...
        circuit_grad = qp.FABLE(grad_matrix, wires=range(5), tol=0)

        assert qp.math.allclose(qp.matrix(circuit_default), qp.matrix(circuit_grad))
        assert qp.math.get_interface(qp.matrix(circuit_grad)) == "autograd"
    ...
    def test_fable_jax_jit(self, input_matrix):
        ...
        assert qp.math.allclose(gradient_numeric, gradient_jax[0, 0], rtol=0.001)
        assert qp.math.allclose(jitted_fn(input_matrix), circuit_jax(input_matrix))
    ...
    def test_fable_autograd(self, input_matrix):
        ...
        grad_fn = qp.grad(circuit_autograd)
        gradient_numeric = (
    ...
    def test_default_lightning_devices(self, input_matrix):
        ...
        dev2 = qp.device("lightning.qubit", wires=range(5))
        ...
        expected = (
            len(input_matrix)
            * qp.matrix(circuit, wire_order=range(5))().real[
        ...
        lightning = (
            len(input_matrix)
            * qp.matrix(circuit_lightning, wire_order=range(5))().real[
                0 : len(input_matrix), 0 : len(input_matrix)
            ]
        )
    ...
    def test_variety_of_matrix_shapes(self, input, wires):
        ...
        dev = qp.device("default.qubit")
        s = int(qp.math.ceil(qp.math.log2(max(input.shape))))
        s = max(s, 1)
        dim = 2**s
        ...
        circuit_mat = dim * qp.matrix(circuit, wire_order=range(wires))().real[:dim, :dim]
    ...
    def test_decomposition_new_fixed_input(self):
        ...
        op = qp.FABLE(matrix, wires=range(3), tol=0)
