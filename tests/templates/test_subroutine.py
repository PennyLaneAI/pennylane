...
def test_fallback_creating_resources_AbstractArray():
    ...
    r = qp.core.operator.abstractify(qp.MultiControlledX(Wire[3], work_wires=Wire[0]))
    expected[r] = 1
    assert resources == expected
...
class TestGraphDecomposition:
    ...
    def test_change_op_basis_subroutine_resource_rep_with_a_subroutine_uncompute(self):
        ...
        assert rr.params["compute_op"] == CNOT(Wire[2])

        assert rr.params["target_op"] == PauliX(Wire)
