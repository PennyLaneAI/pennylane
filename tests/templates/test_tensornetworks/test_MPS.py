...
class TestIndicesMPS:
    ...
    def test_exception_n_block_wires_large(self, n_wires, n_block_wires):
        ...
        with pytest.raises(
            ValueError,
            match=f"n_block_wires must be smaller than or equal to the number of wires; got n_block_wires = {n_block_wires} and number of wires = {n_wires}",
        ):
            compute_indices_MPS(range(n_wires), n_block_wires)
    ...
    def test_exception_n_block_wires_small(self):
        ...
        with pytest.raises(
            ValueError,
            match=f"number of wires in each block must be larger than or equal to 2; got n_block_wires = {n_block_wires}",
        ):
            compute_indices_MPS(range(n_wires), n_block_wires)
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                ...
                [1, 2, 3, 4],
                6,
                "n_block_wires must be smaller than or equal to the number of wires; got n_block_wires = 6 and number of wires = 4",
            ),
            (
                None,
                None,
                [1, 2, 3, 4],
                0,
                "number of wires in each block must be larger than or equal to 2; got n_block_wires = 0",
            ),
        ],
    )
...
class TestAttributes:
    ...
    def test_get_n_blocks_warning(self, wires, n_block_wires):
        ...
        with pytest.warns(
            Warning,
            match=f"The number of wires should be a multiple of {int(n_block_wires/2)}; got {len(wires)}",
        ):
    ...
    def test_get_n_blocks(self, wires, n_block_wires):
        ...
        with pytest.raises(
            ValueError,
            match=f"n_block_wires must be smaller than or equal to the number of wires; got n_block_wires = {n_block_wires} and number of wires = {len(wires)}",
        ):
            qml.MPS.get_n_blocks(wires, n_block_wires)
