...
class TestDecomposition:
    ...
    DECOMP_PARAMS = [
        (qp.math.array([[[-0.080, 2.629]]]), [0, 1, 2, 3], qp.math.array([1, 1, 0, 0]), False),
        (
            qp.math.array([[[-0.080, 0.101], [-0.080, 0.101]]]),
            [0, 1, 2, 3, 4, 5],
            qp.math.array([0, 1, 0, 1, 0, 1]),
            True,
        ),
    ]
    ...
    def test_decomposition_new(self, weights, wires, init_state, include_pi):
        op = qp.GateFabric(weights, wires=wires, init_state=init_state, include_pi=include_pi)
        for rule in qp.list_decomps(qp.GateFabric):
