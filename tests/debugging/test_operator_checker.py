...
from pennylane.debugging.operator_checker import (
    equal_up_to_phase,
    is_diagonal,
    OperatorChecker,
    CheckerError,
...
qmlops_skip = {
    qml.PauliRot,  # Uses a hyperparameter, not supported yet
    qml.Hamiltonian,
...
class TestQMLOperators:
    """Test the OperatorChecker on native PennyLane operators."""

    Checker = OperatorChecker(verbosity="comment")

    @pytest.mark.parametrize("op", qmlops)
    def test_standard_qml_ops(self, op):
        """Test that the OperatorChecker reports "pass" on all PennyLane operators
        that have standard input format. Exceptions are
        - qml.MultiRZ, which makes use of hyperparameters (This is something the
          OperatorChecker comments on, setting the result status to "comment")
...
error_ops = [
    ...
    (OpWrongBasis, "The operator OpWrongBasis is not diagonal in the provided basis"),
    (
        OpWrongNumParamsComputeEigvals,
        "compute_eigvals() missing 1 required positional",
        "Operator method OpWrongNumParamsComputeEigvals.compute_eigvals does not work",
    ),
...
fatal_error_ops = [
    ...
    (OpWrongMatrixShape, "The operator OpWrongMatrixShape defines a non-square matrix"),
...
class TestFlawedArgs:
    ...
    def test_wrong_number_wires(self):
        """Test that the OperatorChecker reports a fatal error
    ...
    def test_no_suitable_number_params(self):
        ...
        Checker = OperatorChecker(verbosity="comment", max_num_params=2)
