...
class TestConvertToMBQCFormalismPass:
    ...
    def test_convert_to_mbqc_formalism_allocop_aux_wires(self, run_filecheck):
        """Test that ConvertToMBQCFormalismPass can add 13 aux wires to the circuit."""
        program = """
            func.func @test_func() {
                %num_qubits = arith.constant 2 : i64
                // CHECK:  %0 = quantum.alloc( 16) : !quantum.reg
                %1 = quantum.alloc (%num_qubits : i64) : !quantum.reg
                return
            }
        """
