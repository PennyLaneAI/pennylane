...
class CustomTwoQubitOperation(qp.operation.Operation):
    ...
    @staticmethod
    def compute_matrix():
        return qp.math.conj(qp.math.transpose(qp.CNOT.compute_matrix()))


def circuit_1():
    """Circuit 1 with quantum chemistry gates"""
    qp.RZ(1.0, wires=[0])
    qp.PhaseShift(1.0, wires=[1])
    qp.SingleExcitation(2.0, wires=[1, 2])
    qp.PauliX(0)
...
def circuit_4():
    ...
    qp.RandomLayers(weights=qp.math.array([[0.1, -2.1, 1.4]]), wires=range(2))
    return qp.expval(qp.PauliZ(0))
...
def circuit_5():
    ...
    qp.DiagonalQubitUnitary(
        [qp.math.exp(1j * 0.1), qp.math.exp(1j * PI), INVSQ2 * (1 + 1j), INVSQ2 * (1 - 1j)],
        wires=[0, 1],
    )
    return qp.expval(qp.PauliZ(0))
...
def circuit_6():
    ...
    qp.PhaseShift(5 * math.pi / 2, wires=1)
    qp.adjoint(qp.T(wires=2))
    qp.PhaseShift(-3 * math.pi / 4, wires=3)
    return qp.expval(qp.PauliZ(0))
...
class TestCliffordCompile:
    ...
    @pytest.mark.parametrize(
        "op, res",
        [
            (qp.DoubleExcitation(2.0, wires=[0, 1, 2, 3]), False),
            (qp.PauliX(wires=[1]), True),
            (qp.RX(3 * PI, wires=[1]), True),
            (qp.adjoint(qp.RX(3 * PI, wires=[1])), True),
    ...
    def test_decomposition(self, circuit):
        ...
        old_tape = qp.tape.make_qscript(circuit)()
    ...
    def test_decomposition_with_rs_qjit_repeated_decomp(self, circuit):
        ...
        qjit_cir_copy = qp.qjit(decomp_cir_copy)
    ...
    def test_qnode_decomposition(self):
        ...
        def qfunc():
            qp.PhaseShift(1.0, wires=[0])
            qp.PhaseShift(2.0, wires=[1])
            qp.ISWAP(wires=[0, 1])
            return qp.expval(qp.PauliZ(0))

        original_qnode = qp.QNode(qfunc, dev)
        transformed_qnode = qp.QNode(clifford_t_decomposition(qfunc), dev)
    ...
    def test_phase_shift_decomposition(self):
        ...
        old_tape = qp.tape.make_qscript(circuit_6)()

        [new_tape], _ = clifford_t_decomposition(old_tape, max_depth=3)

        compiled_ops = new_tape.operations

        assert qp.equal(compiled_ops[0], qp.adjoint(qp.S(0)))
        assert qp.equal(compiled_ops[1], qp.S(1))
        assert qp.equal(compiled_ops[2], qp.adjoint(qp.T(2)))
        assert qp.equal(compiled_ops[3], qp.T(3))
    ...
    def test_total_error(self, epsilon, circuit):
        ...
        qnode_basic = qp.QNode(circuit, dev)
        qnode_transformed = clifford_t_decomposition(
            qnode_basic, epsilon=epsilon, method="sk", max_depth=10, basis_set=("T", "T*", "H")
        )
        mat_exact = qp.matrix(qnode_basic, wire_order=[0, 1])()
        mat_approx = qp.matrix(qnode_transformed, wire_order=[0, 1])()
        phase = qp.math.divide(
            mat_exact,
            mat_approx,
            out=qp.math.zeros_like(mat_exact, dtype=complex),
            where=mat_exact != 0,
        )[qp.math.nonzero(qp.math.round(mat_exact, 10))][0]
        mat_exact /= phase
    ...
    @pytest.mark.parametrize(
        "op", [qp.RX(1.0, wires="a"), qp.U3(1, 2, 3, wires=[1]), qp.PhaseShift(1.0, wires=[2])]
    )
    def test_one_qubit_decomposition(self, op):
        ...
        valid_gates = _CLIFFORD_PHASE_GATES + (qp.RZ,)
        assert all(
        ...
        assert isinstance(global_op, qp.GlobalPhase)

        matrix_op = reduce(
            lambda x, y: x @ y, [qp.matrix(op) for op in decomp_ops][::-1]
        ) * qp.matrix(global_op)
        ...
        phase = qp.math.divide(
            matrix_op,
            qp.matrix(op),
            out=qp.math.zeros_like(matrix_op, dtype=complex),
            where=matrix_op != 0,
        )[qp.math.nonzero(qp.math.round(matrix_op, 10))]
        assert qp.math.allclose(phase / phase[0], qp.math.ones(qp.math.shape(phase)[0]), atol=1e-5)

    @pytest.mark.parametrize(
        "op",
        [
            qp.PSWAP(1.0, wires=["a", "b"]),
            qp.SingleExcitation(1, wires=[1, 2]),
            qp.IsingXX(1.0, wires=[2, 3]),
        ],
    )
    def test_two_qubit_decomposition(self, op):
        ...
        mapped_op = [qp.map_wires(op, wire_map=wire_map) for op in decomp_ops][::-1]
        matrix_op = reduce(
            lambda x, y: x @ y, [qp.matrix(op, wire_order=[0, 1]) for op in mapped_op]
        ) * qp.matrix(global_op, wire_order=[0, 1])
        ...
        assert qp.math.allclose(phase / phase[0], qp.math.ones(qp.math.shape(phase)[0]))

    @pytest.mark.parametrize(
        "op",
        [
            qp.adjoint(qp.RX(1.0, wires=["b"])),
            qp.Rot(1, 2, 3, wires=[2]),
            qp.PhaseShift(1.0, wires=[0]),
            qp.PhaseShift(3 * PI, wires=[0]),
        ],
    )
    ...
    def test_merge_param_gates(self):
        ...
        assert isinstance(merged_ops[0], qp.RX)
        assert merged_ops[0].parameters == [0.8]  # 0.1 + 0.2 + 0.5 for wire 0
        assert isinstance(merged_ops[1], qp.RY)
    ...
    @pytest.mark.parametrize("op", [qp.U1(1.0, wires=["b"])])
    ...
    def test_zero_global_phase(self):
        ...
        [tape], _ = qp.clifford_t_decomposition(tape)

        assert not sum(isinstance(op, qp.GlobalPhase) for op in tape.operations)
    ...
    def test_clifford_decompose_interfaces(self, method, kwargs):
        ...
        def circuit(x):
            qp.RZ(x[0], wires=[0])
            qp.PhaseShift(x[1], wires=[1])
            qp.SingleExcitation(x[2], wires=[1, 2])
            qp.PauliX(0)
            return qp.expval(qp.PauliZ(1))

        original_qnode = qp.QNode(circuit, dev)
        transfmd_qnode = qp.QNode(clifford_t_decomposition(circuit, method=method, **kwargs), dev)
        ...
        for qcirc in [original_qnode, transfmd_qnode]:
            # Autograd Interface
            A = qp.numpy.array(coeffs)
            fres_numpy = qcirc(A)
            grad_numpy = qp.grad(qcirc, argnums=0)(A)
        ...
        # Compare results
        assert all(qp.math.allclose(res1, res2, atol=1e-2) for res1, res2 in zip(*funres))
        assert all(qp.math.allclose(res1, res2, atol=1e-2) for res1, res2 in zip(*igrads))
...
class TestCliffordCached:
    ...
    def test_clifford_cached(self):
        ...
        num_angles = 1
        rand_angles = qp.math.random.rand(num_angles)
        rand_angles = qp.math.concatenate((rand_angles, -rand_angles))

        num_repeat = 2
        old_tape = qp.tape.make_qscript(circuit_9)(num_repeat, rand_angles)
    ...
    def test_wire_mapping(self):
        ...
        for wire in range(5):
            assert _map_wires(qp.X(0), wire) == qp.X(wire)
...
class TestCatalyst:
    ...
    def test_decomposition_with_rs_qjit_dynamic_param(self):
        ...
        decomposed_cir = qp.QNode(clifford_t_decomposition(circuit, method="gridsynth"), dev)
        qjit_cir = qp.qjit(decomposed_cir)
    ...
    def test_decomposition_with_sk_qjit_raise(self):
        ...
        dev = qp.device("lightning.qubit", wires=4)
        qnode_cir = qp.qnode(dev)(circuit_7)
