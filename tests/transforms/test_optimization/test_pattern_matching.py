...
class TestPatternMatchingOptimization:
    ...
    def test_simple_quantum_function_pattern_matching(self):
        ...
        with qp.queuing.AnnotatedQueue() as q_template:
            qp.CNOT(wires=[1, 2])
            qp.CNOT(wires=[0, 1])
            qp.CNOT(wires=[1, 2])
            qp.CNOT(wires=[0, 1])
        ...
        template = qp.tape.QuantumScript.from_queue(q_template)
        ...
        assert np.allclose(qp.matrix(optimized_qnode)(), qp.matrix(qnode)())
    ...
    def test_simple_quantum_function_pattern_matching_qnode(self):
        ...
        optimized_qnode()
        assert np.allclose(qp.matrix(optimized_qnode)(), qp.matrix(circuit)())
    ...
    def test_template_with_toffoli(self):
        ...
        toffolis_qnode = qp.specs(qnode)().resources.quantum_operations.get("Toffoli", 0)
        toffolis_optimized_qnode = qp.specs(optimized_qnode)().resources.quantum_operations.get(
            "Toffoli", 0
        )
    ...
    def test_template_with_swap(self):
        ...
        gate_qnode = qp.specs(qnode)().resources.quantum_operations
        swap_qnode = gate_qnode.get("SWAP", 0)
        cnot_qnode = gate_qnode.get("CNOT", 0)

        gate_qnode_optimized = qp.specs(optimized_qnode)().resources.quantum_operations
    ...
    def test_template_with_multiple_control_swap(self):
        ...
        def circuit():
            ...
            qp.PauliZ(wires=3)
            qp.CNOT(wires=[2, 0])
            qp.CSWAP(wires=[0, 1, 3])
            qp.CSWAP(wires=[0, 1, 3])
            qp.CZ(wires=[0, 2])
            qp.PauliX(wires=1)
            qp.PauliX(wires=3)
            qp.Toffoli(wires=[2, 0, 3])
            qp.CSWAP(wires=[0, 3, 1])
            qp.CSWAP(wires=[0, 1, 3])
            return qp.expval(qp.PauliX(wires=0))
    ...
    def test_parametrized_pattern_matching(self):
        ...
        with qp.queuing.AnnotatedQueue() as q_template_rz:
            qp.RZ(0.1, wires=[0])
            qp.RZ(-0.1, wires=[0])

        template_rz = qp.tape.QuantumScript.from_queue(q_template_rz)
        with qp.queuing.AnnotatedQueue() as q_template_rx:
            qp.RX(0.2, wires=[0])
            qp.RX(-0.2, wires=[0])

        template_rx = qp.tape.QuantumScript.from_queue(q_template_rx)
        dev = qp.device("default.qubit", wires=5)
        ...
        rx_qnode = qp.specs(qnode)(0.1, 0.2).resources.quantum_operations.get("RX", 0)
        rx_optimized_qnode = qp.specs(optimized_qnode)(0.1, 0.2).resources.quantum_operations.get(
            "RX", 0
        )

        rz_qnode = qp.specs(qnode)(0.1, 0.2).resources.quantum_operations.get("RZ", 0)
        rz_optimized_qnode = qp.specs(optimized_qnode)(0.1, 0.2).resources.quantum_operations.get(
            "RZ", 0
        )
        ...
        optimized_tape = qp.workflow.construct_tape(optimized_qnode)(0.1, 0.2)
        assert len(optimized_tape.operations) == 8
        assert rx_optimized_qnode == 0
        assert rz_optimized_qnode == 0

        assert qnode_res == optimized_qnode_res
        assert np.allclose(qp.matrix(optimized_qnode)(0.1, 0.2), qp.matrix(qnode)(0.1, 0.2))
    ...
    def test_multiple_patterns(self):
        ...
        with qp.queuing.AnnotatedQueue() as q_template_cnot:
            qp.CNOT(wires=[0, 1])
            qp.CNOT(wires=[0, 1])

        template_cnot = qp.tape.QuantumScript.from_queue(q_template_cnot)
        with qp.queuing.AnnotatedQueue() as q_template_x:
            qp.PauliX(wires=[0])
            qp.PauliX(wires=[0])

        template_x = qp.tape.QuantumScript.from_queue(q_template_x)
        with qp.queuing.AnnotatedQueue() as q_template_z:
            qp.PauliZ(wires=[0])
            qp.PauliZ(wires=[0])

        template_z = qp.tape.QuantumScript.from_queue(q_template_z)
        dev = qp.device("default.qubit", wires=5)
    ...
    def test_vbe_adder_3_pattern_matching(self):
        ...
        qnode = qp.QNode(vbe_adder_3, dev)
        qnode_res = qnode()
    ...
    def test_transform_tape(self):
        ...
        measurement = [qp.expval(qp.X(0))]
        tape = qp.tape.QuantumScript(operations, measurement)

        pattern = qp.tape.QuantumScript([qp.S(0), qp.S(0), qp.Z(0)])

        batch, postprocessing_fn = qp.transforms.pattern_matching_optimization(
...
class TestPatternMatching:
    ...
    def test_pattern_matching_paper_example(self):
        ...
        with qp.queuing.AnnotatedQueue() as q_pattern:
            qp.CNOT(wires=[3, 0])
            qp.PauliX(wires=4)
            qp.PauliZ(wires=0)
            qp.CNOT(wires=[4, 2])
        ...
        pattern = qp.tape.QuantumScript.from_queue(q_pattern)
