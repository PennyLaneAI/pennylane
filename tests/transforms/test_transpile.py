...
class TestTranspile:
    ...
    def test_transpile_global_phase(self):
        ...
        tape = qp.workflow.construct_tape(original_qnode)()
        transpiled_tape = qp.workflow.construct_tape(transpiled_qnode)()
        original_ops = list(tape)
        ...
        assert isinstance(transpiled_ops[3], qp.CNOT)
        assert transpiled_ops[3].wires == qp.wires.Wires([0, 1])

        assert isinstance(transpiled_ops[4], qp.GlobalPhase)

        assert qp.math.allclose(original_probs, transpiled_probs, atol=np.finfo(float).eps)
