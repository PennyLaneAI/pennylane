...
test_cases_zyz = [
    # Special unitaries
    (I, [0.0, 0.0, 0.0, 0]),
    (Z, [np.pi / 2, 0.0, np.pi / 2, -1.5707963267948966]),
    (S, [np.pi / 4, 0.0, np.pi / 4, -0.7853981633974483]),
    (T, [np.pi / 8, 0.0, np.pi / 8, -0.39269908047469393]),
    (H, [np.pi, np.pi / 2, 0.0, -1.5707963267948966]),
    (X, [np.pi / 2, np.pi, 10.995574287564276, -1.5707963267948966]),
    # Single rotations
    (qml.RZ(0.3, wires=0).matrix(), [0.15, 0.0, 0.15, 0]),
    (qml.RZ(-0.5, wires=0).matrix(), [12.316370614359172, 0.0, 12.316370614359172, 0]),
    (qml.Rot(0.2, 0.5, -0.3, wires=0).matrix(), [0.2, 0.5, 12.266370614359172, 0]),
    ...
    (
        ...
        [11.566370614359172, 2.0, 9.566370614359172, -0.020000000000000042],
    ),
...
test_cases_xyx = [
    ...
    (X, [4.71238898038469, 0.0, 10.995574287564276, -1.5707963267948966]),
    (Y, [1 / 2 * np.pi, np.pi, 1 / 2 * np.pi, -1.5707963267948966]),
    (Z, [10.995574287564276, np.pi, 1 / 2 * np.pi, -1.5707963267948966]),
...
test_cases_xzx = [
    ...
    (Y, [np.pi / 2, np.pi, 7 / 2 * np.pi, -np.pi / 2]),
    (Z, [0, np.pi, 0, -np.pi / 2]),
...
test_cases_zxz = [
    ...
    (qml.RZ(0.3, wires=0).matrix(), [0.15, 0.0, 0.15, 0]),
    (qml.RZ(-0.5, wires=0).matrix(), [12.316370614359172, 0.0, 12.316370614359172, 0]),
    (qml.Rot(0.2, 0.5, -0.3, wires=0).matrix(), [11.195574287564275, 0.5, 1.27079633, 0]),
    (
        np.array(
            [
                [0, -9.831019270939975e-01 + 0.1830590094588862j],
