...
single_qubit_decomps_zyz = [
    ...
    (
        ...
        typeof_gates_zyz,
        [12.316370614359172, 0.0, 12.316370614359172, 0],
    ),
    (
        qml.Rot(0.2, 0.5, -0.3, wires=0).matrix(),
        typeof_gates_zyz,
        [0.2, 0.5, 12.266370614359172, 0],
    ),
    (
        np.array(
            [
...
single_qubit_decomps_xyx = [
    ...
    (
        qml.QubitUnitary(qml.RZ.compute_matrix(np.array([np.pi, np.pi / 2])), wires=0).matrix(),
        typeof_gates_xyx,
        [[10.99557429, 10.99557429], [np.pi, np.pi / 2], [np.pi / 2, np.pi / 2], [0, 0]],
    ),
    (
...
single_qubit_decomps_xzx = [
    ...
    (I, typeof_gates_xzx, [0, 0, 0, 0]),
    (X, typeof_gates_xzx, [np.pi / 2, 0, np.pi / 2, -np.pi / 2]),
    (Y, typeof_gates_xzx, [np.pi / 2, np.pi, 7 / 2 * np.pi, -np.pi / 2]),
    (Z, typeof_gates_xzx, [0, np.pi, 0, -np.pi / 2]),
    (H, typeof_gates_xzx, [np.pi / 2, np.pi / 2, np.pi / 2, -np.pi / 2]),
    (
        qml.QubitUnitary(qml.RZ.compute_matrix(np.array([np.pi, np.pi / 2])), wires=0).matrix(),
        typeof_gates_xzx,
...
class TestQubitUnitaryXZXDecomposition:
    ...
    @pytest.mark.parametrize("U,expected_gates,expected_params", single_qubit_decomps_xzx)
    def test_xzx_decomposition(self, U, expected_gates, expected_params):
        """Test that a one-qubit matrix in isolation is correctly decomposed."""

        obtained_gates = one_qubit_decomposition(U, Wires("a"), "XZX", return_global_phase=True)
        _run_assertions(U, expected_gates, expected_params, obtained_gates)

    @pytest.mark.torch
    @pytest.mark.parametrize("U,expected_gates,expected_params", single_qubit_decomps_xzx)
    def test_xzx_decomposition_torch(self, U, expected_gates, expected_params):
        """Test that a one-qubit operation in Torch is correctly decomposed."""
    ...
    @pytest.mark.tf
    @pytest.mark.parametrize("U,expected_gates,expected_params", single_qubit_decomps_xzx)
    def test_xzx_decomposition_tf(self, U, expected_gates, expected_params):
        """Test that a one-qubit operation in Tensorflow is correctly decomposed."""
    ...
    @pytest.mark.jax
    @pytest.mark.parametrize("U,expected_gates,expected_params", single_qubit_decomps_xzx)
    def test_xzx_decomposition_jax(self, U, expected_gates, expected_params):
        """Test that a one-qubit operation in JAX is correctly decomposed."""
...
single_qubit_decomps_zxz = [
    ...
    (
        qml.Rot(0.2, 0.5, -0.3, wires=0).matrix(),
        typeof_gates_zxz,
        [11.195574287564275, 0.5, 1.27079633, 0],
    ),
    (
        np.array(
            [
    ...
    (H, typeof_gates_zxz, [np.pi / 2, np.pi / 2, np.pi / 2, -np.log(1j) / 1j]),
    (X, typeof_gates_zxz, [0, np.pi, 4 * np.pi, -np.log(1j) / 1j]),
    (
        np.exp(1j * 0.02) * qml.Rot(-1.0, 2.0, -3.0, wires=0).matrix(),
        typeof_gates_zxz,
        [
...
def _run_assertions_rot(U, expected_gate, expected_params, obtained_gates):
    """Runs assertions for the rot decomposition."""
...
class TestOneQubitRotDecomposition:
    ...
    @pytest.mark.parametrize("U,expected_gate,expected_params", single_qubit_decomps_rot)
    def test_one_qubit_decomposition_rot(self, U, expected_gate, expected_params):
        """Test that a one-qubit matrix in isolation is correctly decomposed to RZ or Rot gate."""

        obtained_gates = one_qubit_decomposition(U, Wires("a"), "rot", return_global_phase=True)

        _run_assertions_rot(U, expected_gate, expected_params, obtained_gates)

    @pytest.mark.torch
    @pytest.mark.parametrize("U,expected_gate,expected_params", single_qubit_decomps_rot)
    def test_rot_decomposition_torch(self, U, expected_gate, expected_params):
        """Test that a one-qubit operation in Torch is correctly decomposed."""
    ...
    @pytest.mark.tf
    @pytest.mark.parametrize("U,expected_gate,expected_params", single_qubit_decomps_rot)
    def test_rot_decomposition_tf(self, U, expected_gate, expected_params):
        """Test that a one-qubit operation in Tensorflow is correctly decomposed."""
    ...
    @pytest.mark.jax
    @pytest.mark.parametrize("U,expected_gate,expected_params", single_qubit_decomps_rot)
    def test_rot_decomposition_jax(self, U, expected_gate, expected_params):
        """Test that a one-qubit operation in JAX is correctly decomposed."""
