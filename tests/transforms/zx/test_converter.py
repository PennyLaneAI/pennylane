...
class TestConvertersZX:
    ...
    def test_supported_operations(self, operation, script):
        ...
        I = qp.math.eye(2 ** len(operation.wires))
        ...
        matrix_qscript = qp.matrix(qs, wire_order=qs.wires)

        zx_g = qp.transforms.to_zx(qs)
        ...
        mat_product = qp.math.dot(matrix_qscript, qp.math.conj(matrix_zx.T))
        # Remove global phase
        ...
        qscript_back = qp.transforms.from_zx(zx_g)
        assert isinstance(qscript_back, qp.tape.QuantumScript)

        matrix_qscript_back = qp.matrix(qscript_back, wire_order=list(range(len(qs.wires))))

        # Check whether the two matrices are each others conjugate transposes
        mat_product = qp.math.dot(matrix_qscript, qp.math.conj(matrix_qscript_back.T))
        # Remove global phase
        if not np.allclose(mat_product[0, 0], 1.0):
            mat_product /= mat_product[0, 0]
    ...
    def test_circuit(self, decompose):
        ...
        qscript_back = qp.transforms.from_zx(zx_g, decompose_phases=decompose)
        assert isinstance(qscript_back, qp.tape.QuantumScript)
    ...
    def test_expand_measurements(self):
        ...
        measurements = [qp.expval(qp.PauliZ(0) @ qp.PauliX(1))]

        qs = QuantumScript(operations, measurements)
        zx_g = qp.transforms.to_zx(qs, expand_measurements=True)
        assert isinstance(zx_g, pyzx.graph.graph_s.GraphS)

        # Add rotation Hadamard because of PauliX
        operations.append(qp.RY(-np.pi / 2, wires=1))
        operations_with_rotations = operations
        qscript_with_rot = QuantumScript(operations_with_rotations, [])
        matrix_qscript = qp.matrix(qscript_with_rot, wire_order=[0, 1])
    ...
    def test_embeddings(self):
        ...
        prep = [qp.AngleEmbedding(features=[1, 2], wires=range(2), rotation="Z")]
    ...
    def test_cx_gate(self):
        ...
        expected_op = [qp.Hadamard(wires=[1]), qp.CNOT(wires=[1, 0]), qp.Hadamard(wires=[1])]
    ...
    def test_qnode_decorator(self):
        ...
        @qp.transforms.to_zx(expand_measurements=True)
        @qp.qnode(device=dev)
        def circuit(p):
            qp.RZ(p[0], wires=1)
            qp.RZ(p[1], wires=1)
