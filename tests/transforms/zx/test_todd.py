...
class TestTODD:
    ...
    def test_hermitian_involutory_gates_cancellation(self, gate):
        ...
        (new_qs,), _ = qp.transforms.zx.todd(qs)
    ...
    @pytest.mark.parametrize(
        ...
        (
            ...
            (0.25 * np.pi, [qp.T(0)]),
            (0.5 * np.pi, [qp.S(0)]),
            (0.75 * np.pi, [qp.T(0), qp.S(0)]),
            (np.pi, [qp.Z(0)]),
            (1.25 * np.pi, [qp.T(0), qp.Z(0)]),
            (1.5 * np.pi, [qp.adjoint(qp.S(0))]),
            (1.75 * np.pi, [qp.T(0), qp.adjoint(qp.S(0))]),
            (2 * np.pi, []),
        ),
    )
    ...
    def test_transformed_tape(self, measurements):
        ...
        (transformed_tape,), _ = qp.transforms.zx.todd(original_tape)
    ...
    def test_equivalent_state(self):
        ...
        reduced_circ = qp.transforms.zx.todd(original_circ)
