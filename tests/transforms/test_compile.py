...
class TestCompile:
    ...
    def test_compile_non_commuting_observables(self):
        ...
        ops = (qp.RX(0.1, 0), qp.RX(0.2, 0), qp.Barrier(only_visual=True), qp.X(0), qp.X(0))
        ms = (qp.expval(qp.X(0)), qp.expval(qp.Y(0)))
        tape = qp.tape.QuantumScript(ops, ms, shots=50)

        [out], _ = qp.compile(tape)
        expected = qp.tape.QuantumScript((qp.RX(0.3, 0),), ms, shots=50)
    ...
    def test_compile_mcm(self):
        ...
        m0 = qp.measure(0)
        ops = [qp.X(0), qp.X(0), *m0.measurements, qp.ops.Conditional(m0, qp.S(0))]
        tape = qp.tape.QuantumScript(ops, [qp.probs()], shots=50)

        [new_tape], _ = qp.compile(tape)
...
class TestCompileIntegration:
    ...
    def test_compile_empty_pipeline(self, wires):
        ...
        tape = qp.workflow.construct_tape(qnode)(0.3, 0.4, 0.5)
        names_expected = [op.name for op in tape.operations]
        wires_expected = [op.wires for op in tape.operations]

        transformed_tape = qp.workflow.construct_tape(transformed_qnode)(0.3, 0.4, 0.5)
    ...
    def test_compile_default_pipeline(self, wires):
        ...
        qfunc = build_qfunc(wires)
        dev = qp.device("default.qubit", wires=Wires(wires))
    ...
    def test_compile_default_pipeline_removes_barrier(self):
        ...
        tape = qp.tape.QuantumScript([qp.PauliX(0), qp.Barrier([0, 1]), qp.CNOT([0, 1])])
    ...
    def test_compile_decomposes_state_prep(self):
        ...
        class DummyStatePrep(qp.operation.StatePrepBase):
        ...
        tape = qp.tape.QuantumScript([state_prep_op])

        [compiled_tape], _ = qp.compile(tape)
        expected = qp.tape.QuantumScript(state_prep_op.decomposition())
        qp.assert_equal(compiled_tape, expected)
    ...
    def test_compile_template(self):
        ...
        def qfunc(x, params):
            ...
            qp.adjoint(
                qp.adjoint(qp.templates.BasicEntanglerLayers(params, wires=range(3)))
            ) ** 2
        ...
        tape = qp.workflow.construct_tape(transformed_qnode)(x, params)
        compare_operation_lists(tape.operations, names_expected, wires_expected)


def qfunc_emb(x, params):
    qp.templates.AngleEmbedding(x, wires=range(3))
    qp.templates.BasicEntanglerLayers(params, wires=range(3))
    return qp.expval(qp.PauliZ(wires=2))
...
dev_3wires = qp.device("default.qubit", wires=3)
...
class TestCompileInterfaces:
    ...
    def test_compile_autograd(self, diff_method):
        ...
        original_qnode = qp.QNode(qfunc_emb, dev_3wires, diff_method=diff_method)
        transformed_qnode = qp.QNode(transformed_qfunc_emb, dev_3wires, diff_method=diff_method)

        x = np.array([0.1, 0.2, 0.3], requires_grad=False)
        params = np.ones((2, 3))

        # Check that the numerical output is the same
        assert qp.math.allclose(original_qnode(x, params), transformed_qnode(x, params))

        # Check that the gradient is the same
        assert qp.math.allclose(
            qp.grad(original_qnode)(x, params), qp.grad(transformed_qnode)(x, params)
        )
    ...
    def test_compile_torch(self):
        ...
        original_qnode = qp.QNode(qfunc_emb, dev_3wires, diff_method="parameter-shift")
        ...
        assert qp.math.allclose(original_params.grad, transformed_params.grad)

        # Check operation list
        tape = qp.workflow.construct_tape(transformed_qnode)(transformed_x, transformed_params)
        ops = tape.operations
        compare_operation_lists(ops, expected_op_list, expected_wires_list)
    ...
    def test_compile_jax_jit(self, diff_method):
        ...
        def test_qfunc(x):
            qp.Rot(x, x + 1, x + 2, wires=0)
            qp.RX(2 * x, wires=0)
            qp.CNOT(wires=[0, 1])
            qp.QubitUnitary(qp.RX.compute_matrix(x), wires=1)
            qp.QubitUnitary(qp.RZ.compute_matrix(x + 1), wires=1)
            return qp.expval(qp.PauliX(0) @ qp.PauliZ(1))

        original_qnode = qp.QNode(test_qfunc, dev, diff_method=diff_method)

        pipeline = [cancel_inverses, unitary_to_rot, single_qubit_fusion]

        compiled_qfunc = qp.compile(test_qfunc, pipeline=pipeline)
        compiled_qnode = qp.QNode(compiled_qfunc, dev, diff_method=diff_method)
        ...
        # Check that the numerical output is the same
        assert qp.math.allclose(original_qnode(x), jitted_compiled_qnode(x))
