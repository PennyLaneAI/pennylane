...
single_qubit_decompositions = [
    ...
    (qp.RZ(0.3, wires=0).matrix(), typeof_gates_zyz, [0.15, 0.0, 0.15]),
    (
        qp.RZ(-0.5, wires=0).matrix(),
        typeof_gates_zyz,
        [4 * np.pi - 0.25, 0.0, 4 * np.pi - 0.25],
    ),
    (qp.Rot(0.2, 0.5, -0.3, wires=0).matrix(), typeof_gates_zyz, [0.2, 0.5, 4 * np.pi - 0.3]),
    (
        np.array(
            [
...
class TestRecursiveDecomposition:
    ...
    def test_two_qubit_decomposition_seperable_tensor_product(self):
        ...
        op = qp.QubitUnitary(U, wires=range(2))
    ...
    def test_non_qubit_unitary_operations(self):
        ...
        decomp = _recursively_decompose_qubit_unitary(qp.RZ(0.5, 0))
        assert decomp == [qp.RZ(0.5, 0)]
...
class TestDecomposeSingleQubitUnitaryTransform:
    ...
    def test_unitary_to_rot_device_transform_programqnode(self):
        ...
        U = np.exp(1j * 0.02) * qp.Rot(-1.0, 2.0, -3.0, wires=0).matrix()
    ...
    def test_unitary_to_rot_too_big_unitary(self):
        ...
        tof = qp.Toffoli(wires=[0, 1, 2]).matrix()

        def qfunc_with_large_qubit_unitary():
            qp.QubitUnitary(H, wires="a")
            qp.QubitUnitary(tof, wires=["a", "b", "c"])
    ...
    def test_unitary_to_rot_jax_jit(self, U, expected_gates, expected_params):
        ...
        def test_qfunc(U):
            qfunc(U)
            return qp.expval(qp.PauliX("a") @ qp.PauliZ("b"))

        original_qnode = qp.QNode(test_qfunc, dev)
        transformed_qnode = qp.QNode(unitary_to_rot(test_qfunc), dev)
...
def original_qfunc_for_grad(angles):
    qp.Hadamard(wires="a")
    qp.RZ(angles[0], wires="a")
    qp.RX(angles[1], wires="b")
    qp.CNOT(wires=["b", "a"])
...
class TestQubitUnitaryDifferentiability:
    ...
    def test_gradient_unitary_to_rot(self, rot_angles, diff_method):
        ...
        assert qp.math.allclose(original_qnode(angles), transformed_qnode(angles), atol=1e-7)

        original_grad = qp.grad(original_qnode)(angles)
        transformed_grad = qp.grad(transformed_qnode)(angles)
    ...
    def test_gradient_unitary_to_rot_torch(self, rot_angles, diff_method):
        ...
        assert qp.math.allclose(original_result, transformed_result, atol=1e-7)
...
def test_unitary_to_rot_multiple_two_qubit(num_reps, seed):
    ...
    original_qnode = qp.QNode(my_circuit, dev)
    transformed_qnode = qp.QNode(unitary_to_rot(my_circuit), dev)

    original_matrix = qp.matrix(original_qnode)()
    transformed_matrix = qp.matrix(transformed_qnode)()
...
class TestTwoQubitUnitaryDifferentiability:
    ...
    def test_gradient_unitary_to_rot_two_qubit(self, diff_method):
        ...
        original_qnode = qp.QNode(two_qubit_decomp_qnode, device=dev, diff_method=diff_method)

        transformed_qfunc = unitary_to_rot(two_qubit_decomp_qnode)
        transformed_qnode = qp.QNode(transformed_qfunc, dev, diff_method=diff_method)

        assert qp.math.allclose(original_qnode(x, y, z), transformed_qnode(x, y, z))

        tape = qp.workflow.construct_tape(transformed_qnode)(x, y, z)
        assert len(tape.operations) == 29

        original_grad = qp.grad(original_qnode)(x, y, z)
        transformed_grad = qp.grad(transformed_qnode)(x, y, z)

        assert qp.math.allclose(original_grad, transformed_grad, atol=1e-6)
    ...
    def test_gradient_unitary_to_rot_torch_two_qubit(self):
        ...
        tape = qp.workflow.construct_tape(transformed_qnode)(
    ...
    def test_gradient_unitary_to_rot_two_qubit_jax(self, diff_method):
        ...
        assert qp.math.allclose(original_qnode(x), transformed_qnode(x))

        tape = qp.workflow.construct_tape(transformed_qnode)(x)
        assert len(tape.operations) == 27
