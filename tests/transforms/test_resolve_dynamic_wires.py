...
def test_error_if_no_available_wires():
    ...
    tape = qp.tape.QuantumScript([qp.allocation.Allocate.from_num_wires(2)])

    with pytest.raises(qp.exceptions.AllocationError, match="no wires left to allocate"):
...
def test_error_if_use_deallocated_wire():
    ...
    op = qp.allocation.Allocate.from_num_wires(1)
    tape = qp.tape.QuantumScript([op, qp.allocation.Deallocate(op.wires), qp.X(op.wires)])
    with pytest.raises(qp.exceptions.AllocationError, match="Encountered deallocated wires"):
        qp.transforms.resolve_dynamic_wires(tape, min_int=0)
...
def test_deallocated_wire_in_measurement():
    ...
    tape = qp.tape.QuantumScript(
        [op, qp.allocation.Deallocate(op.wires)], [qp.probs(wires=op.wires)]
    )
    with pytest.raises(qp.exceptions.AllocationError, match="Encountered deallocated wires"):
        qp.transforms.resolve_dynamic_wires(tape, min_int=0)
...
def test_highly_nested_min_int():
    ...
    [new_tape], fn = qp.transforms.resolve_dynamic_wires(tape, min_int=5)
...
def test_map_measurement_on_dynamic_wires():
    ...
    alloc_op = qp.allocation.Allocate.from_num_wires(1, state=AllocateState.ZERO)
    tape = qp.tape.QuantumScript([alloc_op], [qp.probs(wires=alloc_op.wires)])

    [new_tape], fn = qp.transforms.resolve_dynamic_wires(tape, zeroed=("a",))
    assert fn(("a",)) == "a"

    expected = qp.tape.QuantumScript([], [qp.probs(wires="a")])
...
def test_shots_preserved():
    ...
    tape = qp.tape.QuantumScript([op, qp.allocation.Deallocate(op.wires)], [qp.state()], shots=500)
    [new_tape], _ = qp.transforms.resolve_dynamic_wires(tape, min_int=0)
    assert new_tape.shots == qp.measurements.Shots(500)
...
class TestFirstExtraction:
    ...
    def test_use_zeroed_wire_as_zeroed(self):
        ...
        alloc_op = qp.allocation.Allocate.from_num_wires(1, state=AllocateState.ZERO)
        tape = qp.tape.QuantumScript(
            [alloc_op, qp.X(alloc_op.wires), qp.allocation.Deallocate(alloc_op.wires)]
        )

        [new_tape], _ = qp.transforms.resolve_dynamic_wires(tape, zeroed=("a",), any_state=("b",))

        assert len(new_tape.operations) == 1
        qp.assert_equal(new_tape[0], qp.X("a"))

    def test_use_any_state_as_any_state(self):
        """Test that an any_state wire is used when one is requested and available."""

        alloc_op = qp.allocation.Allocate.from_num_wires(1, state=AllocateState.ANY)
        tape = qp.tape.QuantumScript(
            [alloc_op, qp.X(alloc_op.wires), qp.allocation.Deallocate(alloc_op.wires)]
        )
        ...
        assert len(new_tape.operations) == 1
        qp.assert_equal(new_tape[0], qp.X("b"))
    ...
    def test_use_zeroed_if_no_any_state_available(self):
        ...
        [new_tape], _ = qp.transforms.resolve_dynamic_wires(tape, zeroed=("a",))
    ...
    def test_reset_any_state_if_no_zeroed_available(self):
        ...
        [new_tape], _ = qp.transforms.resolve_dynamic_wires(tape, any_state=("a",))

        assert len(new_tape) == 2
        assert isinstance(new_tape[0], qp.ops.MidMeasure)
        assert new_tape[0].wires == qp.wires.Wires(("a",))
    ...
    def test_no_reset_if_forbidden(self):
        ...
        with pytest.raises(qp.exceptions.AllocationError, match="no wires left to allocate"):
            qp.transforms.resolve_dynamic_wires(tape, any_state=("a",), allow_resets=False)
...
class TestReuse:
    ...
    def test_reuse_zeroed_qubit(self):
        ...
        alloc1 = qp.allocation.Allocate.from_num_wires(1, state=AllocateState.ZERO, restored=True)
        dealloc1 = qp.allocation.Deallocate(alloc1.wires)
        alloc2 = qp.allocation.Allocate.from_num_wires(1, state=AllocateState.ZERO, restored=True)
        dealloc2 = qp.allocation.Deallocate(alloc2.wires)

        tape = qp.tape.QuantumScript(
            [alloc1, qp.X(alloc1.wires), dealloc1, alloc2, qp.Y(alloc2.wires), dealloc2]
        )

        [new_tape], _ = qp.transforms.resolve_dynamic_wires(tape, zeroed=("a",))

        expected = qp.tape.QuantumScript([qp.X("a"), qp.Y("a")])
    ...
    def test_reuse_garbage_qubit(self):
        ...
        alloc1 = qp.allocation.Allocate.from_num_wires(1, state=AllocateState.ANY, restored=False)
        dealloc1 = qp.allocation.Deallocate(alloc1.wires)
        alloc2 = qp.allocation.Allocate.from_num_wires(1, state=AllocateState.ANY, restored=False)
        dealloc2 = qp.allocation.Deallocate(alloc2.wires)
    ...
    def test_reused_garbage_qubit_as_zeroed(self):
        ...
        alloc1 = qp.allocation.Allocate.from_num_wires(1, state=AllocateState.ANY, restored=False)
        dealloc1 = qp.allocation.Deallocate(alloc1.wires)
        alloc2 = qp.allocation.Allocate.from_num_wires(1, state=AllocateState.ZERO, restored=False)
        dealloc2 = qp.allocation.Deallocate(alloc2.wires)
        ...
        qp.assert_equal(new_tape[0], qp.X("a"))
        qp.assert_equal(new_tape[2], qp.Y("a"))

        assert isinstance(new_tape[1], qp.ops.MidMeasure)
        assert new_tape[1].wires == qp.wires.Wires(("a",))
    ...
    def test_error_if_no_zeroed_wires_left_no_reset(self):
        ...
        with pytest.raises(qp.exceptions.AllocationError, match="no wires left to allocate."):
            qp.transforms.resolve_dynamic_wires(tape, zeroed=("a",), allow_resets=False)
    ...
    def test_no_resets_min_int(self):
        ...
        [new_tape], _ = qp.transforms.resolve_dynamic_wires(tape, allow_resets=False, min_int=2)

        expected = qp.tape.QuantumScript([qp.X(2), qp.Y(3)])
