...
class TestGradients:
    ...
    def test_autograd(self, tol):
        """Tests that the output of the hamiltonian gradient transform
        can be differentiated using autograd, yielding second derivatives."""
        dev = qml.device("default.qubit.autograd", wires=2)
        params = np.array([0.543, -0.654], requires_grad=True)
        params_ham = np.array([-1.5, 2.], requires_grad=True)
        ...
        def cost_fn(x):
            with qml.tape.JacobianTape() as tape:
                ...
                qml.RY(x[1], wires=[1])
                qml.CNOT(wires=[0, 1])
                qml.expval(qml.Hamiltonian(params_ham, [qml.PauliZ(0), qml.PauliX(1)]))

            tape.trainable_params = {0, 1, 2, 3}
            tapes, fn = qml.gradients.hamiltonian_grad(tape)
