...
class TestComputeVJP:
    ...
    def test_compute_single_measurement_single_params(self):
        ...
        vjp = qp.gradients.compute_vjp_single(dy, jac)
    ...
    def test_compute_multiple_measurement_single_params(self):
        ...
        vjp = qp.gradients.compute_vjp_multi(dy, jac)
    ...
    def test_dtype_torch(self, dtype1, dtype2):
        ...
        assert qp.gradients.compute_vjp_multi(dy, jac)[0].dtype == dtype1
    ...
    def test_dtype_tf(self, dtype1, dtype2):
        ...
        assert qp.gradients.compute_vjp_multi(dy, jac)[0].dtype == dtype
...
class TestVJP:
    ...
    def test_no_trainable_parameters(self):
        ...
        dy = np.array([1.0])
        tapes, fn = qp.gradients.vjp(tape, dy, param_shift)
    ...
    def test_dtype_matches_dy(self, dtype):
        ...
        _, func = qp.gradients.vjp(tape, dy, qp.gradients.param_shift)
...
class TestVJPGradients:
    ...
    def test_autograd(self, tol):
        ...
        res = qp.jacobian(cost_fn)(params, dy)
        assert np.allclose(res, qp.jacobian(expected)(params), atol=tol, rtol=0)
    ...
    def test_torch(self, tol):
        ...
        tape.trainable_params = {0, 1}
        tapes, fn = qp.gradients.vjp(tape, dy, param_shift)
        vjp = fn(qp.execute(tapes, dev, qp.gradients.param_shift))
        ...
        exp = qp.jacobian(lambda x: expected(x)[0])(params_np)
    ...
    def test_tf(self, tol, seed):
        ...
        assert np.allclose(res, qp.jacobian(expected)(params_np), atol=tol, rtol=0)
    ...
    #     nwires = 5
    #     dev = qp.device("lightning.qubit", wires=nwires)
    #     dev.C_DTYPE = vanilla_numpy.complex64
    #     dev.R_DTYPE = vanilla_numpy.float32

    #     @qp.qnode(dev, interface="tf", diff_method="adjoint")
    #     def circuit(weights, features):
    #         for i in range(nwires):
    #            qp.RX(features[i], wires=i)
    ...
    def test_jax(self, tol):
        ...
        exp = qp.jacobian(expected)(params_np)
        assert np.allclose(res, exp, atol=tol, rtol=0)
...
class TestBatchVJP:
    ...
    def test_one_tape_no_trainable_parameters(self):
        ...
        v_tapes, fn = qp.gradients.batch_vjp(tapes, dys, param_shift)
    ...
    def test_reduction_append(self):
        ...
        v_tapes, fn = qp.gradients.batch_vjp(tapes, dys, param_shift, reduction="append")
        res = fn(dev.execute(v_tapes))
    ...
    def test_reduction_extend(self):
        ...
        v_tapes, fn = qp.gradients.batch_vjp(tapes, dys, param_shift, reduction="extend")
        res = fn(dev.execute(v_tapes))
    ...
    def test_batched_params_probs_jacobian(self):
        ...
        x0, x1 = 0.5, 0.8
        ops = [qp.RX(x0, 0), qp.RX(x1, 0), qp.RY(data, 0)]
        tape = qp.tape.QuantumScript(ops, [qp.probs(wires=0)], trainable_params=[0, 1])
        dy = np.array([[0.6, -0.7], [0.2, -0.7], [-5.2, 0.6]])
        v_tapes, fn = qp.gradients.batch_vjp([tape], [dy], qp.gradients.param_shift)
