...
class TestGenerateTapesAndCoeffs:
    ...
    def check_tapes_and_coeffs_equality(self, grad_tapes, tup, expected, broadcasted):
        ...
        if broadcasted and num_tapes > 0:
            t = grad_tapes[0]
            assert len(t.operations) == len(old_tape.operations) + 1
            expected_ops = copy.copy(old_tape.operations)
            angles = np.array([np.pi / 2, -np.pi / 2] * len(words))
            words = tuple(w for w in words for _ in (0, 1))
            expected_ops.insert(insert_idx, qml.PauliRot(angles, words, wires))
    ...
    def test_raises_non_pulse_op(self):
        ...
        with pytest.raises(ValueError, match="pulse_generator does not support differentiating"):
            _generate_tapes_and_coeffs(tape, 0, 1e-6, cache, False)

    @pytest.mark.parametrize("add_constant", [False, True])
    @pytest.mark.parametrize("bcast", [False, True])
    def test_single_op_single_term(self, add_constant, bcast):
    ...
    @pytest.mark.parametrize("bcast", [False, True])
    def test_single_op_multi_term(self, same_terms, bcast):
    ...
    def test_multi_op_multi_term(self):
        ...
        self.check_tapes_and_coeffs_equality(grad_tapes, (start, end, coeffs), expected, False)
        self.check_cache_equality(cache, exp_cache)

        # Second trainable parameter - everything for H0 is cached
        grad_tapes, (start, end, coeffs), cache = _generate_tapes_and_coeffs(
            tape, 1, self.atol, cache, False
        )
...
class TestParshiftAndContract:
    ...
    @pytest.mark.parametrize("bcast", [False, True])
    def test_single_measure_no_shot_vector(self, num_params, num_out, bcast):
        ...
        if bcast:
            results = np.array(results).T[None]
        coeffs = [np.random.random(num_out) for _ in range(num_params)]
        output = _parshift_and_contract(
            results, coeffs, single_measure=True, single_shot_entry=True, use_broadcasting=bcast
        )
    ...
    @pytest.mark.parametrize("bcast", [False, True])
    def test_single_measure_with_shot_vector(self, num_params, num_out, len_shot_vector, bcast):
        ...
        coeffs = [np.random.random(num_out) for _ in range(num_params)]
        output = _parshift_and_contract(
            results, coeffs, single_measure=True, single_shot_entry=False, use_broadcasting=bcast
        )
    ...
    @pytest.mark.parametrize("bcast", [False, True])
    def test_multi_measure_no_shot_vector(self, num_params, num_out, num_measurements, bcast):
        ...
        coeffs = [np.random.random(num_out) for _ in range(num_params)]
        output = _parshift_and_contract(
            results, coeffs, single_measure=False, single_shot_entry=True, use_broadcasting=bcast
        )
    ...
    def test_multi_measure_with_shot_vector(
        self, num_params, num_out, num_measurements, len_shot_vector, bcast
    ):
        """Test _parshift_and_contract with multiple measurements without shot vector."""
        ...
        if bcast:
            results = np.array(results).transpose([1, 2, 0])[None]
        coeffs = [np.random.random(num_out) for _ in range(num_params)]
        output = _parshift_and_contract(
            results, coeffs, single_measure=False, single_shot_entry=False, use_broadcasting=bcast
        )
        # Note that these checks are equal to those for single_measure and single_shot_entry
        assert isinstance(output, tuple)
        assert len(output) == len_shot_vector
