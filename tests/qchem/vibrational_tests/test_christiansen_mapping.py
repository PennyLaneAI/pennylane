...
from pennylane.qchem import BoseWord, BoseSentence, christiansen_mapping
...
def test_christiansen_mapping_bose_word_ps(bosonic_op, result):
...
def test_christiansen_mapping_for_identity():
...
def test_christiansen_mapping_for_identity_ps():
...
@pytest.mark.parametrize(
    "operator",
    (
        BoseWord({(0, 1): "-", (1, 0): "+", (2, 2): "-", (3, 1): "-"}),  # ('1 0^ 2 1')
        BoseWord({(0, 1): "-", (1, 0): "+", (2, 2): "+", (3, 1): "-"}),  # ('1 0^ 2^ 1')
        BoseWord({(0, 3): "-", (1, 0): "+", (2, 2): "+", (3, 3): "-"}),  # ('3 0^ 2^ 3')
        BoseWord({(0, 3): "-", (1, 2): "+", (2, 2): "+", (3, 3): "-"}),  # ('3 2^ 2^ 3')
    ),
)
def test_christiansen_mapping_for_null_operator_bose_word_ps(operator):
    """Test that the christiansen_mapping function works when the result is 0"""

    assert christiansen_mapping(operator, ps=True).simplify() is None

    # in operation return format, '0 * I'
    op = christiansen_mapping(operator).simplify()
...
def test_bose_sentence_identity():
