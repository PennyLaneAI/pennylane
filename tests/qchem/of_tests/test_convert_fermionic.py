...
@pytest.mark.parametrize(
    ...
    [
        (
            of.FermionOperator("3^ 2", 0.5) + of.FermionOperator("0 2^", 1.0),
            fermi.FermiSentence(
                {
        ...
        (
            of.FermionOperator("1^ 2^ 0 3", 1.2) + of.FermionOperator("3^ 2^ 1 0", 4.5),
            fermi.FermiSentence(
                {
        ...
        (
            of.FermionOperator("2^ 2", 1.5),
            fermi.FermiSentence({fermi.FermiWord({(0, 2): "+", (1, 2): "-"}): 1.5}),
        ),
        (of.FermionOperator("0^ 0"), fermi.FermiWord({(0, 0): "+", (1, 0): "-"})),
        (
            of.FermionOperator("1^ 2", 1.5)
            + of.FermionOperator("2^ 1", 1.5)
            + of.FermionOperator("3^ 2", 0.8),
            fermi.FermiSentence(
                {
...
def test_convert_from_openfermion_type_fs():
    ...
    of_op = of.FermionOperator("2^ 3") + of.FermionOperator("1^ 2")
    converted_op = qml.qchem.from_openfermion(of_op)
...
def test_convert_from_openfermion_tol():
    ...
    of_op = (
        of.FermionOperator("2^ 3", 2.0)
        + of.FermionOperator("1^ 2", 3.0)
        + of.FermionOperator("2^ 1", 0.5)
    )
