...
from pennylane.resource.resource import Resources, OperationWithResources, _count_resources
...
class TestOperationWithResources:
    """Test that the OperationWithResources class is constructed correctly"""

    def test_raise_not_implemented_error(self):
        """Test that a not implemented error is raised if the class is
        initialized without a `resources` method."""

        class CustomOpNoResource(OperationWithResources):
            num_wires = 2

        class CustomOPWithResources(OperationWithResources):
            num_wires = 2
...
class _CustomOpWithResource(OperationWithResources):  # pylint: disable=too-few-public-methods
    num_wires = 2
    name = "CustomOp1"
