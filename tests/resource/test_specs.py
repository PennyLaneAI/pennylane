...
class TestSpecsTransform:
    ...
    #         # Both approaches should ultimately decompose Toffoli gates at device level
    #         # (either through standard decomposition or graph decomposition)
    #         expected_primitive_gates = ["CNOT", "T", "Tdg", "Hadamard"]
            
    #         # Check that at least some primitive gates appear
    #         disabled_primitives = [g for g in expected_primitive_gates if g in gate_types_disabled]
    #         enabled_primitives = [g for g in expected_primitive_gates if g in gate_types_enabled]
            
    #         assert len(disabled_primitives) > 0, f"Expected primitive gates in disabled mode, got: {gate_types_disabled}"
    #         assert len(enabled_primitives) > 0, f"Expected primitive gates in enabled mode, got: {gate_types_enabled}"
            
    #         # Total gate counts should be reasonable for two Toffoli decompositions
    #         assert resources_disabled.num_gates >= 8  # Minimum for 2 Toffoli gates
    #         assert resources_enabled.num_gates >= 8
