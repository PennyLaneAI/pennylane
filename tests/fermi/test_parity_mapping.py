...
def test_error_is_raised_for_dimension_mismatch():
    ...
    with pytest.raises(ValueError, match="Creating/annihilating a particle  on qubit number 6 for a 6 qubit system"):
...
FERMI_WORDS_AND_OPS = [
    ...
    (
        ...
        ([(0.5 + 0j), (0.0 + 0.5j)], [qml.PauliX(0) @ qml.PauliX(1), qml.PauliY(0) @ qml.PauliX(1)]),
    ),
    (
        FermiWord({(0, 0): "+", (1, 0): "-"}), 4,
...
FERMI_OPS_COMPLEX = [
    ...
    (
      ...
      # 0.0625 [Y0 Y4 Z5])
        (
            [0.0625, 0.0625j, 0.0625, 0.0625j, 0.0625, 0.0625j, 0.0625, 0.0625j, -0.0625j, 0.0625, -0.0625j, 0.0625, -0.0625j, 0.0625, -0.0625j, 0.0625],
            [
    ...
    (
        FermiWord({(0, 1): "+", (1, 1): "-", (2, 3): "+", (3, 4): "-", (4,2): "+", (5,5): "-"}),  6,
       ...
       # -0.03125 [Y2 Y4]
        (
            [0.03125, 0.03125j, 0.03125, 0.03125j, -0.03125j, 0.03125, -0.03125j, 0.03125,0.03125,0.03125j,0.03125, 0.03125j, -0.03125j, 0.03125, -0.03125j, 0.03125,-0.03125, -0.03125j, -0.03125, -0.03125j, 0.03125j, -0.03125, 0.03125j, -0.03125, -0.03125, -0.03125j, -0.03125, -0.03125j, 0.03125j, -0.03125, 0.03125j, -0.03125],
            [
                qml.PauliZ(0) @ qml.PauliZ(1) @ qml.PauliX(2) @ qml.PauliZ(3) @ qml.PauliX(4) @ qml.PauliZ(5),
                ...
                qml.PauliZ(0) @ qml.PauliZ(1) @ qml.PauliY(2) @ qml.PauliZ(3) @ qml.PauliY(4) @ qml.PauliZ(5),
...
def test_parity_transform_for_identity_ps():
    ...
    assert parity_transform(FermiWord({}), 2, ps=True) == PauliSentence({PauliWord({0: "I"}): 1.0 + 0.0j})
...
FERMI_AND_PAULI_SENTENCES = [
    ...
    (
        FermiSentence({fw2: 2}), 4,
        PauliSentence({PauliWord({}): (1 + 0j), PauliWord({0: "Z"}): (-1 + 0j)}),
    ),
    (
        FermiSentence({fw1: 1, fw2: 1}), 4,
        PauliSentence(
            {
                PauliWord({0: "Y"}): -0.25j,
