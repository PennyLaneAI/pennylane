...
def basic_entangler_circuit(n_wires, shots=10000, interface="autograd"):
    ...
    @qp.qnode(dev, interface=interface)
    def circuit(x):
        qp.BasicEntanglerLayers(weights=x, wires=range(n_wires))
...
def basic_entangler_circuit_exact_state(n_wires, sub_wires, interface="autograd"):
    ...
    def circuit(x):
        qp.BasicEntanglerLayers(weights=x, wires=range(n_wires))
        return qp.density_matrix(sub_wires)
...
class TestReplaceObs:
    ...
    def test_replace_tape(self):
        ...
        tape = qp.tape.QuantumScript([], [qp.classical_shadow(wires=0)])
        new_tapes, _ = _replace_obs(tape, qp.probs, wires=0)

        assert len(new_tapes) == 1
        assert len(new_tapes[0].operations) == 0
        assert len(new_tapes[0].observables) == 1
        assert isinstance(new_tapes[0].observables[0], qp.measurements.ProbabilityMP)

    def test_replace_qnode(self):
        """Test that the transform works for QNodes"""
        circuit = hadamard_circuit(2, shots=1000)
        circuit = _replace_obs(circuit, qp.probs, wires=[0, 1])
        res = circuit()
...
class TestStateForward:
    ...
    def test_hadamard_state(self, wires, diffable):
        ...
        circuit = qp.shadows.shadow_state(circuit, wires=range(wires), diffable=diffable)
    ...
    def test_partial_state(self, diffable):
        ...
        circuit = qp.shadows.shadow_state(circuit, wires=wires_list, diffable=diffable)
        ...
        assert qp.math.allclose(actual[0], expected[0], atol=1e-1)
        assert qp.math.allclose(actual[1], expected[1], atol=1e-1)
    ...
    def test_large_state_warning(self):
        ...
        with pytest.warns(UserWarning, match=msg):
            qp.shadows.shadow_state(tape, wires=[0, 1, 2, 3, 4, 5, 6, 7], diffable=True)
    ...
    def test_multi_measurement_error(self):
        ...
        def circuit_shadow():
            qp.Hadamard(wires=0)
            qp.CNOT(wires=[0, 1])
            return qp.classical_shadow(wires=[0, 1]), qp.expval(qp.PauliZ(0))
        ...
        def circuit_expval():
            qp.Hadamard(wires=0)
            qp.CNOT(wires=[0, 1])
            return qp.shadow_expval(qp.PauliZ(0)), qp.expval(qp.PauliZ(0))
...
class TestStateForwardInterfaces:
    ...
    def test_qft_state(self, interface, diffable):
        ...
        circuit = qp.shadows.shadow_state(circuit, wires=[0, 1, 2], diffable=diffable)
...
class TestStateBackward:
    ...
    x = np.random.uniform(
        0.8, 2, size=qp.BasicEntanglerLayers.shape(n_layers=1, n_wires=3)
    ).tolist()
    ...
    def test_backward_autograd(self):
        ...
        sub_wires = [[0, 1], [1, 2]]
        shadow_circuit = qp.shadows.shadow_state(shadow_circuit, wires=sub_wires, diffable=True)
        ...
        actual = qp.jacobian(lambda x: qp.math.real(qp.math.stack(shadow_circuit(x))))(x)

        for act, w in zip(qp.math.unstack(actual), sub_wires):
            exact_circuit = basic_entangler_circuit_exact_state(3, w, "autograd")
            expected = qp.jacobian(lambda x: qp.math.real(exact_circuit(x)))(x)

            assert qp.math.allclose(act, expected, atol=1e-1)
    ...
    def test_backward_jax(self):
        ...
        x = jnp.array(self.x)
        actual = jax.jacobian(lambda x: qp.math.real(qp.math.stack(shadow_circuit(x))))(x)

        for act, w in zip(qp.math.unstack(actual), sub_wires):
            exact_circuit = basic_entangler_circuit_exact_state(3, w, "jax")
            expected = jax.jacobian(lambda x: qp.math.real(exact_circuit(x)))(x)
    ...
    def test_backward_tf(self):
        ...
        with tf.GradientTape() as tape:
            out = qp.math.stack(shadow_circuit(x))
    ...
    def test_backward_torch(self):
        ...
        actual = torch.autograd.functional.jacobian(lambda x: qp.math.stack(shadow_circuit(x)), x)
