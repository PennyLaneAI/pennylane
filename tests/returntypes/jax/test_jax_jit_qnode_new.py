...
class TestQubitIntegrationHigherOrder:
    ...
    def test_second_derivative(self, dev_name, diff_method, mode, interface, tol):
        ...
        if diff_method == "adjoint":
            ...
            gradient_kwargs = {"sampler_seed": SEED_FOR_SPSA, "num_directions": 10}
            tol = TOL_FOR_SPSA
...
@pytest.mark.parametrize("interface", ["jax-jit", "jax"])
class TestCV:
...
class TestJIT:
    ...
    def test_gradient(self, dev_name, diff_method, mode, jacobian, tol):
        ...
        @qnode(dev, diff_method=diff_method, interface="jax-jit", mode=mode, **gradient_kwargs)
        def circuit(x):
            qml.RY(x[0], wires=0)
            qml.RX(x[1], wires=0)
            return qml.expval(qml.PauliZ(0))
