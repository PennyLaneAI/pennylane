...
from pennylane.gradients import spsa_gradient
...
class TestSpsaGradient:
    ...
    def test_independent_parameter(self, num_directions, mocker):
        ...
        spy = mocker.spy(qml.gradients.spsa, "generate_multishifted_tapes")
    ...
    def test_no_trainable_params_qnode_autograd(self):
        ...
        with pytest.warns(UserWarning, match="gradient of a QNode with no trainable parameters"):
            res = spsa_gradient(circuit, h=h_val, shots=default_shot_vector)(weights)
    ...
    def test_all_zero_diff_methods(self):
        ...
        all_result = spsa_gradient(circuit, h=h_val, shots=default_shot_vector)(params)
        ...
        for result in all_result:
            ...
            tapes, _ = spsa_gradient(circuit.tape, h=h_val, shots=default_shot_vector)
            assert tapes == []
    ...
    def test_output_shape_matches_qnode(self):
        ...
        transform = [
            qml.math.shape(spsa_gradient(c, h=h_val, shots=default_shot_vector)(x))
            for c in circuits
        ]
...
class TestReturn:
    ...
    def test_1_1(self, shot_vec, meas, shape, op_wires):
        ...
        tapes, fn = spsa_gradient(tape, shots=grad_transform_shots)
        all_res = fn(dev.batch_execute(tapes))
