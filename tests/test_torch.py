...
class TorchQNodeTests(BaseTest):
    ...
    def test_qnode_fail(self):
        ...
        def qf(x):
            qml.RX(x, wires=[0])
            return qml.output.expval(qml.PauliZ(wires=0)), 0.3
        ...
        def qf(x):
            qml.RX(x, wires=[0])
            ex = qml.output.expval(qml.PauliZ(wires=1))
        ...
        def qf(x):
            qml.RX(x, wires=[0])
            ex = qml.output.expval(qml.PauliZ(wires=1))
            return qml.output.expval(qml.PauliZ(wires=0)), ex
        ...
        def qf(x):
            qml.RX(x, wires=[0])
            ev = qml.output.expval(qml.PauliZ(wires=1))
            qml.RY(0.5, wires=[0])
            return ev
        ...
        def qf(x):
            qml.RX(x, wires=[0])
            qml.CNOT(wires=[0, 1])
            return qml.output.expval(qml.PauliZ(wires=0)), qml.output.expval(qml.PauliZ(wires=1)), qml.output.expval(qml.PauliX(wires=0))
    ...
    def test_qnode_array_parameters(self):
        ...
        def circuit_n1s(dummy1, array, dummy2):
            ...
            return qml.output.expval(qml.PauliX(wires=0))  # returns a scalar

        @qml.qnode(self.dev1, interface='torch')
        def circuit_n1v(dummy1, array, dummy2):
            qml.RY(0.5 * array[0,1], wires=0)
            qml.RY(-0.5 * array[1,1], wires=0)
            return qml.output.expval(qml.PauliX(wires=0)),  # note the comma, returns a 1-vector
        ...
        def circuit_nn(dummy1, array, dummy2):
            ...
            return qml.output.expval(qml.PauliX(wires=0)), qml.output.expval(qml.PauliX(wires=1))  # returns a 2-vector
    ...
    def test_array_parameters_evaluate(self):
        ...
        def ansatz(x, y, z):
            ...
            qml.CNOT(wires=[0, 1])
            return qml.output.expval(qml.PauliZ(wires=0)), qml.output.expval(qml.PauliY(wires=1))
    ...
    def test_multiple_expectation_different_wires(self):
        ...
        def circuit(x, y, z):
            ...
            qml.RY(y, wires=[0])
            qml.RX(z, wires=[0])
            return qml.output.expval(qml.PauliY(wires=0)), qml.output.expval(qml.PauliZ(wires=1))
    ...
    def test_multiple_keywordargs_used(self):
        ...
        def circuit(w, x=None, y=None):
            qml.RX(x, wires=[0])
            qml.RX(y, wires=[1])
            return qml.output.expval(qml.PauliZ(wires=0)), qml.output.expval(qml.PauliZ(wires=1))
