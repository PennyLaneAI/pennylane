...
class TestInsert:
    ...
    with qp.queuing.AnnotatedQueue() as q_custom_tape:
        qp.RX(0.9, wires=0)
        qp.RY(0.4, wires=1)
        qp.CNOT(wires=[0, 1])
        qp.RY(0.5, wires=0)
    ...
    def test_invalid_position(self, pos):
        ...
        with pytest.raises(ValueError, match="Position must be either 'start', 'end', or 'all'"):
            insert(self.tape, qp.AmplitudeDamping, 0.4, position=pos)

    def test_start(self):
        """Test if the expected tape is returned when the start position is requested"""
        tapes, _ = insert(self.tape, qp.AmplitudeDamping, 0.4, position="start")
        tape = tapes[0]
    ...
    def test_all(self):
        ...
        tapes, _ = insert(self.tape, qp.PhaseDamping, 0.4, position="all")
        tape = tapes[0]
    ...
    def test_before(self):
        ...
        tapes, _ = insert(self.tape, qp.PhaseDamping, 0.4, position="all", before=True)
        tape = tapes[0]
        with qp.queuing.AnnotatedQueue() as q_tape_exp:
            qp.PhaseDamping(0.4, wires=0)
            qp.RX(0.9, wires=0)
    ...
    def test_operation_as_position(self, op):
        ...
        tapes, _ = insert(self.custom_tape, qp.PhaseDamping, 0.4, position=op, before=True)
        tape = tapes[0]
    ...
    def test_operation_list_as_position(self):
        ...
        tapes, _ = insert(self.tape, qp.PhaseDamping, 0.4, position=[qp.RX, qp.RY])
        tape = tapes[0]
    ...
    def test_end(self):
        ...
        tapes, _ = insert(self.tape, qp.GeneralizedAmplitudeDamping, [0.4, 0.5], position="end")
        tape = tapes[0]
        ...
        with qp.queuing.AnnotatedQueue() as q_tape_exp:
            ...
            qp.RY(0.5, wires=0)
            qp.RX(0.6, wires=1)
            qp.GeneralizedAmplitudeDamping(0.4, 0.5, wires=0)
            qp.GeneralizedAmplitudeDamping(0.4, 0.5, wires=1)
            qp.expval(qp.PauliZ(0) @ qp.PauliZ(1))
    ...
    def test_start_with_state_prep(self):
        ...
        tapes, _ = insert(self.tape_with_prep, qp.AmplitudeDamping, 0.4, position="start")
        tape = tapes[0]
    ...
    def test_all_with_state_prep(self):
        ...
        that has state preparation"""
        tapes, _ = insert(self.tape_with_prep, qp.PhaseDamping, 0.4, position="all")
        tape = tapes[0]
    ...
    def test_end_with_state_prep(self):
        ...
        that has state preparation"""
        tapes, _ = insert(
            self.tape_with_prep, qp.GeneralizedAmplitudeDamping, [0.4, 0.5], position="end"
        )
        tape = tapes[0]
...
def test_insert_qnode():
    ...
    @insert(op=qp.AmplitudeDamping, op_args=0.2, position="end")
    @qp.qnode(dev)
    def f_noisy(w, x, y, z):
        qp.RX(w, wires=0)
        qp.RY(x, wires=1)
...
def test_insert_dev(dev_name):
    ...
    new_dev = insert(dev, qp.PhaseShift, 0.4)
...
def test_insert_template():
    ...
    @insert(op=qp.PhaseDamping, op_args=0.3, position="all")
    @qp.qnode(dev)
    def f1(w1, w2):
