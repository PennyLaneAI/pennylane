...
"""
Tests for the ``adjoint_metric_tensor`` method of the :mod:`pennylane` :class:`QubitDevice` class.
"""
import pytest
...
class TestAdjointMetricTensor:
    """Tests for the adjoint_metric_tensor method"""

    @pytest.fixture
    def dev(self):
        return qml.device("default.qubit", wires=2)

    def test_no_generator(self, dev):
        """Test if a QuantumFunctionError is raised for a tape with operations that do not
        have a generator."""

        class RX_without_gen(qml.RX):
            generator = [None, 1]

        with qml.tape.JacobianTape() as tape:
            qml.RX(0.1, wires=0)
            RX_without_gen(0.1, wires=0)
            qml.expval(qml.PauliZ(0))

        with pytest.raises(qml.QuantumFunctionError, match="The adjoint metric tensor method"):
            dev.adjoint_metric_tensor(tape)
    ...
    @pytest.mark.parametrize(
            "G, expectation",
            [(qml.RX, 0.0), (qml.RY, 0.25), (qml.RZ, 0.25)])
    def test_single_pauli_rotation_mt(self, G, theta, expectation, tol, dev):
        """Tests that the adjoint metric tensor of a single Pauli rotation is correct."""
        ...
        adjoint_mt = dev.adjoint_metric_tensor(tape)

        # compare to expectation which is simple for a single Pauli rotation
        expected_mt = np.array([[expectation]])
        assert np.allclose(adjoint_mt, expected_mt, atol=tol, rtol=0)

    @pytest.mark.parametrize("theta", np.linspace(-2 * np.pi, 2 * np.pi, 7))
    def test_Rot_mt(self, theta, tol, dev):
        """Tests that the adjoint metric tensor of an arbitrary
        Euler-angle-parameterized gate is correct."""
        ...
        T = np.array([0.0, 1j*np.sin(params[0])/2, -1j*np.cos(params[0])*np.sin(params[1])/2])
        L = np.array([[0.25, 0.0, np.cos(params[1])/4],
                      [0.0, 0.25, 0.0],
                      [np.cos(params[1])/4, 0.0, 0.25]])
        expected = L-np.outer(np.conj(T), T)

        assert np.allclose(adjoint_mt, expected, atol=tol, rtol=0)

class TestAdjointMetricTensorQNode:
    """Test QNode integration with the adjoint_metric_tensor method"""
    ...
    def test_qnode(self, mocker, tol, dev):
        ...
        spy = mocker.spy(dev, "adjoint_metric_tensor")
