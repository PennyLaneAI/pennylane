...
class TestResourceHintValidation:
    ...
    def test_collect_estimated_probabilities(self):
        assert collect_estimated_probabilities([0.75]) == (0.75,)
        assert collect_estimated_probabilities([None, None]) is None

    def test_validate_estimated_probabilities_sum_too_large(self):
        with pytest.raises(ValueError, match="sum to at most 1"):
            validate_estimated_probabilities([0.6, 0.6])

    def test_collect_estimated_probabilities_partial(self):
        with pytest.raises(ValueError, match="every non-default branch"):
            collect_estimated_probabilities([0.5, None])
...
def test_cond_elif_probabilities_in_jaxpr():
    """Multi-branch cond collects estimated_probability from cond and else_if calls."""
    import jax
    ...
    def workflow(x):
        @qp.cond(x > 2, estimated_probability=0.2)
        def branch():
            return 1

        @branch.else_if(x > 0, estimated_probability=0.3)
        def elif_branch():
            return 2
