...
class TestLieClosure:
    ...
    def test_verbose(self, capsys, dense):
        """Test the verbose output"""
        gen11 = dla11[:-1]
        _ = lie_closure(gen11, verbose=True, dense=dense)
    ...
    @pytest.mark.parametrize("dense", [False, True])
    def test_verbose_false(self, capsys, dense):
        """Test that there is no output when verbose is False"""
        gen11 = dla11[:-1]
        _ = lie_closure(gen11, verbose=False, dense=dense)
        captured = capsys.readouterr()
        assert captured.out == ""
    ...
    @pytest.mark.parametrize("dense", [False, True])
    def test_max_iterations(self, capsys, dense):
        ...
        with pytest.warns(UserWarning, match="reached the maximum number of iterations"):
            res = qml.pauli.lie_closure(generators, verbose=True, max_iterations=1, dense=dense)
    ...
    def test_lie_closure_transverse_field_ising_1D_open(self, n, dense):
        ...
        res = qml.pauli.lie_closure(generators, dense=dense)
        assert len(res) == (2 * n - 1) * (2 * n - 2) // 2

    @pytest.mark.parametrize("dense", [False, True])
    @pytest.mark.parametrize("n", range(3, 5))
    def test_lie_closure_transverse_field_ising_1D_cyclic(self, n, dense):
    ...
    @pytest.mark.parametrize("dense", [False, True])
    def test_lie_closure_heisenberg_generators_odd(self, dense):
    ...
    @pytest.mark.parametrize("dense", [False, True])
    def test_lie_closure_heisenberg_generators_even(self, dense):
    ...
    def test_lie_closure_heisenberg(self, n, res, dense):
        ...
        g = qml.lie_closure(generators, dense=dense)
        assert len(g) == res

    @pytest.mark.parametrize("dense", [False, True])
    def test_universal_gate_set(self, dense):
        ...
        vspace = qml.lie_closure(generators, dense=dense)

        assert len(vspace) == 4**3


### Test dense capabilities of lie_closure
...
class TestLieClosureDense:
    """Tests for lie_closure(*args, dense=True)"""
    ...
    def test_simple_lie_closure_dense(self):
        ...
        gen12 = dla12[:-1]
        res12 = lie_closure(gen12, dense=True)
    ...
    def test_lie_closure_dense_with_pl_ops(self):
        ...
        gen11 = dla[:-1]
        res11 = lie_closure(gen11, dense=True)
    ...
    def test_lie_closure_dense_with_PauliWords(self):
        ...
        res = lie_closure(gen, dense=True)
