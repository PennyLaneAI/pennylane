...
class TestCartanSubalgebra:
    """Tests for qp.liealg.horizontal_cartan_subalgebra"""
    ...
    def test_horizontal_cartan_subalgebra_Ising(self, n, len_g, len_h, len_mtilde, provide_adj):
        ...
        g = qp.lie_closure(gens, pauli=True)
        ...
        new_adj_re = qp.structure_constants(newg)
    ...
    def test_horizontal_cartan_subalgebra_matrix_input(self, start_idx):
        ...
        k = np.array([qp.matrix(op, wire_order=range(2)) for op in k])
        m = np.array([qp.matrix(op, wire_order=range(2)) for op in m])

        newg, k, mtilde, h, new_adj = horizontal_cartan_subalgebra(k, m, start_idx=start_idx)
        assert len(h) + len(mtilde) == len(m)

        new_adj_re = qp.structure_constants(newg, matrix=True)

        assert np.allclose(new_adj_re, new_adj)
        assert qp.liealg.check_abelian(h)
    ...
    def test_horizontal_cartan_subalgebra_adjvec_output(self):
        ...
        np_newg, _, np_mtilde, np_h, new_adj = qp.liealg.horizontal_cartan_subalgebra(
    ...
    def test_horizontal_cartan_subalgebra_verbose(self, capsys):
        ...
        k = [1.0 * Z(0), 1.0 * Z(1)]
        m = [1.0 * X(0) @ X(1), -1.0 * Y(0) @ X(1), -1.0 * X(0) @ Y(1), 1.0 * Y(0) @ Y(1)]
        _ = qp.liealg.horizontal_cartan_subalgebra(k, m, verbose=True)
...
class TestChangeBasisAdRep:
    ...
    def test_permutation(self, seed):
        ...
        ops = [qp.X(0), qp.Y(1), qp.Y(0) @ qp.Z(1), qp.X(1)]
        dla = qp.lie_closure(ops)
        adj = qp.structure_constants(dla)
        perm = rng.permutation(len(dla))
        permuted_dla = [dla[i] for i in perm]
        permuted_adj = qp.structure_constants(permuted_dla)
    ...
    def test_tiny_skewed_basis(self, seed):
        ...
        rng = np.random.default_rng(seed)
        dla = [qp.X(0), qp.Y(0), qp.Z(0)]
        adj = qp.structure_constants(dla)
        coeffs = rng.random((len(dla), len(dla)))
        skewed_dla = [qp.sum(*(c * op for c, op in zip(_coeffs, dla))) for _coeffs in coeffs]
        skewed_adj = qp.structure_constants(skewed_dla, is_orthogonal=False)
    ...
    def test_tiny_skewed_basis_from_non_ortho(self, seed):
        ...
        rng = np.random.default_rng(seed)
        ortho_dla = [qp.X(0), qp.Y(0), qp.Z(0)]  # only used to create adj rep.
        dla = [0.2 * qp.X(0) - 0.6 * qp.Y(0), 0.4 * qp.Y(0) + 0.9 * qp.Z(0), qp.Z(0)]

        adj = qp.structure_constants(dla, is_orthogonal=False)
        coeffs = rng.random((len(dla), len(dla)))
        skewed_dla = [qp.sum(*(c * op for c, op in zip(_coeffs, dla))) for _coeffs in coeffs]
        skewed_adj = qp.structure_constants(skewed_dla, is_orthogonal=False)
    ...
    def test_skewed_basis(self, seed):
        ...
        rng = np.random.default_rng(seed)
        ops = [qp.X(0), qp.Y(1), qp.Y(0) @ qp.Z(1)]
        dla = qp.lie_closure(ops)
        adj = qp.structure_constants(dla)
        coeffs = rng.random((len(dla), len(dla)))
        skewed_dla = [qp.sum(*(c * op for c, op in zip(_coeffs, dla))) for _coeffs in coeffs]
    ...
    def test_skewed_basis_from_non_ortho(self, seed):
        ...
        rng = np.random.default_rng(seed)
        ops = [qp.X(0), qp.Y(1), qp.Y(0) @ qp.Z(1)]
        ortho_dla = qp.lie_closure(ops)  # only used to create adj rep.

        coeffs = rng.random((len(ortho_dla), len(ortho_dla)))
        dla = [qp.sum(*(c * op for c, op in zip(_coeffs, ortho_dla))) for _coeffs in coeffs]
        adj = qp.structure_constants(dla, is_orthogonal=False)
...
def _generate_test_cases(seed_value):
    ...
    paulis_1_qubit = [op.pauli_rep for op in qp.pauli.pauli_group(1)]
    paulis_2_qubits = [op.pauli_rep for op in qp.pauli.pauli_group(2)]

    # Create a non-orthonormal basis on 1 qubit
    basis_coeffs_0 = rng.random((4, 4))
    basis_0 = [qp.dot(c, paulis_1_qubit).pauli_rep for c in basis_coeffs_0]
    ...
    expected_0 = [qp.dot(c, basis_0).pauli_rep for c in adjvec_0 @ inv_sqrt_Gram_0]
    ...
    expected_1 = [qp.dot(c, paulis_1_qubit).pauli_rep for c in adjvec_0]

    # Two-qubit test cases
    # Create a non-orthonormal basis on 2 qubits
    basis_coeffs_1 = rng.random((16, 16))
    basis_1 = [qp.dot(c, paulis_2_qubits).pauli_rep for c in basis_coeffs_1]
    ...
    expected_2 = [qp.dot(c, basis_1).pauli_rep for c in adjvec_1 @ inv_sqrt_Gram_1]
    ...
    expected_3 = [qp.dot(c, paulis_2_qubits).pauli_rep for c in adjvec_1]

    # Collect all test cases. All operators are formatted as qp.pauli.PauliSentence
    ps_test_cases = [
    ...
    # Translate test cases to qp.operation.Operations
    op_test_cases = [
    ...
    dense_test_cases = [
        (
            adj_vecs,
            [qp.matrix(ps, wire_order=[0, 1]) for ps in basis],
            [qp.matrix(ps, wire_order=[0, 1]) for ps in expected],
            is_ortho,
        )
        for adj_vecs, basis, expected, is_ortho in ps_test_cases
    ]
...
class TestAdjvecToOp(TestData):
    ...
    def test_NotImplementedError(self):
        ...
        with pytest.raises(
            ...
            _ = adjvec_to_op([[0.0]], [qp.pauli.PauliWord({0: "X"})])
    ...
    def test_with_ps(self, ps_test_case, vspace):
        ...
        if vspace:
            basis = qp.pauli.PauliVSpace(basis)
    ...
    def test_with_op(self, op_test_case):
        ...
        out = adjvec_to_op(adj_vecs, basis, is_orthogonal=False)
        for out_op, exp_op in zip(out, expected):
            assert qp.equal(out_op.simplify(), exp_op.simplify())
        if is_ortho:
            out = adjvec_to_op(adj_vecs, basis, is_orthogonal=True)
            for out_op, exp_op in zip(out, expected):
                assert qp.equal(out_op.simplify(), exp_op.simplify())
    ...
    def test_with_dense(self, dense_test_case):
        ...
        out = adjvec_to_op(adj_vecs, basis, is_orthogonal=False)
        assert qp.math.shape(out) == qp.math.shape(expected)
...
class TestOpToAdjvec(TestData):
    ...
    def test_op_and_dense(self):
        ...
        H = X(0) + Y(0)
        basis = [X(0), Y(0), Z(0)]
        basis = [qp.matrix(op) for op in basis]

        adjvec = qp.liealg.op_to_adjvec([H], basis)
    ...
    def test_consistent_with_input_types(self):
        ...
        g = list(qp.pauli.pauli_group(3))  # su(8)
        g = qp.lie_closure(g, matrix=True)
...
def test_check_abelian_ops():
    ...
    assert qp.liealg.check_abelian(abelian_group0)
    assert not qp.liealg.check_abelian(non_abelian_group0)


def test_check_abelian_matrix():
    """Test check_abelian with matrices as inputs"""
    abelian_group = [qp.matrix(op, wire_order=range(3)) for op in abelian_group0]
    assert qp.liealg.check_abelian(abelian_group)
    non_abelian_group = [qp.matrix(op, wire_order=range(3)) for op in non_abelian_group0]
    assert not qp.liealg.check_abelian(non_abelian_group)
...
def test_check_abelian_NotImplemented():
    ...
    with pytest.raises(NotImplementedError, match="At least one operator in the"):
        _ = qp.liealg.check_abelian([qp.pauli.PauliWord({0: "X"})])
