...
from pennylane.tape.circuit_graph import TapeCircuitGraph as CircuitGraph
...
@pytest.fixture
def obs():
    """A fixture of observables to go after the ops fixture."""
    return [
...
class TestCircuitGraph:
    ...
    def test_dependence(self, ops, obs):
        ...
        for k in graph.nodes:
            assert k is all_ops[k.queue_idx]
    ...
    def test_ancestors_and_descendants_example(self, ops, obs):
        ...
        ancestors = circuit.ancestors([all_ops[6]])
        assert len(ancestors) == 3
        for o_idx in (0, 1, 3):
            assert all_ops[o_idx] in ancestors

        descendants = circuit.descendants([all_ops[6]])
        assert descendants == set([all_ops[8]])
