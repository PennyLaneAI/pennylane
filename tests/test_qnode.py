...
class BasicTest(BaseTest):
    ...
    def test_op_successors(self):
        ...
        def qf(x):
            ...
            qml.RY(0.4, wires=[0])
            qml.RZ(-0.2, wires=[1])
            return qml.output.expval(qml.PauliX(wires=0)), qml.output.expval(qml.PauliZ(wires=1))
    ...
    def test_jacobian_fail(self):
        ...
        def qf(x):
            qml.RX(x, wires=[0])
            return qml.output.expval(qml.Hermitian(np.diag([x, 0]), wires=0))
        q = qml.QNode(qf, self.dev2)
...
class TestQNodeGradients:
    ...
    def test_qnode_cv_gradient_methods(self):
        ...
        def qf(x, y):
            ...
            #qml.Kerr(0.4, [0])
            return qml.output.expval(qml.X(wires=0)), qml.output.expval(qml.X(wires=1))
    ...
    def test_qnode_gradient_repeated_gate_parameters(self, tol):
        ...
        def qf(x, y):
            ...
            return qml.output.expval(qml.PauliX(wires=0))
    ...
    def test_qnode_gradient_parameters_inside_array(self, tol):
        ...
        def qf(x, y):
            qml.RX(x, wires=[0])
            qml.RY(x, wires=[0])
            return qml.output.expval(qml.Hermitian(np.diag([y, 1]), wires=0))
    ...
    def test_differentiate_all_positional(self, tol):
        ...
        def circuit1(a, b, c):
            ...
            qml.RX(b, wires=1)
            qml.RX(c, wires=2)
            return tuple(qml.output.expval(qml.PauliZ(wires=idx)) for idx in range(3))
    ...
    def test_differentiate_positional_multidim(self, tol):
        ...
        def circuit(a, b):
            ...
            return qml.output.expval(qml.PauliZ(wires=0)), qml.output.expval(qml.PauliZ(wires=1)), qml.output.expval(qml.PauliZ(wires=2))
