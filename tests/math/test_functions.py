...
@pytest.mark.parametrize(
    "tensor,expected", [(torch.tensor(0), 0), (torch.tensor([0]), 1), (torch.tensor([[0]]), 2)]
)
def test_ndim_torch(tensor, expected):
    """Tests the registered ndim function of Torch"""
    t = torch.tensor(tensor)
    assert qml.math.ndim(t) == expected
